...
# Usage of this file
#
# (1) Build CPU version of edgevox-onnx
#    ./build-aarch64-linux-gnu.sh
#
# (2) Build GPU version of edgevox-onnx
#
#   (a) Make sure your board has NVIDIA GPU(s)
#
#   (b) For Jetson Nano B01 (using CUDA 10.2, cudnn8)
#
#       export EDGEVOX_ONNX_ENABLE_GPU=ON
#       export EDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION=1.11.0
#       ./build-aarch64-linux-gnu.sh
#
#   (c) For Jetson Orin NX (using CUDA 11.4, cudnn8)
#
#       export EDGEVOX_ONNX_ENABLE_GPU=ON
#       export EDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION=1.16.0
#       ./build-aarch64-linux-gnu.sh
#
...
#
#       export EDGEVOX_ONNX_ENABLE_GPU=ON
#       export EDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION=1.18.1
#       ./build-aarch64-linux-gnu.sh
#
#   (e) For  NVIDIA Jetson Orin NX (16GB ram) with CUDA 12.2, cudnn8
#
#       export EDGEVOX_ONNX_ENABLE_GPU=ON
#       export EDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION=1.18.0
#       ./build-aarch64-linux-gnu.sh
...
if [[ x"$EDGEVOX_ONNX_ENABLE_GPU" == x"" ]]; then
  # By default, don't use CPU
  EDGEVOX_ONNX_ENABLE_GPU=OFF
fi

if [[ x"$EDGEVOX_ONNX_ENABLE_GPU" == x"ON" ]]; then
  # Build shared libs if building GPU is enabled.
  BUILD_SHARED_LIBS=ON
fi

if [[ x"$EDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION" == x"" ]]; then
  # Used only when EDGEVOX_ONNX_ENABLE_GPU is ON
  EDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION="1.11.0"
fi
...
cmake \
  ...
  -DEDGEVOX_ONNX_ENABLE_GPU=$EDGEVOX_ONNX_ENABLE_GPU \
  ...
  -DEDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION=$EDGEVOX_ONNX_LINUX_ARM64_GPU_ONNXRUNTIME_VERSION \
