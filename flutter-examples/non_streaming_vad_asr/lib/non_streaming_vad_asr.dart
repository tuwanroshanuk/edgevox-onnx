...
Future<edgevox_onnx.OfflineRecognizer> createOfflineRecognizer() async {
  final type = 2;
  ...
  return edgevox_onnx.OfflineRecognizer(config);
}
...
class _NoStreamingAsrVAdScreenState extends State<NoStreamingAsrVAdScreen> {
  ...
  edgevox_onnx.OfflineRecognizer? _recognizer;
  static const int _sampleRate = 16000;

  // VAD related vars
  edgevox_onnx.VoiceActivityDetector? _vad;
  edgevox_onnx.CircularBuffer? _buffer;
  
  // VAD config
  late edgevox_onnx.VadModelConfig _vadConfig;
  ...
  Future<void> _start() async {
    if (!_isInitialized) {
      edgevox_onnx.initBindings();

      // 初始化 VAD
      final sileroVadConfig = edgevox_onnx.SileroVadModelConfig(
      ...
      _vadConfig = edgevox_onnx.VadModelConfig(
      ...
      _vad = edgevox_onnx.VoiceActivityDetector(
        config: _vadConfig, 
        bufferSizeInSeconds: 30
      );
      _buffer = edgevox_onnx.CircularBuffer(capacity: 30 * _sampleRate);
