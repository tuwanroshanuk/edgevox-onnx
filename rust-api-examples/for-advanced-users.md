...
This note is for users who want to control how the `edgevox-onnx` Rust crate
...
## Use your own edgevox-onnx libraries

If you already have edgevox-onnx libraries on disk, set
`EDGEVOX_ONNX_LIB_DIR` to the `lib` directory before building:

```bash
export EDGEVOX_ONNX_LIB_DIR=/path/to/edgevox-onnx/lib
```

Examples:

- source build output: `/path/to/edgevox-onnx/build/install/lib`
- manually extracted release archive:
  `/path/to/edgevox-onnx-v1.13.4-linux-x64-static-lib/lib`

If `EDGEVOX_ONNX_LIB_DIR` is set, the build script uses that directory and does
not auto-download another archive.

## Automatic download behavior

If `EDGEVOX_ONNX_LIB_DIR` is not set, `edgevox-onnx-sys/build.rs` downloads a
...
|----|--------------|-----------------|
| Linux | x86_64 | `edgevox-onnx-v1.13.4-linux-x64-static-lib.tar.bz2` |
| Linux | aarch64 | `edgevox-onnx-v1.13.4-linux-aarch64-static-lib.tar.bz2` |
| macOS | x86_64 | `edgevox-onnx-v1.13.4-osx-x64-static-lib.tar.bz2` |
| macOS | arm64 | `edgevox-onnx-v1.13.4-osx-arm64-static-lib.tar.bz2` |
| Windows | x86_64 | `edgevox-onnx-v1.13.4-win-x64-static-MT-Release-lib.tar.bz2` |
...
| OS | Architecture | Archive example |
|----|--------------|-----------------|
| Linux | x86_64 | `edgevox-onnx-v1.13.4-linux-x64-shared-lib.tar.bz2` |
| Linux | aarch64 | `edgevox-onnx-v1.13.4-linux-aarch64-shared-cpu-lib.tar.bz2` |
| macOS | x86_64 | `edgevox-onnx-v1.13.4-osx-x64-shared-lib.tar.bz2` |
| macOS | arm64 | `edgevox-onnx-v1.13.4-osx-arm64-shared-lib.tar.bz2` |
| Windows | x86_64 | `edgevox-onnx-v1.13.4-win-x64-shared-MT-Release-lib.tar.bz2` |

In practice, use the latest release tag instead of the example version above.

## Configure the `edgevox-onnx` crate in Cargo.toml
...
```toml
[dependencies]
edgevox-onnx = { version = "1.13.4", default-features = false, features = ["shared"] }
```
...
When `EDGEVOX_ONNX_LIB_DIR` is set, the same behavior applies, but the files come
