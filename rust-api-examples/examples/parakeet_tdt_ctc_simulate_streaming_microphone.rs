...
//
// This file demonstrates how to use Parakeet TDT CTC with edgevox-onnx's
...
// Parakeet TDT CTC recognizer on each detected segment, providing an experience
...
fn create_vad(args: &Args) -> VoiceActivityDetector {
    ...
    VoiceActivityDetector::create(&config, 60.0).expect("Failed to create VAD")
}

fn create_recognizer(args: &Args) -> OfflineRecognizer {
    let mut config = OfflineRecognizerConfig::default();
    config.model_config.nemo_ctc.model = Some(args.model.clone());
