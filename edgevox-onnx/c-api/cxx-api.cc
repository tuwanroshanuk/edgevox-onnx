...
Wave ReadWave(const std::string &filename) {
  auto p = EdgevoxOnnxReadWave(filename.c_str());
...
bool WriteWave(const std::string &filename, const Wave &wave) {
  return EdgevoxOnnxWriteWave(wave.samples.data(), wave.samples.size(),
                             wave.sample_rate, filename.c_str());
}

OnlineStream::OnlineStream(const EdgevoxOnnxOnlineStream *p)
    : MoveOnly<OnlineStream, EdgevoxOnnxOnlineStream>(p) {}

void OnlineStream::Destroy(const EdgevoxOnnxOnlineStream *p) const {
  EdgevoxOnnxDestroyOnlineStream(p);
}

void OnlineStream::AcceptWaveform(int32_t sample_rate, const float *samples,
                                  int32_t n) const {
  EdgevoxOnnxOnlineStreamAcceptWaveform(p_, sample_rate, samples, n);
}

void OnlineStream::InputFinished() const {
  EdgevoxOnnxOnlineStreamInputFinished(p_);
}

void OnlineStream::SetOption(const char *key, const char *value) const {
  EdgevoxOnnxOnlineStreamSetOption(p_, key, value);
}

const char *OnlineStream::GetOption(const char *key) const {
  return EdgevoxOnnxOnlineStreamGetOption(p_, key);
}

int32_t OnlineStream::HasOption(const char *key) const {
  return EdgevoxOnnxOnlineStreamHasOption(p_, key);
}

OnlineRecognizer OnlineRecognizer::Create(
    const OnlineRecognizerConfig &config) {
  struct EdgevoxOnnxOnlineRecognizerConfig c;
  memset(&c, 0, sizeof(c));
  ...
  auto p = EdgevoxOnnxCreateOnlineRecognizer(&c);
  return OnlineRecognizer(p);
}

OnlineRecognizer::OnlineRecognizer(const EdgevoxOnnxOnlineRecognizer *p)
    : MoveOnly<OnlineRecognizer, EdgevoxOnnxOnlineRecognizer>(p) {}

void OnlineRecognizer::Destroy(const EdgevoxOnnxOnlineRecognizer *p) const {
  EdgevoxOnnxDestroyOnlineRecognizer(p);
}

OnlineStream OnlineRecognizer::CreateStream() const {
  auto s = EdgevoxOnnxCreateOnlineStream(p_);
  return OnlineStream{s};
}

OnlineStream OnlineRecognizer::CreateStream(const std::string &hotwords) const {
  auto s = EdgevoxOnnxCreateOnlineStreamWithHotwords(p_, hotwords.c_str());
  return OnlineStream{s};
}

bool OnlineRecognizer::IsReady(const OnlineStream *s) const {
  return EdgevoxOnnxIsOnlineStreamReady(p_, s->Get());
}

void OnlineRecognizer::Decode(const OnlineStream *s) const {
  EdgevoxOnnxDecodeOnlineStream(p_, s->Get());
}

void OnlineRecognizer::Reset(const OnlineStream *s) const {
  EdgevoxOnnxOnlineStreamReset(p_, s->Get());
}

bool OnlineRecognizer::IsEndpoint(const OnlineStream *s) const {
  return EdgevoxOnnxOnlineStreamIsEndpoint(p_, s->Get());
}
...
void OnlineRecognizer::Decode(const OnlineStream *ss, int32_t n) const {
  ...
  std::vector<const EdgevoxOnnxOnlineStream *> streams(n);
  for (int32_t i = 0; i != n; ++i) {
    streams[i] = ss[i].Get();
  }

  EdgevoxOnnxDecodeMultipleOnlineStreams(p_, streams.data(), n);
}

OnlineRecognizerResult OnlineRecognizer::GetResult(
    const OnlineStream *s) const {
  auto r = EdgevoxOnnxGetOnlineStreamResult(p_, s->Get());
...
// Non-streaming ASR
// ============================================================================
OfflineStream::OfflineStream(const EdgevoxOnnxOfflineStream *p)
    : MoveOnly<OfflineStream, EdgevoxOnnxOfflineStream>(p) {}

void OfflineStream::Destroy(const EdgevoxOnnxOfflineStream *p) const {
  EdgevoxOnnxDestroyOfflineStream(p);
}

void OfflineStream::AcceptWaveform(int32_t sample_rate, const float *samples,
                                   int32_t n) const {
  EdgevoxOnnxAcceptWaveformOffline(p_, sample_rate, samples, n);
}

void OfflineStream::SetOption(const char *key, const char *value) const {
  EdgevoxOnnxOfflineStreamSetOption(p_, key, value);
}

const char *OfflineStream::GetOption(const char *key) const {
  return EdgevoxOnnxOfflineStreamGetOption(p_, key);
}

int32_t OfflineStream::HasOption(const char *key) const {
  return EdgevoxOnnxOfflineStreamHasOption(p_, key);
}

static EdgevoxOnnxOfflineRecognizerConfig Convert(
    const OfflineRecognizerConfig &config) {
  struct EdgevoxOnnxOfflineRecognizerConfig c;
  memset(&c, 0, sizeof(c));
...
OfflineRecognizer OfflineRecognizer::Create(
  ...
  auto p = EdgevoxOnnxCreateOfflineRecognizer(&c);
  return OfflineRecognizer(p);
}

void OfflineRecognizer::SetConfig(const OfflineRecognizerConfig &config) const {
  auto c = Convert(config);
  EdgevoxOnnxOfflineRecognizerSetConfig(p_, &c);
}

OfflineRecognizer::OfflineRecognizer(const EdgevoxOnnxOfflineRecognizer *p)
    : MoveOnly<OfflineRecognizer, EdgevoxOnnxOfflineRecognizer>(p) {}

void OfflineRecognizer::Destroy(const EdgevoxOnnxOfflineRecognizer *p) const {
  EdgevoxOnnxDestroyOfflineRecognizer(p);
}

OfflineStream OfflineRecognizer::CreateStream() const {
  auto s = EdgevoxOnnxCreateOfflineStream(p_);
  return OfflineStream{s};
}

OfflineStream OfflineRecognizer::CreateStream(
    const std::string &hotwords) const {
  auto s = EdgevoxOnnxCreateOfflineStreamWithHotwords(p_, hotwords.c_str());
  return OfflineStream{s};
}

void OfflineRecognizer::Decode(const OfflineStream *s) const {
  EdgevoxOnnxDecodeOfflineStream(p_, s->Get());
}
...
void OfflineRecognizer::Decode(const OfflineStream *ss, int32_t n) const {
  ...
  std::vector<const EdgevoxOnnxOfflineStream *> streams(n);
  for (int32_t i = 0; i != n; ++i) {
    streams[i] = ss[i].Get();
  }

  EdgevoxOnnxDecodeMultipleOfflineStreams(p_, streams.data(), n);
}

OfflineRecognizerResult OfflineRecognizer::GetResult(
    const OfflineStream *s) const {
  auto r = EdgevoxOnnxGetOfflineStreamResult(s->Get());
...
OfflineTts OfflineTts::Create(const OfflineTtsConfig &config) {
  struct EdgevoxOnnxOfflineTtsConfig c;
  memset(&c, 0, sizeof(c));
  ...
  c.model.zipvoice.espeak_voice = config.model.zipvoice.espeak_voice.c_str();
  ...
  auto p = EdgevoxOnnxCreateOfflineTts(&c);
  return OfflineTts(p);
}

OfflineTts::OfflineTts(const EdgevoxOnnxOfflineTts *p)
    : MoveOnly<OfflineTts, EdgevoxOnnxOfflineTts>(p) {}

void OfflineTts::Destroy(const EdgevoxOnnxOfflineTts *p) const {
  EdgevoxOnnxDestroyOfflineTts(p);
}

int32_t OfflineTts::SampleRate() const {
  return EdgevoxOnnxOfflineTtsSampleRate(p_);
}

int32_t OfflineTts::NumSpeakers() const {
  return EdgevoxOnnxOfflineTtsNumSpeakers(p_);
}
...
GeneratedAudio OfflineTts::Generate(const std::string &text,
  ...
  const EdgevoxOnnxGeneratedAudio *audio =
      EdgevoxOnnxOfflineTtsGenerateWithConfig(p_, text.c_str(), &c, callback,
                                             arg);
...
KeywordSpotter KeywordSpotter::Create(const KeywordSpotterConfig &config) {
  struct EdgevoxOnnxKeywordSpotterConfig c;
  memset(&c, 0, sizeof(c));
  ...
  auto p = EdgevoxOnnxCreateKeywordSpotter(&c);
  return KeywordSpotter(p);
}

KeywordSpotter::KeywordSpotter(const EdgevoxOnnxKeywordSpotter *p)
    : MoveOnly<KeywordSpotter, EdgevoxOnnxKeywordSpotter>(p) {}

void KeywordSpotter::Destroy(const EdgevoxOnnxKeywordSpotter *p) const {
  EdgevoxOnnxDestroyKeywordSpotter(p);
}

OnlineStream KeywordSpotter::CreateStream() const {
  auto s = EdgevoxOnnxCreateKeywordStream(p_);
  return OnlineStream{s};
}

OnlineStream KeywordSpotter::CreateStream(const std::string &keywords) const {
  auto s = EdgevoxOnnxCreateKeywordStreamWithKeywords(p_, keywords.c_str());
  return OnlineStream{s};
}

bool KeywordSpotter::IsReady(const OnlineStream *s) const {
  return EdgevoxOnnxIsKeywordStreamReady(p_, s->Get());
}

void KeywordSpotter::Decode(const OnlineStream *s) const {
  return EdgevoxOnnxDecodeKeywordStream(p_, s->Get());
}
...
void KeywordSpotter::Decode(const OnlineStream *ss, int32_t n) const {
  ...
  EdgevoxOnnxDecodeMultipleKeywordStreams(p_, streams.data(), n);
}

KeywordResult KeywordSpotter::GetResult(const OnlineStream *s) const {
  auto r = EdgevoxOnnxGetKeywordResult(p_, s->Get());
...
void KeywordSpotter::Reset(const OnlineStream *s) const {
  EdgevoxOnnxResetKeywordStream(p_, s->Get());
}
...
OfflineSpeechDenoiser OfflineSpeechDenoiser::Create(
  ...
  struct EdgevoxOnnxOfflineSpeechDenoiserConfig c;
  FillSpeechDenoiserModelConfig(config.model, &c.model);

  auto p = EdgevoxOnnxCreateOfflineSpeechDenoiser(&c);

  return OfflineSpeechDenoiser(p);
}

void OfflineSpeechDenoiser::Destroy(
    const EdgevoxOnnxOfflineSpeechDenoiser *p) const {
  EdgevoxOnnxDestroyOfflineSpeechDenoiser(p);
}

OfflineSpeechDenoiser::OfflineSpeechDenoiser(
    const EdgevoxOnnxOfflineSpeechDenoiser *p)
    : MoveOnly<OfflineSpeechDenoiser, EdgevoxOnnxOfflineSpeechDenoiser>(p) {}

DenoisedAudio OfflineSpeechDenoiser::Run(const float *samples, int32_t n,
                                         int32_t sample_rate) const {
  auto audio = EdgevoxOnnxOfflineSpeechDenoiserRun(p_, samples, n, sample_rate);
  if (audio == nullptr) {
    return {};
  }
...
int32_t OfflineSpeechDenoiser::GetSampleRate() const {
  return EdgevoxOnnxOfflineSpeechDenoiserGetSampleRate(p_);
}

OnlineSpeechDenoiser OnlineSpeechDenoiser::Create(
    const OnlineSpeechDenoiserConfig &config) {
  struct EdgevoxOnnxOnlineSpeechDenoiserConfig c;
  FillSpeechDenoiserModelConfig(config.model, &c.model);

  auto p = EdgevoxOnnxCreateOnlineSpeechDenoiser(&c);
  return OnlineSpeechDenoiser(p);
}

void OnlineSpeechDenoiser::Destroy(
    const EdgevoxOnnxOnlineSpeechDenoiser *p) const {
  EdgevoxOnnxDestroyOnlineSpeechDenoiser(p);
}

OnlineSpeechDenoiser::OnlineSpeechDenoiser(
    const EdgevoxOnnxOnlineSpeechDenoiser *p)
    : MoveOnly<OnlineSpeechDenoiser, EdgevoxOnnxOnlineSpeechDenoiser>(p) {}

DenoisedAudio OnlineSpeechDenoiser::Run(const float *samples, int32_t n,
                                        int32_t sample_rate) const {
  auto audio = EdgevoxOnnxOnlineSpeechDenoiserRun(p_, samples, n, sample_rate);
  if (audio == nullptr) {
    return {};
  }
...
DenoisedAudio OnlineSpeechDenoiser::Flush() const {
  auto audio = EdgevoxOnnxOnlineSpeechDenoiserFlush(p_);
  if (audio == nullptr) {
    return {};
  }
...
void OnlineSpeechDenoiser::Reset() const {
  EdgevoxOnnxOnlineSpeechDenoiserReset(p_);
}

int32_t OnlineSpeechDenoiser::GetSampleRate() const {
  return EdgevoxOnnxOnlineSpeechDenoiserGetSampleRate(p_);
}

int32_t OnlineSpeechDenoiser::GetFrameShiftInSamples() const {
  return EdgevoxOnnxOnlineSpeechDenoiserGetFrameShiftInSamples(p_);
}

CircularBuffer CircularBuffer::Create(int32_t capacity) {
  auto p = EdgevoxOnnxCreateCircularBuffer(capacity);
  return CircularBuffer(p);
}

CircularBuffer::CircularBuffer(const EdgevoxOnnxCircularBuffer *p)
    : MoveOnly<CircularBuffer, EdgevoxOnnxCircularBuffer>(p) {}

void CircularBuffer::Destroy(const EdgevoxOnnxCircularBuffer *p) const {
  EdgevoxOnnxDestroyCircularBuffer(p);
}

void CircularBuffer::Push(const float *samples, int32_t n) const {
  EdgevoxOnnxCircularBufferPush(p_, samples, n);
}

std::vector<float> CircularBuffer::Get(int32_t start_index, int32_t n) const {
  const float *samples = EdgevoxOnnxCircularBufferGet(p_, start_index, n);
  if (!samples) return {};
...
void CircularBuffer::Pop(int32_t n) const {
  EdgevoxOnnxCircularBufferPop(p_, n);
}

int32_t CircularBuffer::Size() const {
  return EdgevoxOnnxCircularBufferSize(p_);
}

int32_t CircularBuffer::Head() const {
  return EdgevoxOnnxCircularBufferHead(p_);
}

void CircularBuffer::Reset() const { EdgevoxOnnxCircularBufferReset(p_); }

VoiceActivityDetector VoiceActivityDetector::Create(
    const VadModelConfig &config, float buffer_size_in_seconds) {
  struct EdgevoxOnnxVadModelConfig c;
  memset(&c, 0, sizeof(c));
  ...
  auto p = EdgevoxOnnxCreateVoiceActivityDetector(&c, buffer_size_in_seconds);
  return VoiceActivityDetector(p);
}

VoiceActivityDetector::VoiceActivityDetector(
    const EdgevoxOnnxVoiceActivityDetector *p)
    : MoveOnly<VoiceActivityDetector, EdgevoxOnnxVoiceActivityDetector>(p) {}

void VoiceActivityDetector::Destroy(
    const EdgevoxOnnxVoiceActivityDetector *p) const {
  EdgevoxOnnxDestroyVoiceActivityDetector(p);
}

void VoiceActivityDetector::AcceptWaveform(const float *samples,
                                           int32_t n) const {
  EdgevoxOnnxVoiceActivityDetectorAcceptWaveform(p_, samples, n);
}

bool VoiceActivityDetector::IsEmpty() const {
  return EdgevoxOnnxVoiceActivityDetectorEmpty(p_);
}

bool VoiceActivityDetector ::IsDetected() const {
  return EdgevoxOnnxVoiceActivityDetectorDetected(p_);
}

void VoiceActivityDetector::Pop() const {
  EdgevoxOnnxVoiceActivityDetectorPop(p_);
}

void VoiceActivityDetector::Clear() const {
  EdgevoxOnnxVoiceActivityDetectorClear(p_);
}

SpeechSegment VoiceActivityDetector::Front() const {
  auto f = EdgevoxOnnxVoiceActivityDetectorFront(p_);
  ...
  EdgevoxOnnxDestroySpeechSegment(f);
...
void VoiceActivityDetector::Reset() const {
  EdgevoxOnnxVoiceActivityDetectorReset(p_);
}

void VoiceActivityDetector::Flush() const {
  EdgevoxOnnxVoiceActivityDetectorFlush(p_);
}
...
LinearResampler LinearResampler::Create(int32_t samp_rate_in_hz,
                                        ...
                                        int32_t num_zeros) {
  auto p = EdgevoxOnnxCreateLinearResampler(samp_rate_in_hz, samp_rate_out_hz,
                                           filter_cutoff_hz, num_zeros);
  return LinearResampler(p);
}

LinearResampler::LinearResampler(const EdgevoxOnnxLinearResampler *p)
    : MoveOnly<LinearResampler, EdgevoxOnnxLinearResampler>(p) {}

void LinearResampler::Destroy(const EdgevoxOnnxLinearResampler *p) const {
  EdgevoxOnnxDestroyLinearResampler(p);
}

void LinearResampler::Reset() const { EdgevoxOnnxLinearResamplerReset(p_); }

std::vector<float> LinearResampler::Resample(const float *input,
                                             int32_t input_dim,
                                             bool flush) const {
  auto out = EdgevoxOnnxLinearResamplerResample(p_, input, input_dim, flush);
  if (!out) return {};
...
int32_t LinearResampler::GetInputSamplingRate() const {
  return EdgevoxOnnxLinearResamplerResampleGetInputSampleRate(p_);
}

int32_t LinearResampler::GetOutputSamplingRate() const {
  return EdgevoxOnnxLinearResamplerResampleGetOutputSampleRate(p_);
}

std::string GetVersionStr() { return EdgevoxOnnxGetVersionStr(); }

std::string GetGitSha1() { return EdgevoxOnnxGetGitSha1(); }

std::string GetGitDate() { return EdgevoxOnnxGetGitDate(); }

bool FileExists(const std::string &filename) {
  return EdgevoxOnnxFileExists(filename.c_str());
}
...
OfflinePunctuation OfflinePunctuation::Create(
  ...
  struct EdgevoxOnnxOfflinePunctuationConfig c;
  memset(&c, 0, sizeof(c));
  ...
  const EdgevoxOnnxOfflinePunctuation *punct =
      EdgevoxOnnxCreateOfflinePunctuation(&c);
  return OfflinePunctuation(punct);
}

OfflinePunctuation::OfflinePunctuation(const EdgevoxOnnxOfflinePunctuation *p)
    : MoveOnly<OfflinePunctuation, EdgevoxOnnxOfflinePunctuation>(p) {}

void OfflinePunctuation::Destroy(const EdgevoxOnnxOfflinePunctuation *p) const {
  EdgevoxOnnxDestroyOfflinePunctuation(p);
}
...
OnlinePunctuation OnlinePunctuation::Create(
  ...
  struct EdgevoxOnnxOnlinePunctuationConfig c;
  memset(&c, 0, sizeof(c));
  ...
  const EdgevoxOnnxOnlinePunctuation *punct =
      EdgevoxOnnxCreateOnlinePunctuation(&c);
  return OnlinePunctuation(punct);
}

OnlinePunctuation::OnlinePunctuation(const EdgevoxOnnxOnlinePunctuation *p)
    : MoveOnly<OnlinePunctuation, EdgevoxOnnxOnlinePunctuation>(p) {}

void OnlinePunctuation::Destroy(const EdgevoxOnnxOnlinePunctuation *p) const {
  EdgevoxOnnxDestroyOnlinePunctuation(p);
}

std::string OnlinePunctuation::AddPunctuation(const std::string &text) const {
  const char *result = EdgevoxOnnxOnlinePunctuationAddPunct(p_, text.c_str());
  if (!result) return {};
  std::string ans(result);
  EdgevoxOnnxOnlinePunctuationFreeText(result);
  return ans;
}
...
AudioTagging AudioTagging::Create(const AudioTaggingConfig &config) {
  struct EdgevoxOnnxAudioTaggingConfig c;
  memset(&c, 0, sizeof(c));
  ...
  const EdgevoxOnnxAudioTagging *tagger = EdgevoxOnnxCreateAudioTagging(&c);
  return AudioTagging(tagger);
}

AudioTagging::AudioTagging(const EdgevoxOnnxAudioTagging *p)
    : MoveOnly<AudioTagging, EdgevoxOnnxAudioTagging>(p) {}

void AudioTagging::Destroy(const EdgevoxOnnxAudioTagging *p) const {
  EdgevoxOnnxDestroyAudioTagging(p);
}

OfflineStream AudioTagging::CreateStream() const {
  auto s = EdgevoxOnnxAudioTaggingCreateOfflineStream(p_);
  return OfflineStream{s};
}

std::vector<AudioEvent> AudioTagging::Compute(const OfflineStream *s,
                                              int32_t top_k /*= -1*/) {
  auto events = EdgevoxOnnxAudioTaggingCompute(p_, s->Get(), top_k);
...
static void FillSourceSeparationModelConfig(
    ...
    EdgevoxOnnxOfflineSourceSeparationModelConfig *dst) {
  memset(dst, 0, sizeof(*dst));
...
OfflineSourceSeparation OfflineSourceSeparation::Create(
  ...
  struct EdgevoxOnnxOfflineSourceSeparationConfig c;
  memset(&c, 0, sizeof(c));
  FillSourceSeparationModelConfig(config.model, &c.model);

  auto p = EdgevoxOnnxCreateOfflineSourceSeparation(&c);
  return OfflineSourceSeparation(p);
}

void OfflineSourceSeparation::Destroy(
    const EdgevoxOnnxOfflineSourceSeparation *p) const {
  EdgevoxOnnxDestroyOfflineSourceSeparation(p);
}

OfflineSourceSeparation::OfflineSourceSeparation(
    const EdgevoxOnnxOfflineSourceSeparation *p)
    : MoveOnly<OfflineSourceSeparation, EdgevoxOnnxOfflineSourceSeparation>(p) {}

SourceSeparationOutput OfflineSourceSeparation::Process(
    const float *const *samples, int32_t num_channels, int32_t num_samples,
    int32_t sample_rate) const {
  auto output = EdgevoxOnnxOfflineSourceSeparationProcess(
  ...
  EdgevoxOnnxDestroySourceSeparationOutput(output);
  return ans;
}

int32_t OfflineSourceSeparation::GetOutputSampleRate() const {
  return EdgevoxOnnxOfflineSourceSeparationGetOutputSampleRate(p_);
}

int32_t OfflineSourceSeparation::GetNumberOfStems() const {
  return EdgevoxOnnxOfflineSourceSeparationGetNumberOfStems(p_);
}
...
SpokenLanguageIdentification SpokenLanguageIdentification::Create(
  ...
  struct EdgevoxOnnxSpokenLanguageIdentificationConfig c;
  memset(&c, 0, sizeof(c));
  ...
  const EdgevoxOnnxSpokenLanguageIdentification *p =
...
SpokenLanguageIdentification::SpokenLanguageIdentification(
    ...
    : MoveOnly<SpokenLanguageIdentification,
               EdgevoxOnnxSpokenLanguageIdentification>(p) {}

void SpokenLanguageIdentification::Destroy(
    const EdgevoxOnnxSpokenLanguageIdentification *p) const {
  EdgevoxOnnxDestroySpokenLanguageIdentification(p);
}

OfflineStream SpokenLanguageIdentification::CreateStream() const {
  const EdgevoxOnnxOfflineStream *s =
      EdgevoxOnnxSpokenLanguageIdentificationCreateOfflineStream(p_);
  return OfflineStream(s);
}
...
SpokenLanguageIdentificationResult SpokenLanguageIdentification::Compute(
  ...
  const EdgevoxOnnxSpokenLanguageIdentificationResult *r =
      EdgevoxOnnxSpokenLanguageIdentificationCompute(p_, s->Get());
...
SpeakerEmbeddingExtractor SpeakerEmbeddingExtractor::Create(
  ...
  struct EdgevoxOnnxSpeakerEmbeddingExtractorConfig c;
  memset(&c, 0, sizeof(c));
...
SpeakerEmbeddingExtractor::SpeakerEmbeddingExtractor(
    ...
    : MoveOnly<SpeakerEmbeddingExtractor,
               EdgevoxOnnxSpeakerEmbeddingExtractor>(p) {}

void SpeakerEmbeddingExtractor::Destroy(
    const EdgevoxOnnxSpeakerEmbeddingExtractor *p) const {
  EdgevoxOnnxDestroySpeakerEmbeddingExtractor(p);
}

int32_t SpeakerEmbeddingExtractor::Dim() const {
  return EdgevoxOnnxSpeakerEmbeddingExtractorDim(p_);
}

OnlineStream SpeakerEmbeddingExtractor::CreateStream() const {
  const EdgevoxOnnxOnlineStream *s =
      EdgevoxOnnxSpeakerEmbeddingExtractorCreateStream(p_);
  return OnlineStream(s);
}

bool SpeakerEmbeddingExtractor::IsReady(const OnlineStream *s) const {
  return EdgevoxOnnxSpeakerEmbeddingExtractorIsReady(p_, s->Get()) != 0;
}

std::vector<float> SpeakerEmbeddingExtractor::ComputeEmbedding(
    const OnlineStream *s) const {
  const float *v =
      EdgevoxOnnxSpeakerEmbeddingExtractorComputeEmbedding(p_, s->Get());
  if (!v) return {};
  int32_t dim = Dim();
...
SpeakerEmbeddingManager::SpeakerEmbeddingManager(
    const EdgevoxOnnxSpeakerEmbeddingManager *p)
    : MoveOnly<SpeakerEmbeddingManager, EdgevoxOnnxSpeakerEmbeddingManager>(p) {}

void SpeakerEmbeddingManager::Destroy(
    const EdgevoxOnnxSpeakerEmbeddingManager *p) const {
  EdgevoxOnnxDestroySpeakerEmbeddingManager(p);
}

bool SpeakerEmbeddingManager::Add(const std::string &name,
                                  const float *v) const {
  return EdgevoxOnnxSpeakerEmbeddingManagerAdd(p_, name.c_str(), v) != 0;
}

bool SpeakerEmbeddingManager::AddList(const std::string &name,
                                      const float **v) const {
  return EdgevoxOnnxSpeakerEmbeddingManagerAddList(p_, name.c_str(), v) != 0;
}

bool SpeakerEmbeddingManager::AddListFlattened(const std::string &name,
                                               const float *v,
                                               int32_t n) const {
  return EdgevoxOnnxSpeakerEmbeddingManagerAddListFlattened(p_, name.c_str(), v,
                                                           n) != 0;
}

bool SpeakerEmbeddingManager::Remove(const std::string &name) const {
  return EdgevoxOnnxSpeakerEmbeddingManagerRemove(p_, name.c_str()) != 0;
}

std::string SpeakerEmbeddingManager::Search(const float *v,
                                            float threshold) const {
  const char *name =
      EdgevoxOnnxSpeakerEmbeddingManagerSearch(p_, v, threshold);
  if (!name) return {};
  std::string ans(name);
...
std::vector<SpeakerMatch> SpeakerEmbeddingManager::GetBestMatches(
  ...
  const EdgevoxOnnxSpeakerEmbeddingManagerBestMatchesResult *r =
      EdgevoxOnnxSpeakerEmbeddingManagerGetBestMatches(p_, v, threshold, n);
  ...
  if (r) {
    ...
    EdgevoxOnnxSpeakerEmbeddingManagerFreeBestMatches(r);
  }
  return ans;
}
...
bool SpeakerEmbeddingManager::Verify(const std::string &name, const float *v,
                                     float threshold) const {
  return EdgevoxOnnxSpeakerEmbeddingManagerVerify(p_, name.c_str(), v,
                                                 threshold) != 0;
}

bool SpeakerEmbeddingManager::Contains(const std::string &name) const {
  return EdgevoxOnnxSpeakerEmbeddingManagerContains(p_, name.c_str()) != 0;
}

int32_t SpeakerEmbeddingManager::NumSpeakers() const {
  return EdgevoxOnnxSpeakerEmbeddingManagerNumSpeakers(p_);
}

std::vector<std::string> SpeakerEmbeddingManager::GetAllSpeakers() const {
  const char *const *names =
      EdgevoxOnnxSpeakerEmbeddingManagerGetAllSpeakers(p_);
...
OfflineSpeakerDiarization OfflineSpeakerDiarization::Create(
  ...
  struct EdgevoxOnnxOfflineSpeakerDiarizationConfig c;
  memset(&c, 0, sizeof(c));
  ...
  const EdgevoxOnnxOfflineSpeakerDiarization *p =
...
OfflineSpeakerDiarization::OfflineSpeakerDiarization(
    ...
    : MoveOnly<OfflineSpeakerDiarization,
               EdgevoxOnnxOfflineSpeakerDiarization>(p) {}

void OfflineSpeakerDiarization::Destroy(
    const EdgevoxOnnxOfflineSpeakerDiarization *p) const {
  EdgevoxOnnxDestroyOfflineSpeakerDiarization(p);
}

int32_t OfflineSpeakerDiarization::GetSampleRate() const {
  return EdgevoxOnnxOfflineSpeakerDiarizationGetSampleRate(p_);
}
...
void OfflineSpeakerDiarization::SetConfig(
  ...
  c.clustering.num_clusters = config.clustering.num_clusters;
  c.clustering.threshold = config.clustering.threshold;
  EdgevoxOnnxOfflineSpeakerDiarizationSetConfig(p_, &c);
}
...
OfflineSpeakerDiarization::Process(const float *samples, int32_t n) const {
  const EdgevoxOnnxOfflineSpeakerDiarizationResult *r =
      EdgevoxOnnxOfflineSpeakerDiarizationProcess(p_, samples, n);
...
OfflineDiacritization OfflineDiacritization::Create(
  ...
  struct EdgevoxOnnxOfflineDiacritizationConfig c;
  memset(&c, 0, sizeof(c));
  ...
  const EdgevoxOnnxOfflineDiacritization *diacrt =
      EdgevoxOnnxCreateOfflineDiacritization(&c);
  return OfflineDiacritization(diacrt);
}

OfflineDiacritization::OfflineDiacritization(const EdgevoxOnnxOfflineDiacritization *diacrt)
    : MoveOnly<OfflineDiacritization, EdgevoxOnnxOfflineDiacritization>(diacrt) {}

void OfflineDiacritization::Destroy(const EdgevoxOnnxOfflineDiacritization *diacrt) const {
  EdgevoxOnnxDestroyOfflineDiacritization(diacrt);
}
