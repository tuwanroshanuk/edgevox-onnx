// edgevox-onnx/c-api/cxx-api.h
//
// Copyright (c)  2024  Xiaomi Corporation
/**
 * @file cxx-api.h
 * @brief Public C++ wrapper for the edgevox-onnx C API.
 *
 ...
 */
#ifndef EDGEVOX_ONNX_C_API_CXX_API_H_
#define EDGEVOX_ONNX_C_API_CXX_API_H_
 ...
 */
EDGEVOX_ONNX_API Wave ReadWave(const std::string &filename);
 ...
 */
EDGEVOX_ONNX_API bool WriteWave(const std::string &filename, const Wave &wave);
...
class EDGEVOX_ONNX_API MoveOnly {
 public:
...
class EDGEVOX_ONNX_API OnlineStream
    : public MoveOnly<OnlineStream, EdgevoxOnnxOnlineStream> {
 public:
  /** @brief Wrap an existing C online stream handle. */
  explicit OnlineStream(const EdgevoxOnnxOnlineStream *p);
  ...
  void Destroy(const EdgevoxOnnxOnlineStream *p) const;
};
 ...
 * @endcode
 */
class EDGEVOX_ONNX_API OnlineRecognizer
    : public MoveOnly<OnlineRecognizer, EdgevoxOnnxOnlineRecognizer> {
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOnlineRecognizer *p) const;
 ...
 private:
  explicit OnlineRecognizer(const EdgevoxOnnxOnlineRecognizer *p);
};
...
class EDGEVOX_ONNX_API OfflineStream
    : public MoveOnly<OfflineStream, EdgevoxOnnxOfflineStream> {
 public:
  /** @brief Wrap an existing C offline stream handle. */
  explicit OfflineStream(const EdgevoxOnnxOfflineStream *p);
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflineStream *p) const;
};
 ...
 */
class EDGEVOX_ONNX_API OfflineRecognizer
    : public MoveOnly<OfflineRecognizer, EdgevoxOnnxOfflineRecognizer> {
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflineRecognizer *p) const;
 ...
 private:
  explicit OfflineRecognizer(const EdgevoxOnnxOfflineRecognizer *p);
};
...
class EDGEVOX_ONNX_API OfflineTts
    : public MoveOnly<OfflineTts, EdgevoxOnnxOfflineTts> {
 public:
  /** @brief Create an offline TTS engine. */
  static OfflineTts Create(const OfflineTtsConfig &config);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflineTts *p) const;
 ...
 private:
  explicit OfflineTts(const EdgevoxOnnxOfflineTts *p);
};
...
class EDGEVOX_ONNX_API KeywordSpotter
    : public MoveOnly<KeywordSpotter, EdgevoxOnnxKeywordSpotter> {
 public:
  /** @brief Create a keyword spotter from a config struct. */
  static KeywordSpotter Create(const KeywordSpotterConfig &config);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxKeywordSpotter *p) const;
 ...
 private:
  explicit KeywordSpotter(const EdgevoxOnnxKeywordSpotter *p);
};
...
class EDGEVOX_ONNX_API OfflineSpeechDenoiser
    : public MoveOnly<OfflineSpeechDenoiser, EdgevoxOnnxOfflineSpeechDenoiser> {
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflineSpeechDenoiser *p) const;
 ...
 private:
  explicit OfflineSpeechDenoiser(const EdgevoxOnnxOfflineSpeechDenoiser *p);
};
...
class EDGEVOX_ONNX_API OnlineSpeechDenoiser
    : public MoveOnly<OnlineSpeechDenoiser, EdgevoxOnnxOnlineSpeechDenoiser> {
 public:
  /** @brief Create an online speech denoiser. */
  static OnlineSpeechDenoiser Create(const OnlineSpeechDenoiserConfig &config);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOnlineSpeechDenoiser *p) const;
 ...
 private:
  explicit OnlineSpeechDenoiser(const EdgevoxOnnxOnlineSpeechDenoiser *p);
};
...
class EDGEVOX_ONNX_API CircularBuffer
    : public MoveOnly<CircularBuffer, EdgevoxOnnxCircularBuffer> {
 public:
  /** @brief Create a circular buffer with the given capacity in samples. */
  static CircularBuffer Create(int32_t capacity);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxCircularBuffer *p) const;
 ...
 private:
  explicit CircularBuffer(const EdgevoxOnnxCircularBuffer *p);
};
 ...
 */
class EDGEVOX_ONNX_API VoiceActivityDetector
    : public MoveOnly<VoiceActivityDetector, EdgevoxOnnxVoiceActivityDetector> {
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxVoiceActivityDetector *p) const;
 ...
 private:
  explicit VoiceActivityDetector(const EdgevoxOnnxVoiceActivityDetector *p);
};

/** @brief RAII wrapper for linear resampling. */
class EDGEVOX_ONNX_API LinearResampler
    : public MoveOnly<LinearResampler, EdgevoxOnnxLinearResampler> {
 public:
  /** @brief Construct an empty wrapper. */
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxLinearResampler *p) const;
 ...
 private:
  explicit LinearResampler(const EdgevoxOnnxLinearResampler *p);
};

/** @brief Return the edgevox-onnx version string as a C++ string. */
EDGEVOX_ONNX_API std::string GetVersionStr();
/** @brief Return the build Git SHA1 as a C++ string. */
EDGEVOX_ONNX_API std::string GetGitSha1();
/** @brief Return the build Git date as a C++ string. */
EDGEVOX_ONNX_API std::string GetGitDate();
/** @brief Return `true` if a file exists. */
EDGEVOX_ONNX_API bool FileExists(const std::string &filename);
...
class EDGEVOX_ONNX_API OfflinePunctuation
    : public MoveOnly<OfflinePunctuation, EdgevoxOnnxOfflinePunctuation> {
 public:
  /** @brief Create an offline punctuation model. */
  static OfflinePunctuation Create(const OfflinePunctuationConfig &config);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflinePunctuation *p) const;

  /** @brief Add punctuation to a complete input text. */
  std::string AddPunctuation(const std::string &text) const;

 private:
  explicit OfflinePunctuation(const EdgevoxOnnxOfflinePunctuation *p);
};
...
class EDGEVOX_ONNX_API OnlinePunctuation
    : public MoveOnly<OnlinePunctuation, EdgevoxOnnxOnlinePunctuation> {
 public:
  /** @brief Create an online punctuation model. */
  static OnlinePunctuation Create(const OnlinePunctuationConfig &config);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOnlinePunctuation *p) const;

  /** @brief Add punctuation to one input text chunk. */
  std::string AddPunctuation(const std::string &text) const;

 private:
  explicit OnlinePunctuation(const EdgevoxOnnxOnlinePunctuation *p);
};
...
class EDGEVOX_ONNX_API AudioTagging
    : public MoveOnly<AudioTagging, EdgevoxOnnxAudioTagging> {
 public:
  /** @brief Create an audio tagger. */
  static AudioTagging Create(const AudioTaggingConfig &config);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxAudioTagging *p) const;
 ...
 private:
  explicit AudioTagging(const EdgevoxOnnxAudioTagging *p);
};
...
class EDGEVOX_ONNX_API OfflineSourceSeparation
 ...
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflineSourceSeparation *p) const;
 ...
 private:
  explicit OfflineSourceSeparation(const EdgevoxOnnxOfflineSourceSeparation *p);
};
...
class EDGEVOX_ONNX_API SpokenLanguageIdentification
 ...
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxSpokenLanguageIdentification *p) const;
...
class EDGEVOX_ONNX_API SpeakerEmbeddingExtractor
 ...
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxSpeakerEmbeddingExtractor *p) const;
...
class EDGEVOX_ONNX_API SpeakerEmbeddingManager
 ...
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxSpeakerEmbeddingManager *p) const;
 ...
 private:
  explicit SpeakerEmbeddingManager(const EdgevoxOnnxSpeakerEmbeddingManager *p);
};
...
class EDGEVOX_ONNX_API OfflineSpeakerDiarization
 ...
 public:
  ...
  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflineSpeakerDiarization *p) const;
...
class EDGEVOX_ONNX_API OfflineDiacritization
    : public MoveOnly<OfflineDiacritization, EdgevoxOnnxOfflineDiacritization> {
 public:
  /** @brief Create an offline diacritization model. */
  static OfflineDiacritization Create(const OfflineDiacritizationConfig &config);

  /** @brief Destroy the wrapped C handle. */
  void Destroy(const EdgevoxOnnxOfflineDiacritization *diacrt) const;

  /** @brief Add diacritics to a complete input text. */
  std::string AddDiacritics(const std::string &text) const;

 private:
  explicit OfflineDiacritization(const EdgevoxOnnxOfflineDiacritization *diacrt);
};

}  // namespace edgevox_onnx::cxx

#endif  // EDGEVOX_ONNX_C_API_CXX_API_H_
