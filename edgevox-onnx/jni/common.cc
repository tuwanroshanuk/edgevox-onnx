...
void PrependAdspLibraryPath(const std::string &new_path) {
  EDGEVOX_ONNX_LOGE("This function is not for Windows. Ignore it");
}
#else
void PrependAdspLibraryPath(const std::string &new_path) {
  ...
  if (setenv("ADSP_LIBRARY_PATH", updated_path.c_str(), 1) != 0) {
    EDGEVOX_ONNX_LOGE("Failed to set ADSP_LIBRARY_PATH to '%s'",
                     updated_path.c_str());
  } else {
    EDGEVOX_ONNX_LOGE("Successfully set ADSP_LIBRARY_PATH to '%s'",
                     updated_path.c_str());
  }
  /*
