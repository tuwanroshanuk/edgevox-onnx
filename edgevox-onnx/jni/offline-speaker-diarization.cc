// edgevox-onnx/jni/offline-speaker-diarization.cc
//
...
static OfflineSpeakerDiarizationConfig GetOfflineSpeakerDiarizationConfig(
  ...
  EDGEVOX_ONNX_JNI_READ_STRING(ans.segmentation.pyannote.model, model,
                              pyannote_config_cls, pyannote_config);

  EDGEVOX_ONNX_JNI_READ_INT(ans.segmentation.num_threads, numThreads,
                           segmentation_config_cls, segmentation_config);

  EDGEVOX_ONNX_JNI_READ_BOOL(ans.segmentation.debug, debug,
                            segmentation_config_cls, segmentation_config);

  EDGEVOX_ONNX_JNI_READ_STRING(ans.segmentation.provider, provider,
                              segmentation_config_cls, segmentation_config);
  ...
  EDGEVOX_ONNX_JNI_READ_STRING(ans.embedding.model, model, embedding_config_cls,
                              embedding_config);

  EDGEVOX_ONNX_JNI_READ_INT(ans.embedding.num_threads, numThreads,
                           embedding_config_cls, embedding_config);

  EDGEVOX_ONNX_JNI_READ_BOOL(ans.embedding.debug, debug, embedding_config_cls,
                            embedding_config);

  EDGEVOX_ONNX_JNI_READ_STRING(ans.embedding.provider, provider,
                              embedding_config_cls, embedding_config);
  ...
  EDGEVOX_ONNX_JNI_READ_INT(ans.clustering.num_clusters, numClusters,
                           clustering_config_cls, clustering_config);

  EDGEVOX_ONNX_JNI_READ_FLOAT(ans.clustering.threshold, threshold,
                             clustering_config_cls, clustering_config);

  EDGEVOX_ONNX_JNI_READ_FLOAT(ans.min_duration_on, minDurationOn, cls, config);

  EDGEVOX_ONNX_JNI_READ_FLOAT(ans.min_duration_off, minDurationOff, cls, config);
...
Java_com_k2fsa_edgevox_onnx_OfflineSpeakerDiarization_newFromAsset(
  ...
  bool ok = false;
  auto config =
      edgevox_onnx::GetOfflineSpeakerDiarizationConfig(env, _config, &ok);
  ...
  auto sd = new edgevox_onnx::OfflineSpeakerDiarization(
#if __ANDROID_API__ >= 9
      mgr,
#endif
      config);
...
Java_com_k2fsa_edgevox_onnx_OfflineSpeakerDiarization_newFromFile(
  ...
  auto sd = new edgevox_onnx::OfflineSpeakerDiarization(config);
...
Java_com_k2fsa_edgevox_onnx_OfflineSpeakerDiarization_processWithCallback(
  ...
  std::function<int32_t(int32_t, int32_t, void *)> callback_wrapper =
    ...
    if (mid == nullptr) {
      EDGEVOX_ONNX_LOGE("Failed to get the callback. Ignore it.");
      env->DeleteLocalRef(cls);
      return 0;
    }
    env->DeleteLocalRef(cls);
