// edgevox-onnx/jni/speech-denoiser.cc
//
...
OfflineSpeechDenoiserModelConfig GetOfflineSpeechDenoiserModelConfig(
  ...
  fid = env->GetFieldID(
      ...
      "Lcom/nexus/edgevox/onnx/OfflineSpeechDenoiserGtcrnModelConfig;");
  jobject gtcrn = env->GetObjectField(model, fid);
  jclass gtcrn_cls = env->GetObjectClass(gtcrn);

  EDGEVOX_ONNX_JNI_READ_STRING(ans.gtcrn.model, model, gtcrn_cls, gtcrn);

  fid = env->GetFieldID(
      model_config_cls, "dpdfnet",
      "Lcom/nexus/edgevox/onnx/OfflineSpeechDenoiserDpdfNetModelConfig;");
  jobject dpdfnet = env->GetObjectField(model, fid);
  jclass dpdfnet_cls = env->GetObjectClass(dpdfnet);

  EDGEVOX_ONNX_JNI_READ_STRING(ans.dpdfnet.model, model, dpdfnet_cls, dpdfnet);

  EDGEVOX_ONNX_JNI_READ_INT(ans.num_threads, numThreads, model_config_cls,
                           model);
  EDGEVOX_ONNX_JNI_READ_BOOL(ans.debug, debug, model_config_cls, model);
  EDGEVOX_ONNX_JNI_READ_STRING(ans.provider, provider, model_config_cls, model);
...
OfflineSpeechDenoiserConfig GetOfflineSpeechDenoiserConfig(JNIEnv *env,
  ...
  jfieldID fid = env->GetFieldID(
      cls, "model", "Lcom/nexus/edgevox/onnx/OfflineSpeechDenoiserModelConfig;");
...
jobject NewDenoisedAudio(JNIEnv *env, const DenoisedAudio &denoised) {
  jclass cls = env->FindClass("com/nexus/edgevox/onnx/DenoisedAudio");
  if (cls == nullptr) {
    EDGEVOX_ONNX_LOGE("Failed to get class for DenoisedAudio");
    return nullptr;
  }

  jmethodID constructor = env->GetMethodID(cls, "<init>", "([FI)V");
  if (constructor == nullptr) {
    EDGEVOX_ONNX_LOGE("Failed to get constructor for DenoisedAudio");
