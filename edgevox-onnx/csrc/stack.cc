// edgevox-onnx/csrc/stack.cc
//
// Copyright (c) 2023 Jingzhao Ou (jingzhao.ou@gmail.com)

#include "edgevox-onnx/csrc/stack.h"
...
static void PrintShape(const std::vector<int64_t> &a) {
  for (auto i : a) {
    EDGEVOX_ONNX_LOGE("%d ", static_cast<int32_t>(i));
  }
  EDGEVOX_ONNX_LOGE("\n");
}
...
Ort::Value Stack(OrtAllocator *allocator,
  ...
  for (int32_t i = 1; i != static_cast<int32_t>(values.size()); ++i) {
    ...
    if (!ret) {
      EDGEVOX_ONNX_LOGE("Incorrect shape in Stack !\n");

      EDGEVOX_ONNX_LOGE("Shape for tensor 0: ");
      PrintShape(v0_shape);

      EDGEVOX_ONNX_LOGE("Shape for tensor %d: ", i);
      PrintShape(s);
