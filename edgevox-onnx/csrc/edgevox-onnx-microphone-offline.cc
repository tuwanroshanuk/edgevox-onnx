// edgevox-onnx/csrc/edgevox-onnx-microphone-offline.cc
//
...
for a list of pre-trained models to download.
  ...
  EDGEVOX_ONNX_LOGE("Creating recognizer ...");
  edgevox_onnx::OfflineRecognizer recognizer(config);
  EDGEVOX_ONNX_LOGE("Recognizer created!");
  ...
  if (device_index == paNoDevice) {
    ...
    fprintf(stderr, " ./bin/edgevox-onnx-alsa-offline \n");
    EDGEVOX_ONNX_EXIT(EXIT_FAILURE);
  }
  ...
  while (!stop) {
    switch (state) {
      ...
      case State::kDecoding: {
        ...
        EDGEVOX_ONNX_LOGE("Decoding Done! Result is:");
        EDGEVOX_ONNX_LOGE("%s", s->GetResult().text.c_str());
