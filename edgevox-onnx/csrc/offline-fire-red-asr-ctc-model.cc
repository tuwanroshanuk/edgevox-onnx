// edgevox-onnx/csrc/offline-fire-red-asr-ctc-model.cc
//
 ...
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.fire_red_asr_ctc.model),
        sess_opts_);
    Init(nullptr, 0);
  }
  ...
  void NormalizeFeatures(float *features, int32_t num_frames,
    ...
    if (static_cast<int32_t>(mean_.size()) != feat_dim) {
      EDGEVOX_ONNX_LOGE("Bad things happened");
      EDGEVOX_ONNX_LOGE("Wrong feat dim %d. Expect: %d", feat_dim,
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if (model_type != "fire-red-asr-2-ctc") {
      EDGEVOX_ONNX_LOGE("Expect model type fire-red-asr-2-ctc. Given: '%s'",
                       model_type.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(subsampling_factor_,
    ...
    if (config_.debug) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("subsampling_factor: %{public}d", subsampling_factor_);
      EDGEVOX_ONNX_LOGE("vocab_size: %{public}d", vocab_size_);
#else
    ...
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(mean_, "cmvn_mean");
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(inv_stddev_, "cmvn_inv_stddev");
    if (mean_.size() != inv_stddev_.size()) {
      EDGEVOX_ONNX_LOGE("Incorrect cmvn. mean size: %d, inv_stddev size: %d",
                       static_cast<int32_t>(mean_.size()),
