// edgevox-onnx/csrc/offline-whisper-model.cc
//
// Copyright (c)  2022-2023  Xiaomi Corporation

#include "edgevox-onnx/csrc/offline-whisper-model.h"
...
class OfflineWhisperModel::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.whisper.encoder), sess_opts_);
    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.whisper.decoder), sess_opts_);
  ...
  int32_t DetectLanguage(Ort::Value &cross_k,    // NOLINT
    ...
    *(offset.GetTensorMutableData<int64_t>()) = 0;
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("Detected language: %s",
 ...
 private:
  void InitEncoder(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA(n_mels_, "n_mels");
    EDGEVOX_ONNX_READ_META_DATA(n_text_layer_, "n_text_layer");
    EDGEVOX_ONNX_READ_META_DATA(n_text_ctx_, "n_text_ctx");
    EDGEVOX_ONNX_READ_META_DATA(n_text_state_, "n_text_state");
    EDGEVOX_ONNX_READ_META_DATA(n_vocab_, "n_vocab");
    EDGEVOX_ONNX_READ_META_DATA(sot_, "sot");
    EDGEVOX_ONNX_READ_META_DATA(eot_, "eot");
    EDGEVOX_ONNX_READ_META_DATA(blank_, "blank_id");
    EDGEVOX_ONNX_READ_META_DATA(translate_, "translate");
    EDGEVOX_ONNX_READ_META_DATA(transcribe_, "transcribe");
    EDGEVOX_ONNX_READ_META_DATA(is_multilingual_, "is_multilingual");
    EDGEVOX_ONNX_READ_META_DATA(no_timestamps_, "no_timestamps");
    ...
    EDGEVOX_ONNX_READ_META_DATA(no_speech_, "no_speech");
    EDGEVOX_ONNX_READ_META_DATA_VEC(sot_sequence_, "sot_sequence");

    if (is_multilingual_) {
      EDGEVOX_ONNX_READ_META_DATA_VEC(all_language_tokens_,
                                     "all_language_tokens");
      EDGEVOX_ONNX_READ_META_DATA_VEC_STRING(all_language_codes_,
                                            "all_language_codes");
      if (all_language_tokens_.size() != all_language_codes_.size()) {
        EDGEVOX_ONNX_LOGE("# lang_id: %d != # lang_code: %d",
  ...
  void InitDecoder(void *model_data, size_t model_data_length) {
    ...
    if (has_attention_output_) {
      ...
      Ort::ModelMetadata meta_data = encoder_sess_->GetModelMetadata();
      EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(n_alignment_heads_,
                                              "n_alignment_heads", 0);

      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Decoder has attention output with %d alignment heads",
                         n_alignment_heads_);
      }
    }
  }
