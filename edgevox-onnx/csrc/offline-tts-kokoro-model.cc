// edgevox-onnx/csrc/offline-tts-kokoro-model.cc
//
 ...
 public:
  explicit Impl(const OfflineTtsModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.kokoro.model), sess_opts_);
  ...
  Ort::Value Run(Ort::Value x, int32_t sid, float speed) {
    ...
    if (len >= dim0) {
      EDGEVOX_ONNX_LOGE("Bad things happened! %d vs %d", len, dim0);
      EDGEVOX_ONNX_EXIT(-1);
    }
 ...
 private:
  void Init(void *model_data, size_t model_data_length, const char *voices_data,
    ...
    if (style_dim_.size() != 3) {
      EDGEVOX_ONNX_LOGE("style_dim should be 3-d, given: %d",
                       static_cast<int32_t>(style_dim_.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (style_dim_[1] != 1) {
      EDGEVOX_ONNX_LOGE("style_dim[1] should be 1, given: %d", style_dim_[1]);
      EDGEVOX_ONNX_EXIT(-1);
    }
