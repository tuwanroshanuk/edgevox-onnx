// edgevox-onnx/csrc/kokoro-multi-lang-lexicon.cc
//
  ...
  std::vector<TokenIDs> ConvertTextToTokenIds(const std::string &_text,
    ...
    for (std::wsregex_iterator i = begin; i != end; ++i) {
      ...
      if (std::regex_match(match_str, we_zh)) {
        if (debug_) {
          EDGEVOX_ONNX_LOGE("Chinese: %s", ms.c_str());
        }
        ids_vec = ConvertChineseToTokenIDs(ms);
      } else {
        if (debug_) {
          EDGEVOX_ONNX_LOGE("Non-Chinese: %s", ms.c_str());
        }
 ...
 private:
    ...
    if (word2ids_.count(w)) {
      ...
      for (const auto &word : words) {
        if (word2ids_.count(word)) {
          ...
          if (debug_) {
            EDGEVOX_ONNX_LOGE("Skip OOV: '%s'", word.c_str());
          }
        }
      }
    }
  ...
  std::vector<std::vector<int32_t>> ConvertNonChineseToTokenIDs(
    ...
    for (const auto &_word : words) {
      ...
      if (IsPunctuation(word)) {
        ...
        if (debug_) {
          EDGEVOX_ONNX_LOGE("Use espeak-ng to handle the OOV: '%s'",
                           word.c_str());
        }
        ...
        for (const auto &v : phonemes) {
          for (const auto p : v) {
            ...
            if (token2id_.count(token)) {
              ...
              if (debug_) {
                EDGEVOX_ONNX_LOGE("Skip OOV token '%s' from '%s'", token.c_str(),
                                 word.c_str());
              }
            }
          }
  ...
  void InitTokens(std::istream &is) {
    ...
    for (const auto &p : token2id_) {
      ...
      if (s.size() != 1) {
        EDGEVOX_ONNX_LOGE("Error for token %s with id %d", p.first.c_str(),
                         p.second);
        EDGEVOX_ONNX_EXIT(-1);
      }
