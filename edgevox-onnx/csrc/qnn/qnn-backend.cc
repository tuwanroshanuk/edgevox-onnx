// edgevox-onnx/csrc/qnn/qnn-backend.cc
//
...
class QnnBackend::Impl {
 public:
  explicit Impl(const std::string &backend_lib, bool debug) : debug_(debug) {
    ...
    if (!ok) {
      EDGEVOX_ONNX_LOGE("Failed to init qnn interface from '%s'",
                       backend_lib.c_str());
      return;
    }
  ...
  ~Impl() {
    if (context_handle_) {
      ...
      EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call contextFree");
    }

    if (device_handle_) {
      auto ret = qnn_interface_.deviceFree(device_handle_);
      EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call deviceFree");
    }

    if (backend_handle_) {
      auto ret = qnn_interface_.backendFree(backend_handle_);
      EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call backendFree");
    }

    if (log_handle_) {
      auto ret = qnn_interface_.logFree(log_handle_);
      EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call logFree");
    }
  }

  void InitContext() {
    if (context_handle_) {
      EDGEVOX_ONNX_LOGE("context handle is already initialized");
      return;
    }

    auto ret = qnn_interface_.contextCreate(backend_handle_, device_handle_,
                                            context_config_, &context_handle_);
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call contextCreate");
  }
 ...
 private:
  bool InitQnnInterface(const std::string &backend_lib) {
    ...
    if (debug_) {
      EDGEVOX_ONNX_LOGE("loaded %s", backend_lib.c_str());
    }
    ...
    if (debug_) {
      EDGEVOX_ONNX_LOGE("Got %s", symbol);
    }
    ...
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call get_interface_providers");

    if (!interface_providers) {
      EDGEVOX_ONNX_LOGE("interface_providers is nullptr");
      return false;
    }

    if (num_providers == 0) {
      EDGEVOX_ONNX_LOGE("Number of providers is 0");
      return false;
    }
    ...
    if (debug_) {
      EDGEVOX_ONNX_LOGE("QNN_API_VERSION_MAJOR: %d", QNN_API_VERSION_MAJOR);
      EDGEVOX_ONNX_LOGE("QNN_API_VERSION_MINOR: %d", QNN_API_VERSION_MINOR);
      EDGEVOX_ONNX_LOGE("QNN_API_VERSION_PATCH: %d", QNN_API_VERSION_PATCH);
    }
    ...
    if (!found_valid_interface) {
      EDGEVOX_ONNX_LOGE("Failed to find valid interface");
      return false;
    }
    ...
    if (debug_) {
      ...
      EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call backendGetBuildId()");

      EDGEVOX_ONNX_LOGE("backend build ID: %s", build_id);
    }
  ...
  void InitLog() {
    ...
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call logCreate");
  }

  void InitBackend() {
    auto ret = qnn_interface_.backendCreate(log_handle_, backend_config_,
                                            &backend_handle_);
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call backendCreate");
  }

  void InitDevice() {
    auto ret =
        qnn_interface_.deviceCreate(log_handle_, nullptr, &device_handle_);
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call deviceCreate");
  }
