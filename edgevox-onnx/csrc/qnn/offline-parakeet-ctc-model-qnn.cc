// edgevox-onnx/csrc/qnn/offline-parakeet-ctc-model-qnn.cc
//
// Copyright (c)  2026  Xiaomi Corporation

#include "edgevox-onnx/csrc/qnn/offline-parakeet-ctc-model-qnn.h"
 ...
 public:
  explicit Impl(const OfflineModelConfig &config) : config_(config) {
    ...
    if (context_binary.empty()) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Init from context binary '%s'",
                         context_binary.c_str());
      }
 ...
 private:
  ...
  void CheckModel() {
    ...
    if (!model_->HasTensor("log_probs")) {
      EDGEVOX_ONNX_LOGE("Model does not have output node 'log_probs'");
      EDGEVOX_ONNX_EXIT(-1);
    }

    auto out_shape = model_->TensorShape("log_probs");
    if (out_shape.size() != 3) {
      EDGEVOX_ONNX_LOGE("The output log_probs should be 3-d, actual '%d'",
                       static_cast<int32_t>(out_shape.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (out_shape[1] <= 0 || out_shape[2] <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid log_probs shape: [%d, %d, %d]", out_shape[0],
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("max_num_frames: %d", max_num_frames_);
