// edgevox-onnx/csrc/qnn/offline-moonshine-model-qnn.cc
//
// Copyright (c)  2026  Xiaomi Corporation

#include "edgevox-onnx/csrc/qnn/offline-moonshine-model-qnn.h"
 ...
 private:
  void InitEncoder() {
    ...
    if (!encoder_binary.empty() && FileExists(encoder_binary)) {
      ...
      if (!encoder_binary.empty() && !FileExists(encoder_binary)) {
        if (config_.debug) {
          EDGEVOX_ONNX_LOGE("Saving context binary to '%s'", encoder_binary.c_str());
        }
  ...
  void InitDecoder() {
    ...
    if (!decoder_binary.empty() && FileExists(decoder_binary)) {
      ...
      if (!decoder_binary.empty() && !FileExists(decoder_binary)) {
        if (config_.debug) {
          EDGEVOX_ONNX_LOGE("Saving context binary to '%s'", decoder_binary.c_str());
        }
  ...
  void PostInitEncoder() {
    ...
    if (audio_shape.size() < 2) {
      EDGEVOX_ONNX_LOGE("Invalid encoder audio tensor rank");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (cross_k_shape.size() < 3) {
      EDGEVOX_ONNX_LOGE("Invalid encoder cross_k_0 tensor rank");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (output_names.empty() || output_names.size() % 2 != 0) {
      EDGEVOX_ONNX_LOGE("Invalid encoder cross-KV output count");
      EDGEVOX_ONNX_EXIT(-1);
    }
    num_layers_ = output_names.size() / 2;

    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("max_audio_len_: %d", max_audio_len_);
      EDGEVOX_ONNX_LOGE("enc_seq_len_: %d", enc_seq_len_);
      EDGEVOX_ONNX_LOGE("hidden_size_: %d", hidden_size_);
      EDGEVOX_ONNX_LOGE("num_layers_: %d", num_layers_);
    }
  }
  ...
  void PostInitDecoder() {
    ...
    if (actual_inputs != expected_inputs) {
      EDGEVOX_ONNX_LOGE("Expected %d decoder inputs, got %d",
    ...
    if (mask_shape.size() < 1) {
      EDGEVOX_ONNX_LOGE("Invalid decoder mask tensor rank");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (logits_shape.size() < 3) {
      EDGEVOX_ONNX_LOGE("Invalid decoder logits tensor rank");
      EDGEVOX_ONNX_EXIT(-1);
    }
