// edgevox-onnx/csrc/qnn/offline-parakeet-tdt-model-qnn.cc
//
// Copyright (c)  2026  Xiaomi Corporation

#include "edgevox-onnx/csrc/qnn/offline-parakeet-tdt-model-qnn.h"
 ...
 private:
  ...
  void CheckEncoder() {
    ...
    if (!encoder_->HasTensor(encoder_input_name_)) {
      EDGEVOX_ONNX_LOGE("The encoder model does not have input '%s'",
    ...
    if (x_shape.size() != 3 || x_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("The encoder input x should be of shape [1, ?, ?]");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("encoder input '%s': [%d, %d, %d]",
                       encoder_input_name_.c_str(), 1, max_num_frames_,
                       feat_dim_);
      EDGEVOX_ONNX_LOGE("encoder output '%s': [%d, %d, %d]",
  ...
  void CheckDecoder() {
    ...
    if (!decoder_->HasTensor(decoder_input_y_name_)) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have input '%s'",
    ...
    if (!decoder_->HasTensor(decoder_output_name_)) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have output '%s'",
  ...
  void CheckJoiner() {
    ...
    if (total_output <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid joiner output shape. total_output: %d",
                       total_output);
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("joiner input '%s': [%d, %d, %d]",
                       joiner_encoder_input_name_.c_str(), 1, encoder_out_dim_,
                       1);
      EDGEVOX_ONNX_LOGE("joiner input '%s': [%d, %d, %d]",
                       joiner_decoder_input_name_.c_str(), 1, decoder_out_dim_,
                       1);
      EDGEVOX_ONNX_LOGE("joiner output '%s': [%d, %d, %d, %d]",
