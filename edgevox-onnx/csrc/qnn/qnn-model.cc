// edgevox-onnx/csrc/qnn/qnn-model.h
//
...
class QnnModel::Impl {
 public:
  Impl(const std::string &model_so, const QnnBackend *backend, bool debug)
    ...
    if (!ok) {
      EDGEVOX_ONNX_LOGE("Failed to load '%s'", model_so.c_str());
      return;
    }

    ok = InitSymbols();
    if (!ok) {
      EDGEVOX_ONNX_LOGE("Failed to get model symbols from '%s'",
                       model_so.c_str());
      return;
    }
  ...
  bool LoadSystemLib(const std::string &binary_context_file,
    ...
    if (!system_lib_handle_) {
      EDGEVOX_ONNX_LOGE("Failed to dlopen '%s'. Error is: '%s'",
                       system_lib.c_str(), dlerror());
      return false;
    }
    if (debug_) {
      EDGEVOX_ONNX_LOGE("loaded %s", system_lib.c_str());
    }
    ...
    if (!get_system_interface_providers) {
      EDGEVOX_ONNX_LOGE("Failed to get QnnSystemInterface_getProviders");
      return false;
    }
    ...
    if (get_system_interface_providers(&system_interface_providers,
      ...
      EDGEVOX_ONNX_LOGE("Failed to get system interface providers.");
      return false;
    }
    ...
    for (uint32_t i = 0; i < num_providers; ++i) {
      if (debug_) {
        EDGEVOX_ONNX_LOGE("QNN_SYSTEM_API_VERSION_MAJOR: %d",
                         static_cast<int32_t>(QNN_SYSTEM_API_VERSION_MAJOR));
        EDGEVOX_ONNX_LOGE("QNN_SYSTEM_API_VERSION_MINOR: %d",
    ...
    if (qnn_system_interface_.systemContextCreate(&sys_ctx_handle) !=
        QNN_SUCCESS) {
      EDGEVOX_ONNX_LOGE("Could not create system handle.");
      return false;
    }
    ...
    if (!CopyMetadataToGraphsInfo(binary_info, graphs_info, graphs_count)) {
      EDGEVOX_ONNX_LOGE("Failed to call CopyMetadataToGraphsInfo");
    ...
    if (graphs_count > 1) {
      EDGEVOX_ONNX_LOGE("Only the first graph is used");
    }
    ...
    if (backend_->QnnInterface().contextCreateFromBinary(
      ...
      free_graphs_info();
      EDGEVOX_ONNX_LOGE("Could not create context from binary.");
      return false;
    }
    ...
    if (backend_->QnnInterface().graphRetrieve(
      ...
      free_graphs_info();
      EDGEVOX_ONNX_LOGE("Unable to retrieve graph handle for graph %d", 0);
      return false;
    }
  ...
  bool SaveBinaryContext(const std::string &filename) {
    ...
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call contextGetBinarySize");

    if (debug_) {
      EDGEVOX_ONNX_LOGE("context binary size: %.3f MB",
    ...
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call contextGetBinary");
    ...
    if (!ofs) {
      EDGEVOX_ONNX_LOGE("Failed to create '%s'", filename.c_str());
      return false;
    }
    ...
    if (!ofs) {
      EDGEVOX_ONNX_LOGE("Failed to write '%s'", filename.c_str());
      return false;
    }
  ...
  std::vector<int32_t> TensorShape(const std::string &name) const {
    ...
    if (!HasTensor(name)) {
      EDGEVOX_ONNX_LOGE("No such tensor '%s'", name.c_str());
      return shape;
    }
  ...
  bool SetInputTensorData(const std::string &name, const float *p, int32_t n) {
    ...
    if (t->v1.dataType == QNN_DATATYPE_FLOAT_32) {
      if (n * sizeof(float) != t->v1.clientBuf.dataSize) {
        EDGEVOX_ONNX_LOGE("tensor '%s' expects %d bytes, but you provide %d bytes",
                         name.c_str(),
  ...
  bool Run() {
    ...
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to run graphExecute");
 ...
 private:
  ...
  bool InitModel(const std::string &model_so) {
    ...
    if (debug_) {
      EDGEVOX_ONNX_LOGE("loaded %s", model_so.c_str());
    }
  ...
  bool InitSymbols() {
    ...
    if (!compose_graphs_fn_handle_) {
      EDGEVOX_ONNX_LOGE("Failed to dlsym for '%s'. Error is: '%s'", symbol,
                       dlerror());
      return false;
    }
  ...
  void InitGraph() {
    ...
    EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call compose_graphs_fn_handle_");

    if (debug_) {
      EDGEVOX_ONNX_LOGE("graphs_count: %d", (int32_t)graphs_count);
    }
    ...
    for (uint32_t i = 0; i < graphs_count; ++i) {
      ...
      ret = backend_->QnnInterface().graphFinalize((*graphs_info)[i].graph,
                                                   nullptr, nullptr);
      EDGEVOX_ONNX_QNN_CHECK(ret, "Failed to call graph_finalize");
    }

    if (graphs_count > 1) {
      EDGEVOX_ONNX_LOGE("We only use the first graph: %s",
  ...
  void InitInputTensors(GraphInfo graph) {
    ...
    for (uint32_t i = 0; i < graph.num_input_tensors; ++i) {
      ...
      if (debug_) {
        EDGEVOX_ONNX_LOGE("input %d", (int)i);
        PrintTensor(p->v2);
      }
  ...
  void InitOutputTensors(GraphInfo graph) {
    ...
    for (uint32_t i = 0; i < graph.num_output_tensors; ++i) {
      ...
      if (debug_ && (i + 3 > graph.num_output_tensors)) {
        EDGEVOX_ONNX_LOGE("output %d", (int)i);
  ...
  void AllocateBuffer() {
    ...
    if (debug_) {
      EDGEVOX_ONNX_LOGE("Allocate %d bytes, or %.3f MB", static_cast<int32_t>(n),
  ...
  void SetupPointers() {
    ...
    if (debug_) {
      if (p == buffer_.data() + buffer_.size()) {
        EDGEVOX_ONNX_LOGE("Setup pointers successfully.");
      } else {
        EDGEVOX_ONNX_LOGE("Bad things happened in setting up pointers.");
      }
    }
  }
