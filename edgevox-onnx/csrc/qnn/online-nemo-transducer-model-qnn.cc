// edgevox-onnx/csrc/qnn/online-nemo-transducer-model-qnn.cc
//
...
#include "edgevox-onnx/csrc/qnn/online-nemo-transducer-model-qnn.h"
...
#include "edgevox-onnx/csrc/qnn/qnn-backend.h"
#include "edgevox-onnx/csrc/qnn/qnn-model.h"
...
class OnlineNemoTransducerModelQnn::Impl {
 public:
  ...
  std::vector<float> RunEncoder(std::vector<float> features, int32_t num_frames,
    ...
    if (!states) {
      EDGEVOX_ONNX_LOGE("states pointer is null");
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (states->size() < state_input_names_.size()) {
      EDGEVOX_ONNX_LOGE("states size %d is less than expected %d",
  ...
  std::pair<std::vector<float>, std::vector<std::vector<float>>> RunDecoder(
    ...
    if (states.size() < 2) {
      EDGEVOX_ONNX_LOGE("states should have at least 2 elements (h and c)");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  int32_t GetLanguagePromptId(const std::string &language) const {
    ...
    EDGEVOX_ONNX_LOGE("Unknown language '%s' for Nemotron 3.5 ASR.",
                     language.c_str());
    ...
    EDGEVOX_ONNX_LOGE("Available languages: %s", available.c_str());
    return dict.at("auto");
  }
 ...
 private:
  ...
  void CreateContextBinary(QnnModel *model,
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("Creating context binary '%s'.", context_binary.c_str());
    }

    bool ok = model->SaveBinaryContext(context_binary);
    if (!ok) {
      EDGEVOX_ONNX_LOGE("Failed to save context binary to '%s'",
                       context_binary.c_str());
    }

    if (config_.debug && ok) {
      EDGEVOX_ONNX_LOGE("Saved context binary to '%s'.", context_binary.c_str());
      EDGEVOX_ONNX_LOGE("Remember to also provide libQnnSystem.so.");
    }
  }
  ...
  void InitComponent(const std::string &lib_filename,
    ...
    if (!FileExists(context_binary)) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Init from %s context binary '%s'", name,
                         context_binary.c_str());
      }
  ...
  void CheckEncoder() {
    if (!encoder_->HasTensor("encoder_out")) {
      EDGEVOX_ONNX_LOGE("The encoder model does not have output 'encoder_out'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    for (const auto &name : state_input_names_) {
      ...
      if (!encoder_->HasTensor(out_name)) {
        EDGEVOX_ONNX_LOGE("The encoder model does not have output '%s'",
                         out_name.c_str());
        EDGEVOX_ONNX_EXIT(-1);
      }
    ...
    if (out_shape.size() != 3 || out_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("The encoder output should be of shape [1, ?, ?]");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void CheckDecoder() {
    ...
    if (!decoder_->HasTensor("y")) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have input 'y'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (!decoder_->HasTensor("h")) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have input 'h'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (!decoder_->HasTensor("c")) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have input 'c'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (!decoder_->HasTensor("decoder_out")) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have output 'decoder_out'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (!decoder_->HasTensor("next_h")) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have output 'next_h'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (!decoder_->HasTensor("next_c")) {
      EDGEVOX_ONNX_LOGE("The decoder model does not have output 'next_c'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("decoder input 'y': [1, 1]");
      EDGEVOX_ONNX_LOGE("decoder input 'h': [%d, 1, %d]", num_layers,
                       hidden_dim);
      EDGEVOX_ONNX_LOGE("decoder input 'c': [%d, 1, %d]", num_layers,
                       hidden_dim);
      EDGEVOX_ONNX_LOGE("decoder output 'decoder_out': [1, 1, %d]",
                       decoder_out_dim_);
    }
  }
  ...
  void CheckJoiner() {
    ...
    if (!joiner_->HasTensor(joiner_output_name_)) {
      EDGEVOX_ONNX_LOGE("The joiner model does not have output '%s'",
                       joiner_output_name_.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (!joiner_->HasTensor(joiner_encoder_input_name_)) {
      EDGEVOX_ONNX_LOGE("The joiner model does not have input '%s'",
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("encoder_out shape: [%d, %d, %d]", enc_out_shape[0],
                       enc_out_shape[1], enc_out_shape[2]);
      EDGEVOX_ONNX_LOGE("encoder_out_dim: %d, num_encoder_frames: %d, "
                       "needs_transpose: %d",
                       encoder_out_dim_, num_encoder_frames_,
                       encoder_out_needs_transpose_);
      EDGEVOX_ONNX_LOGE("subsampling_factor: %d", subsampling_factor_);
      EDGEVOX_ONNX_LOGE("joiner input '%s': [1, %d, 1]",
                       joiner_encoder_input_name_.c_str(), encoder_out_dim_);
      EDGEVOX_ONNX_LOGE("joiner input '%s': [1, %d, 1]",
                       joiner_decoder_input_name_.c_str(), decoder_out_dim_);
      EDGEVOX_ONNX_LOGE("joiner output '%s': rank %d, vocab_size %d",
                       joiner_output_name_.c_str(),
