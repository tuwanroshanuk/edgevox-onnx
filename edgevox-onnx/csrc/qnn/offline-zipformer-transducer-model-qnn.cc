// edgevox-onnx/csrc/qnn/offline-zipformer-transducer-model-qnn.cc
//
// Copyright (c)  2026  Xiaomi Corporation

#include "edgevox-onnx/csrc/qnn/offline-zipformer-transducer-model-qnn.h"
 ...
 public:
  ...
  std::vector<float> RunDecoder(const std::vector<int32_t> &tokens) const {
    if (static_cast<int32_t>(tokens.size()) != context_size_) {
      EDGEVOX_ONNX_LOGE("Expected decoder input size %d, given %d",
  ...
  std::vector<float> RunJoiner(const float *encoder_out,
    ...
    if (static_cast<int32_t>(decoder_out.size()) != decoder_out_dim_) {
      EDGEVOX_ONNX_LOGE("Expected joiner decoder input size %d, given %d",
                       decoder_out_dim_,
 ...
 private:
  ...
  void CheckDecoder() {
    ...
    if (input_names.size() != 1) {
      EDGEVOX_ONNX_LOGE("Expected one decoder input. Given %d",
    ...
    if (in_shape.size() != 2 || in_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("Expected decoder input shape [1, context_size]");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (out_shape.size() != 2 || out_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("The decoder output should be of shape [1, decoder_dim]");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void CheckJoiner() {
    ...
    if (out_shape.size() != 2 || out_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("The joiner output should be of shape [1, vocab_size]");
      EDGEVOX_ONNX_EXIT(-1);
    }

    vocab_size_ = out_shape[1];
    if (vocab_size_ <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid joiner output shape. vocab_size: %d",
                       vocab_size_);
      EDGEVOX_ONNX_EXIT(-1);
    }
  }
