...
#ifndef EDGEVOX_ONNX_CSRC_QNN_UTILS_H_
#define EDGEVOX_ONNX_CSRC_QNN_UTILS_H_
#include <stdio.h>
...
#ifdef _WIN32
  FILE *fp = _wfopen(edgevox_onnx::ToWideString(filename).c_str(), L"rb");
#else
  FILE *fp = fopen(filename.c_str(), "rb");
#endif
  if (!fp) {
    EDGEVOX_ONNX_LOGE("Failed to open '%s'", filename.c_str());
    return {};
  }
...
#endif  // EDGEVOX_ONNX_CSRC_QNN_UTILS_H_
