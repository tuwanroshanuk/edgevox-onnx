// edgevox-onnx/csrc/qnn/offline-paraformer-model-qnn.cc
//
...
class OfflineParaformerModelQnn::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config) : config_(config) {
    ...
    if (!filenames.empty()) {
      if (filenames.size() != 3) {
        EDGEVOX_ONNX_LOGE("Invalid Paraformer QNN model '%s'",
 ...
 private:
  ...
  bool InitEncoder(const std::string &lib_filename,
    ...
    if (context_binary.empty()) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Init from encoder context binary '%s'",
  ...
  bool InitPredictor(const std::string &lib_filename,
    ...
    if (context_binary.empty()) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Init from predictor context binary '%s'",
                         context_binary.c_str());
      }
  ...
  bool InitDecoder(const std::string &lib_filename,
    ...
    if (context_binary.empty()) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Init from decoder context binary '%s'",
                         context_binary.c_str());
      }
  ...
  void CheckEncoderModel() {
    ...
    if (input_tensor_names.size() != 1) {
      EDGEVOX_ONNX_LOGE("Expect 1 input tensor. Actual %d",
                       static_cast<int32_t>(input_tensor_names.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (input_tensor_names[0] != "x") {
      EDGEVOX_ONNX_LOGE("The 1st input should be x, actual '%s'",
    ...
    if (x_shape.size() != 3) {
      EDGEVOX_ONNX_LOGE("The 1st input should be 3-d, actual '%d'",
                       static_cast<int32_t>(x_shape.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (x_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("The x.shape[0] should be 1, actual '%d'", x_shape[0]);
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (!encoder_model_->HasTensor("encoder_out")) {
      EDGEVOX_ONNX_LOGE("Model does not have output node 'encoder_out'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("num_input_frames: %d", num_input_frames_);
      EDGEVOX_ONNX_LOGE("feat_dim: %d", feat_dim_);
      EDGEVOX_ONNX_LOGE("encoder_out_dim1: %d", encoder_out_dim1_);
      EDGEVOX_ONNX_LOGE("encoder_out_dim2: %d", encoder_out_dim2_);
    }
  }
  ...
  void CheckPredictorModel() {
    ...
    if (input_tensor_names[0] != "encoder_out") {
      EDGEVOX_ONNX_LOGE("The 1st input should be encoder_out, actual '%s'",
                       input_tensor_names[0].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (!predictor_model_->HasTensor("alphas")) {
      EDGEVOX_ONNX_LOGE("Model does not have output node 'alphas'");
      EDGEVOX_ONNX_EXIT(-1);
    }

    std::vector<int32_t> alphas_shape = predictor_model_->TensorShape("alphas");
    if (alphas_shape.size() != 2) {
      EDGEVOX_ONNX_LOGE("alphas should be 2-d, given: %d",
                       static_cast<int32_t>(alphas_shape.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (alphas_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("We support only batch size 1 for alphas. Given: %d",
                       alphas_shape[0]);
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (alphas_shape[1] != encoder_out_dim1_) {
      EDGEVOX_ONNX_LOGE("Expected output dim %d for alphas. Given: %d",
  ...
  void CheckDecoderModel() {
    ...
    if (input_tensor_names.size() != 3) {
      EDGEVOX_ONNX_LOGE("Expect 3 input tensors. Actual %d",
                       static_cast<int32_t>(input_tensor_names.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (input_tensor_names[2] != "mask") {
      EDGEVOX_ONNX_LOGE("The 3rd input should be mask, actual '%s'",
                       input_tensor_names[2].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (!decoder_model_->HasTensor("decoder_out")) {
      EDGEVOX_ONNX_LOGE("Model does not have output node 'decoder_out'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (decoder_out_shape.size() != 3) {
      EDGEVOX_ONNX_LOGE("decoder_out should be 3-d, given: %d",
                       static_cast<int32_t>(decoder_out_shape.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (decoder_out_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("We support only batch size 1 for decoder. Given: %d",
                       decoder_out_shape[0]);
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (decoder_out_shape[2] != encoder_out_dim1_) {
      EDGEVOX_ONNX_LOGE("Expected output dim %d for decoder_out. Given: %d",
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("vocab_size: %d", vocab_size_);
    }
  }
