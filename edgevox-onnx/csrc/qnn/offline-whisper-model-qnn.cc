// edgevox-onnx/csrc/qnn/offline-whisper-model-qnn.cc
//
...
static std::string ParseModelPrefix(const std::string &input_name) {
  ...
  if (pos == std::string::npos) {
    EDGEVOX_ONNX_LOGE("Unexpected encoder input name: '%s'", input_name.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  OfflineWhisperDecoderResult Run(std::vector<float> features) const {
    ...
    if (IsMultilingual(model_type_)) {
      ...
      if (!config_.whisper.language.empty()) {
        ...
        if (lang_id < 0) {
          EDGEVOX_ONNX_LOGE("Unsupported language: '%s'",
        ...
        if (config_.debug) {
          EDGEVOX_ONNX_LOGE("Detecting language.");
        }
        ...
        if (config_.debug) {
          EDGEVOX_ONNX_LOGE("Detected Language: %s", r.lang.c_str());
        }
 ...
 private:
  void InitEncoder() {
    ...
    if (encoder_binary.empty()) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Init encoder from context binary '%s'",
                         encoder_binary.c_str());
      }
  ...
  void InitDecoder() {
    ...
    if (decoder_binary.empty()) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("Init decoder from context binary '%s'",
                         decoder_binary.c_str());
      }
  ...
  void InitEncoderFromModelLib() {
    ...
    if (encoder_path.empty()) {
      EDGEVOX_ONNX_LOGE("Please provide --whisper-encoder");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void InitDecoderFromModelLib() {
    ...
    if (decoder_path.empty()) {
      EDGEVOX_ONNX_LOGE("Please provide --whisper-decoder");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void PostInitEncoder() {
    ...
    if (names.empty()) {
      EDGEVOX_ONNX_LOGE("Encoder has no input tensors");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (!encoder_model_->HasTensor("cross_k_0")) {
      EDGEVOX_ONNX_LOGE("Encoder does not have output tensor 'cross_k_0'");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("feat_dim_: %d", feat_dim_);
      EDGEVOX_ONNX_LOGE("num_frames_: %d", num_frames_);
      EDGEVOX_ONNX_LOGE("num_out_frames_: %d", num_out_frames_);
      EDGEVOX_ONNX_LOGE("n_text_layer_: %d", n_text_layer_);
      EDGEVOX_ONNX_LOGE("n_text_state_: %d", n_text_state_);
    }
  }
  ...
  void PostInitDecoder() {
    ...
    if (static_cast<int32_t>(input_names.size()) != expected_num_inputs) {
      EDGEVOX_ONNX_LOGE("Expect %d decoder inputs. Actual: %d",
                       expected_num_inputs,
    ...
    if (!decoder_model_->HasTensor(mask_name)) {
      EDGEVOX_ONNX_LOGE("Decoder does not have input tensor '%s'",
                       mask_name.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (!decoder_model_->HasTensor(logits_name)) {
      EDGEVOX_ONNX_LOGE("Decoder does not have output tensor '%s'",
                       logits_name.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("mask_size_: %d", mask_size_);
      EDGEVOX_ONNX_LOGE("n_text_state_: %d", n_text_state_);
      EDGEVOX_ONNX_LOGE("vocab_size_: %d", vocab_size_);
      EDGEVOX_ONNX_LOGE("self_kv_size_: %d", self_kv_size_);
      EDGEVOX_ONNX_LOGE("self_kv_stride_: %d", self_kv_stride_);
    }
  }
  ...
  void InitSotSequence() {
    switch (model_type_) {
      ...
      case WhisperModelType::Large:
        ...
        break;
      default:
        EDGEVOX_ONNX_LOGE("Unsupported model type: '%s'",
