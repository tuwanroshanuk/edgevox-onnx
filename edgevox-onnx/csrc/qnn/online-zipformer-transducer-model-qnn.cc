// edgevox-onnx/csrc/qnn/online-zipformer-transducer-model-qnn.cc
//
// Copyright (c)  2026  Xiaomi Corporation

#include "edgevox-onnx/csrc/qnn/online-zipformer-transducer-model-qnn.h"
 ...
 private:
  ...
  void CheckEncoder() {
    ...
    if (encoder_input_name_.empty()) {
      if (has_parakeet_pattern) {
        ...
        EDGEVOX_ONNX_LOGE("The encoder model does not have input 'x'");
      }
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (state_input_names_.empty()) {
      EDGEVOX_ONNX_LOGE("The encoder model does not contain streaming states");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void CheckJoiner() {
    if (!joiner_->HasTensor("logit")) {
      EDGEVOX_ONNX_LOGE("The joiner model does not have output 'logit'");
      EDGEVOX_ONNX_EXIT(-1);
    }
