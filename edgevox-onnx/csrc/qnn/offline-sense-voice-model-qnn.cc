// edgevox-onnx/csrc/qnn/offline-sense-voice-model-qnn.cc
//
 ...
 private:
  ...
  void CheckModel() {
    ...
    if (input_tensor_names.size() != 2) {
      EDGEVOX_ONNX_LOGE("Expect two input tensors. Actual %d",
    ...
    if (input_tensor_names[1] != "prompt") {
      EDGEVOX_ONNX_LOGE("The 2nd input should be prompt, actual '%s'",
    ...
    if (x_shape[2] != feat_dim_) {
      EDGEVOX_ONNX_LOGE("The x.shape[2] should be %d, actual '%d'", feat_dim_,
                       x_shape[2]);
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (prompt_shape.size() != 1) {
      EDGEVOX_ONNX_LOGE("The 2nd input should be 1-d, actual '%d'",
                       static_cast<int32_t>(prompt_shape.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (prompt_shape[0] != 4) {
      EDGEVOX_ONNX_LOGE("The prompt.shape[0] should be 4, actual '%d'",
                       prompt_shape[0]);
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (!model_->HasTensor("logits")) {
      EDGEVOX_ONNX_LOGE("Model does not have output node 'logits'");
      EDGEVOX_ONNX_EXIT(-1);
    }
