// edgevox-onnx/csrc/online-recognizer.cc
//
...
bool OnlineRecognizerConfig::Validate() const {
  if (decoding_method == "modified_beam_search") {
    if (max_active_paths <= 0) {
      EDGEVOX_ONNX_LOGE("max_active_paths must be > 0. Given: %d",
                       max_active_paths);
      return false;
    }
  }
  ...
  if (!ctc_fst_decoder_config.graph.empty() &&
    ...
    EDGEVOX_ONNX_LOGE("Errors in ctc_fst_decoder_config");
    return false;
  }

  if (!hotwords_file.empty() && !FileExists(hotwords_file)) {
    EDGEVOX_ONNX_LOGE("--hotwords-file: '%s' does not exist",
                     hotwords_file.c_str());
    return false;
  }
