// edgevox-onnx/csrc/offline-sense-voice-model.cc
//
// Copyright (c)  2022-2023  Xiaomi Corporation

#include "edgevox-onnx/csrc/offline-sense-voice-model.h"
...
class OfflineSenseVoiceModel::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.sense_voice.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    std::string comment;
    EDGEVOX_ONNX_READ_META_DATA_STR_ALLOW_EMPTY(comment, "comment");

    meta_data_.is_funasr_nano = Contains(comment, "Nano");

    EDGEVOX_ONNX_READ_META_DATA(meta_data_.vocab_size, "vocab_size");
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_data_.blank_id, "blank_id", 0);

    EDGEVOX_ONNX_READ_META_DATA(meta_data_.window_size, "lfr_window_size");
    EDGEVOX_ONNX_READ_META_DATA(meta_data_.window_shift, "lfr_window_shift");
    EDGEVOX_ONNX_READ_META_DATA(meta_data_.normalize_samples,
                               "normalize_samples");

    if (!meta_data_.is_funasr_nano) {
      EDGEVOX_ONNX_READ_META_DATA(meta_data_.with_itn_id, "with_itn");

      EDGEVOX_ONNX_READ_META_DATA(meta_data_.without_itn_id, "without_itn");
      ...
      EDGEVOX_ONNX_READ_META_DATA(lang_auto, "lang_auto");
      EDGEVOX_ONNX_READ_META_DATA(lang_zh, "lang_zh");
      EDGEVOX_ONNX_READ_META_DATA(lang_en, "lang_en");
      EDGEVOX_ONNX_READ_META_DATA(lang_ja, "lang_ja");
      EDGEVOX_ONNX_READ_META_DATA(lang_ko, "lang_ko");
      EDGEVOX_ONNX_READ_META_DATA(lang_yue, "lang_yue");
      ...
      EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(meta_data_.neg_mean, "neg_mean");
      EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(meta_data_.inv_stddev, "inv_stddev");
    }
  }
