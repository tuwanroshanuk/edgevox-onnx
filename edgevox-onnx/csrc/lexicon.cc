...
std::vector<int32_t> ConvertTokensToIds(
  ...
  for (const auto &s : tokens) {
    if (!token2id.count(s)) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("Unknown token: %{public}s", s.c_str());
#else
      EDGEVOX_ONNX_LOGE("Unknown token: %s", s.c_str());
#endif
      return {};
    }
...
std::vector<TokenIDs> Lexicon::ConvertTextToTokenIds(
  ...
  switch (language_) {
    ...
    default:
      EDGEVOX_ONNX_LOGE("Unknown language: %d", static_cast<int32_t>(language_));
      EDGEVOX_ONNX_EXIT(-1);
  }
...
std::vector<TokenIDs> Lexicon::ConvertTextToTokenIdsChinese(
  ...
  for (const auto &w : words) {
    ...
    if (!word2ids_.count(w)) {
      EDGEVOX_ONNX_LOGE("OOV %s. Ignore it!", w.c_str());
      continue;
    }
...
#if __OHOS__
    EDGEVOX_ONNX_LOGE("Unknown language: %{public}s", _lang.c_str());
#else
    EDGEVOX_ONNX_LOGE("Unknown language: %s", _lang.c_str());
#endif
    EDGEVOX_ONNX_EXIT(-1);
  }
}
...
void Lexicon::InitLexicon(std::istream &is) {
  ...
  while (std::getline(is, line)) {
    ...
    if (word2ids_.count(word)) {
      EDGEVOX_ONNX_LOGE("Duplicated word: %s. Ignore it.", word.c_str());
      continue;
    }
