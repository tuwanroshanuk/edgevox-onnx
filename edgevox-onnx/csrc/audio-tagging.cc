// edgevox-onnx/csrc/audio-tagging.cc
//
...
bool AudioTaggingConfig::Validate() const {
  ...
  if (top_k < 1) {
    EDGEVOX_ONNX_LOGE("--top-k should be >= 1. Given: %d", top_k);
    return false;
  }

  if (labels.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --labels");
    return false;
  }

  if (!FileExists(labels)) {
    EDGEVOX_ONNX_LOGE("--labels '%s' does not exist", labels.c_str());
    return false;
  }
