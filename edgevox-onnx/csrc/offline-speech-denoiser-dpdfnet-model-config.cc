// edgevox-onnx/csrc/offline-speech-denoiser-dpdfnet-model-config.cc
//
...
bool OfflineSpeechDenoiserDpdfNetModelConfig::Validate() const {
  if (model.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --speech-denoiser-dpdfnet-model");
    return false;
  }

  if (!FileExists(model)) {
    EDGEVOX_ONNX_LOGE("dpdfnet model file '%s' does not exist", model.c_str());
    return false;
  }
