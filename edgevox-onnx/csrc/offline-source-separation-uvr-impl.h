// edgevox-onnx/csrc/offline-source-separation-uvr-impl.h
//
// Copyright (c)  2025  Xiaomi Corporation

#ifndef EDGEVOX_ONNX_CSRC_OFFLINE_SOURCE_SEPARATION_UVR_IMPL_H_
#define EDGEVOX_ONNX_CSRC_OFFLINE_SOURCE_SEPARATION_UVR_IMPL_H_
...
class OfflineSourceSeparationUvrImpl : public OfflineSourceSeparationImpl {
 ...
 private:
  std::pair<std::vector<float>, std::vector<float>> ProcessChunk(
    ...
    if (num_frames != dim_t) {
      EDGEVOX_ONNX_LOGE("num_frames(%d) != dim_t(%d)", num_frames, dim_t);
      EDGEVOX_ONNX_EXIT(-1);
    }
...
#endif  // EDGEVOX_ONNX_CSRC_OFFLINE_SOURCE_SEPARATION_UVR_IMPL_H_
