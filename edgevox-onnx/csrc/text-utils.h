// edgevox-onnx/csrc/text-utils.h
//
// Copyright 2009-2011  Saarland University;  Microsoft Corporation
// Copyright      2023  Xiaomi Corporation
#ifndef EDGEVOX_ONNX_CSRC_TEXT_UTILS_H_
#define EDGEVOX_ONNX_CSRC_TEXT_UTILS_H_
#include <errno.h>
#include <stdlib.h>
...
#ifdef _MSC_VER
#define EDGEVOX_ONNX_STRTOLL(cur_cstr, end_cstr) \
  _strtoi64(cur_cstr, end_cstr, 10);
#else
#define EDGEVOX_ONNX_STRTOLL(cur_cstr, end_cstr) strtoll(cur_cstr, end_cstr, 10);
#endif
...
bool ConvertStringToInteger(const std::string &str, Int *out) {
  ...
  char *end = nullptr;
  errno = 0;
  int64_t i = EDGEVOX_ONNX_STRTOLL(this_str, &end);
  if (end != this_str) {
...
bool SplitStringToIntegers(const std::string &full, const char *delim,
  ...
  for (size_t i = 0; i < split.size(); i++) {
    ...
    char *end = NULL;
    int64_t j = 0;
    j = EDGEVOX_ONNX_STRTOLL(this_str, &end);
...
#if defined(_WIN32)
#define EDGEVOX_ONNX_TO_ORT_PATH(s) (ToWideString(s).c_str())
#else
#define EDGEVOX_ONNX_TO_ORT_PATH(s) ((s).c_str())
#endif
...
#endif  // EDGEVOX_ONNX_CSRC_TEXT_UTILS_H_
