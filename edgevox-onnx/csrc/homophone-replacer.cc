// edgevox-onnx/csrc/homophone-replacer.cc
//
...
bool HomophoneReplacerConfig::Validate() const {
  if (!dict_dir.empty()) {
    EDGEVOX_ONNX_LOGE(
        "From edgevox-onnx v1.12.15, you don't need to provide dict_dir for "
        "this model. Ignore it");
  }

  if (!lexicon.empty() && !FileExists(lexicon)) {
    EDGEVOX_ONNX_LOGE("--hr-lexicon: '%s' does not exist", lexicon.c_str());
    return false;
  }
  ...
  if (!rule_fsts.empty()) {
    ...
    if (files.size() > 1) {
      EDGEVOX_ONNX_LOGE("Only 1 file is supported now.");
      EDGEVOX_ONNX_EXIT(-1);
    }

    for (const auto &f : files) {
      if (!FileExists(f)) {
        EDGEVOX_ONNX_LOGE("Rule fst '%s' does not exist. ", f.c_str());
        return false;
      }
    }
  }
 ...
 public:
  explicit Impl(const HomophoneReplacerConfig &config) : config_(config) {
    ...
    if (!config.rule_fsts.empty()) {
      ...
      for (const auto &f : files) {
        if (config_.debug) {
          EDGEVOX_ONNX_LOGE("hr rule fst: %s", f.c_str());
        }
  ...
  std::string Apply(const std::string &text) const {
    ...
    if (config_.debug) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("Input text: '%{public}s'", text.c_str());
#else
      EDGEVOX_ONNX_LOGE("Input text: '%s'", text.c_str());
#endif
      std::ostringstream os;
    ...
    for (const std::string &w : matcher) {
      ...
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("%s %s", w.c_str(), p.c_str());
      }
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("Output text: '%s'", ans.c_str());
    }
