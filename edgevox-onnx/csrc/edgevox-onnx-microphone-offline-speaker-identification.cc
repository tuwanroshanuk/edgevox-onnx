// edgevox-onnx/csrc/edgevox-onnx-microphone-offline-speaker-identification.cc
//
...
static std::vector<std::vector<float>> ComputeEmbeddings(
    ...
    edgevox_onnx::SpeakerEmbeddingExtractor *extractor) {
  ...
  for (const auto &f : filenames) {
    ...
    const std::vector<float> samples =
        edgevox_onnx::ReadWave(f, &sampling_rate, &is_ok);
...
ReadSpeakerFile(const std::string &filename) {
  ...
  auto is = edgevox_onnx::OpenInputFile(filename);
  if (!is) {
...
(3) Run it !
  ...
  ./bin/edgevox-onnx-microphone-offline-speaker-identification \
  ...
  edgevox_onnx::SpeakerEmbeddingExtractorConfig config;
  config.Register(&po);
  ...
  EDGEVOX_ONNX_LOGE("\nCreating extractor ...");
  edgevox_onnx::SpeakerEmbeddingExtractor extractor(config);
  EDGEVOX_ONNX_LOGE("\nextractor created!");

  edgevox_onnx::SpeakerEmbeddingManager manager(extractor.Dim());
  ...
  if (device_index == paNoDevice) {
    ...
    fprintf(stderr,
            " ./bin/edgevox-onnx-alsa-offline-speaker-identification \n");
    EDGEVOX_ONNX_EXIT(EXIT_FAILURE);
  }
