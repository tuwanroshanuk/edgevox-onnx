// edgevox-onnx/csrc/offline-transducer-nemo-model.cc
//
...
class OfflineTransducerNeMoModel::Impl {
 ...
 private:
  void InitEncoder(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(is_giga_am_, "is_giga_am", 0);
    ...
    std::string url;
    EDGEVOX_ONNX_READ_META_DATA_STR_ALLOW_EMPTY(url, "url");
  ...
  void InitJoiner(void *model_data, size_t model_data_length) {
    ...
    if (is_tdt_) {
      if (vocab_size_ == output_size) {
        EDGEVOX_ONNX_LOGE("It is not a TDT model!");
        EDGEVOX_ONNX_EXIT(-1);
      }

      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("TDT model. vocab_size: %d, num_durations: %d",
                         vocab_size_, output_size - vocab_size_);
      }
    } else if (vocab_size_ != output_size) {
      EDGEVOX_ONNX_LOGE("vocab_size: %d != output_size: %d", vocab_size_,
                       output_size);
      EDGEVOX_ONNX_EXIT(-1);
    }
  }
