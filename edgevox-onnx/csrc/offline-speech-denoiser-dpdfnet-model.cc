// edgevox-onnx/csrc/offline-speech-denoiser-dpdfnet-model.cc
//
 ...
 public:
  explicit Impl(const OfflineSpeechDenoiserModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.dpdfnet.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if (model_type != "dpdfnet") {
      EDGEVOX_ONNX_LOGE("Expect model type 'dpdfnet'. Given: '%s'",
                       model_type.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.version, "version", 1);
    EDGEVOX_ONNX_READ_META_DATA_STR(meta_.profile, "profile");
    ...
    EDGEVOX_ONNX_READ_META_DATA(meta_.window_length, "window_length");
    int32_t normalized = 0;
    int32_t center = 1;
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(normalized, "normalized", 0);
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(center, "center", 1);
    EDGEVOX_ONNX_READ_META_DATA_STR(meta_.window_type, "window_type");
    EDGEVOX_ONNX_READ_META_DATA_STR(meta_.pad_mode, "pad_mode");
    EDGEVOX_ONNX_READ_META_DATA(meta_.freq_bins, "freq_bins");
    EDGEVOX_ONNX_READ_META_DATA(meta_.erb_bins, "erb_bins");
    EDGEVOX_ONNX_READ_META_DATA(meta_.spec_bins, "spec_bins");
    EDGEVOX_ONNX_READ_META_DATA(meta_.state_size, "state_size");
    EDGEVOX_ONNX_READ_META_DATA(meta_.erb_norm_state_size,
                               "erb_norm_state_size");
    EDGEVOX_ONNX_READ_META_DATA(meta_.spec_norm_state_size,
                               "spec_norm_state_size");
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(meta_.erb_norm_init, "erb_norm_init");
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(meta_.spec_norm_init,
                                         "spec_norm_init");
