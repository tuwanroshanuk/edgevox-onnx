// edgevox-onnx/csrc/offline-tts-kokoro-model-config.cc
//
...
bool OfflineTtsKokoroModelConfig::Validate() const {
  if (model.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --kokoro-model");
    return false;
  }

  if (!FileExists(model)) {
    EDGEVOX_ONNX_LOGE("--kokoro-model: '%s' does not exist", model.c_str());
    return false;
  }

  if (tokens.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --kokoro-tokens");
    return false;
  }

  if (!FileExists(tokens)) {
    EDGEVOX_ONNX_LOGE("--kokoro-tokens: '%s' does not exist", tokens.c_str());
    return false;
  }
  ...
  if (data_dir.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --kokoro-data-dir");
    return false;
  }
  ...
  if (!dict_dir.empty()) {
    EDGEVOX_ONNX_LOGE(
        "From edgevox-onnx v1.12.15, you don't need to provide dict_dir or "
