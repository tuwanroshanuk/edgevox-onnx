// edgevox-onnx/csrc/online-ctc-fst-decoder.cc
//
...
void OnlineCtcFstDecoder::Decode(const float *log_probs, int32_t batch_size,
  ...
  if (batch_size != results->size()) {
    EDGEVOX_ONNX_LOGE("Size mismatch! log_probs.size(0) %d, results.size(0): %d",
                     batch_size, static_cast<int32_t>(results->size()));
    EDGEVOX_ONNX_EXIT(-1);
  }

  if (batch_size != n) {
    EDGEVOX_ONNX_LOGE("Size mismatch! log_probs.size(0) %d, n: %d", batch_size,
                     n);
    EDGEVOX_ONNX_EXIT(-1);
  }
