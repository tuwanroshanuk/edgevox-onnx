// edgevox-onnx/csrc/offline-tts-model-config.cc
//
...
bool OfflineTtsModelConfig::Validate() const {
  if (num_threads < 1) {
    EDGEVOX_ONNX_LOGE("num_threads should be > 0. Given %d", num_threads);
    return false;
  }
  ...
  EDGEVOX_ONNX_LOGE("Please provide exactly one tts model.");
