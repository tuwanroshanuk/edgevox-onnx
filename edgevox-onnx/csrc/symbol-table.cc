// edgevox-onnx/csrc/symbol-table.cc
//
...
#include "edgevox-onnx/csrc/base64-decode.h"
#include "edgevox-onnx/csrc/bbpe.h"
...
std::unordered_map<std::string, int32_t> ReadTokens(
  ...
  while (std::getline(is, line)) {
    ...
    iss >> std::ws;
    if (!iss.eof()) {
      EDGEVOX_ONNX_LOGE("Error: %s", line.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
...
void SymbolTable::ApplyBase64Decode() {
  ...
  for (auto &p : id2sym_) {
    if (p.second == " ") {
      ...
      // which is converted to " " while reading it in edgevox-onnx. We convert
      // it back to "" here
      p.second = "";
    } else {
...
std::string SymbolTable::DecodeByteBpe(const std::string &text) const {
  ...
  for (const auto &s : v) {
    if (s == "▁") {
      ...
      // Should not happen
      EDGEVOX_ONNX_LOGE("Skip OOV: %s from %s", s.c_str(), text.c_str());
    }
  }
