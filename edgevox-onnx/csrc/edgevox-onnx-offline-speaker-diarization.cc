// edgevox-onnx/csrc/edgevox-onnx-offline-speaker-diarization.cc
//
...
Offline/Non-streaming speaker diarization with edgevox-onnx
Usage example:
...
a smaller threshold leads to more clusters, i.e., more speakers
  ...
  edgevox_onnx::OfflineSpeakerDiarizationConfig config;
  ...
  edgevox_onnx::OfflineSpeakerDiarization sd(config);
  ...
  const std::vector<float> samples =
      edgevox_onnx::ReadWave(wav_filename, &sample_rate, &is_ok);
  if (!is_ok) {
