// edgevox-onnx/csrc/matcha-tts-lexicon.cc
//
...
class MatchaTtsLexicon::Impl {
 public:
   Impl(const std::string &lexicon, const std::string &tokens,
     ...
     EDGEVOX_ONNX_LOGE("MatchaTtsLexicon espeak_voice=%s skip_replacement=%d",
                      espeak_voice_.c_str(),
                      static_cast<int>(skip_replacement_));

     if (lexicon.empty()) {
       EDGEVOX_ONNX_LOGE("Please provide lexicon.txt for this model");
       EDGEVOX_ONNX_EXIT(-1);
     }
     ...
     if (data_dir.empty()) {
       EDGEVOX_ONNX_LOGE("Please provide data dir for this model");
       EDGEVOX_ONNX_EXIT(-1);
     }
   ...
   std::vector<TokenIDs> ConvertTextToTokenIds(const std::string &_text) const {
     ...
     if (debug_) {
       EDGEVOX_ONNX_LOGE("After replacing punctuations and merging spaces:\n%s",
                        text.c_str());
     }
     ...
     if (debug_) {
#if __OHOS__
       EDGEVOX_ONNX_LOGE("input text:\n%{public}s", _text.c_str());
       EDGEVOX_ONNX_LOGE("after replacing punctuations:\n%{public}s",
                        text.c_str());
#else
       EDGEVOX_ONNX_LOGE("input text:\n%s", _text.c_str());
       EDGEVOX_ONNX_LOGE("after replacing punctuations:\n%s", text.c_str());
...
#if __OHOS__
       EDGEVOX_ONNX_LOGE("after splitting into UTF8:\n%{public}s",
                        os.str().c_str());
#else
       EDGEVOX_ONNX_LOGE("after splitting into UTF8:\n%s", os.str().c_str());
#endif
...
#if __OHOS__
       EDGEVOX_ONNX_LOGE("after removing spaces after punctuations:\n%{public}s",
                        os.str().c_str());
#else
       EDGEVOX_ONNX_LOGE("after removing spaces after punctuations:\n%s",
                        os.str().c_str());
     ...
     for (const std::string &w : matcher) {
       ...
       if (ids.empty()) {
#if __OHOS__
         EDGEVOX_ONNX_LOGE("Ignore OOV '%{public}s'", w.c_str());
#else
         EDGEVOX_ONNX_LOGE("Ignore OOV '%s'", w.c_str());
#endif
  ...
  private:
   std::vector<int32_t> ConvertWordToIds(const std::string &w) const {
     ...
     if (word2ids_.count(w)) {
       ...
       if (ContainsCJK(w)) {
         ...
         if (debug_) {
           EDGEVOX_ONNX_LOGE("use espeak for %s", w.c_str());
         }
         // use espeak
         ...
         for (const auto &p : pp) {
           if (token2id_.count(p)) {
             ...
             EDGEVOX_ONNX_LOGE("Skip token: %s", p.c_str());
           }
         }
       }
     }
   ...
   void InitLexicon(const std::string &lexicon) {
     if (lexicon.empty()) {
       EDGEVOX_ONNX_LOGE("Empty lexicon!");
       return;
     }
   ...
   void InitLexicon(std::istream &is) {
     ...
     while (std::getline(is, line)) {
...
#else
         EDGEVOX_ONNX_LOGE("Duplicated word: %s at line %d:%s. Ignore it.",
                          word.c_str(), line_num, line.c_str());
#endif
         continue;
       }
       ...
       if (ids.empty()) {
         if (debug_) {
#if __OHOS__
           EDGEVOX_ONNX_LOGE("Empty token ids for '%{public}s'", line.c_str());
#else
           EDGEVOX_ONNX_LOGE("Empty token ids for '%s'", line.c_str());
#endif
         }
         continue;
       }
