// edgevox-onnx/csrc/offline-websocket-server.cc
//
...
#include "edgevox-onnx/csrc/offline-websocket-server-impl.h"
...
./bin/edgevox-onnx-offline-websocket-server --help

(1) For transducer models

./bin/edgevox-onnx-offline-websocket-server \
  --port=6006 \
  --num-work-threads=5 \
...
int32_t main(int32_t argc, char *argv[]) {
  ...
  edgevox_onnx::OfflineWebsocketServerConfig config;
  ...
  if (po.NumArgs() != 0) {
    EDGEVOX_ONNX_LOGE("Unrecognized positional arguments!");
    po.PrintUsage();
    EDGEVOX_ONNX_EXIT(EXIT_FAILURE);
  }
  ...
  edgevox_onnx::OfflineWebsocketServer server(io_conn, io_work, config);
  server.Run(port);

  EDGEVOX_ONNX_LOGE("Started!");
  EDGEVOX_ONNX_LOGE("Listening on: %d", port);
  EDGEVOX_ONNX_LOGE("Number of work threads: %d", num_work_threads);
