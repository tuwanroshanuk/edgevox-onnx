// edgevox-onnx/csrc/offline-cohere-transcribe-model.cc
//
...
class OfflineCohereTranscribeModel::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.cohere_transcribe.encoder),
        sess_opts_);
    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.cohere_transcribe.decoder),
        sess_opts_);
 ...
 private:
  void InitEncoder(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA_STR_ALLOW_EMPTY(model_type, "model_type");
    ...
    if (shape.size() != 4) {
      EDGEVOX_ONNX_LOGE("Expect 4-d for encoder output 0. Given: %d",
  ...
  void InitDecoder(void *model_data, size_t model_data_length) {
    ...
    if (shape.size() != 5) {
      EDGEVOX_ONNX_LOGE("Expect 5-d for decoder output 1. Given: %d",
                       static_cast<int32_t>(shape.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (shape[0] != num_layers_) {
      EDGEVOX_ONNX_LOGE("Expected num_layers %d. Given: %d", num_layers_,
