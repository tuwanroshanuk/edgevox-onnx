// edgevox-onnx/csrc/offline-cohere-transcribe-model-config.cc
//
...
bool OfflineCohereTranscribeModelConfig::Validate() const {
  if (encoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --cohere-transcribe-encoder");
    return false;
  }

  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("cohere-transcribe encoder file '%s' does not exist",
                     encoder.c_str());
    return false;
  }

  if (decoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --cohere-transcribe-decoder");
    return false;
  }

  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("cohere-transcribe decoder file '%s' does not exist",
                     decoder.c_str());
    return false;
  }
