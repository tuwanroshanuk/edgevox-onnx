// edgevox-onnx/csrc/offline-whisper-model-config.cc
//
...
bool OfflineWhisperModelConfig::Validate() const {
  ...
  if (uses_qnn) {
    ...
    if (need_model_libs) {
      ...
      if (!FileExists(encoder)) {
        EDGEVOX_ONNX_LOGE("whisper encoder: '%s' does not exist",
                         encoder.c_str());
        return false;
      }

      if (!FileExists(decoder)) {
        EDGEVOX_ONNX_LOGE("whisper decoder: '%s' does not exist",
                         decoder.c_str());
        return false;
      }
    }
    ...
    for (const auto &name : context_binaries) {
      if (FileExists(name) && !EndsWith(name, ".bin")) {
        EDGEVOX_ONNX_LOGE("QNN context binary should end with .bin. Given '%s'",
                         name.c_str());
        return false;
      }
    }
  ...
  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("whisper encoder: '%s' does not exist", encoder.c_str());
    return false;
  }
  ...
  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("whisper decoder: '%s' does not exist", decoder.c_str());
    return false;
  }
...
bool IsMultilingual(WhisperModelType model_type) {
  ...
  EDGEVOX_ONNX_LOGE("Unsupported model: %s", ToString(model_type).c_str());
  EDGEVOX_ONNX_EXIT(-1);
...
WhisperModelType ParseWhisperModelType(const std::string &name) {
  ...
  EDGEVOX_ONNX_LOGE("Unknown Whisper model: '%s'", name.c_str());
  EDGEVOX_ONNX_EXIT(-1);
