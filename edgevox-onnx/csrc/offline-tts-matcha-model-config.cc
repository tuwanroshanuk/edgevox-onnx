// edgevox-onnx/csrc/offline-tts-matcha-model-config.cc
//
...
bool OfflineTtsMatchaModelConfig::Validate() const {
  if (acoustic_model.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --matcha-acoustic-model");
    return false;
  }

  if (!FileExists(acoustic_model)) {
    EDGEVOX_ONNX_LOGE("--matcha-acoustic-model: '%s' does not exist",
                     acoustic_model.c_str());
    return false;
  }

  if (tokens.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --matcha-tokens");
    return false;
  }

  if (!FileExists(tokens)) {
    EDGEVOX_ONNX_LOGE("--matcha-tokens: '%s' does not exist", tokens.c_str());
    return false;
  }
