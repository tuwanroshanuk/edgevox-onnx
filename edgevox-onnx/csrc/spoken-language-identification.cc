// edgevox-onnx/csrc/spoken-language-identification.cc
//
...
bool SpokenLanguageIdentificationWhisperConfig::Validate() const {
  ...
  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("whisper encoder file '%s' does not exist",
                     encoder.c_str());
    return false;
  }
  ...
  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("whisper decoder file '%s' does not exist",
                     decoder.c_str());
    return false;
  }
