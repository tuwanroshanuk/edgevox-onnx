// edgevox-onnx/csrc/phrase-matcher.cc
//
...
class PhraseMatcher::Impl {
 public:
  Impl(const std::unordered_set<std::string> *lexicon,
    ...
    if (debug_) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("max_search_len %{public}d", max_search_len_);
#else
      EDGEVOX_ONNX_LOGE("max_search_len %d", max_search_len_);
#endif
    }
    ...
    for (int32_t i = 0; i < num_words;) {
      ...
      if (!IsAlphaOrPunct(words[i].front())) {
        ...
        while (end > start) {
          ...
          if (debug_) {
#if __OHOS__
            EDGEVOX_ONNX_LOGE("%{public}d-%{public}d: %{public}s", start, end,
                             this_word.c_str());
#else
            EDGEVOX_ONNX_LOGE("%d-%d: %s", start, end, this_word.c_str());
#endif
          }
          if (lexicon_->count(this_word)) {
            ...
            if (debug_) {
#if __OHOS__
              EDGEVOX_ONNX_LOGE("matched %{public}d-%{public}d: %{public}s",
                               start, end, w.c_str());
#else
              EDGEVOX_ONNX_LOGE("matched %d-%d: %s", start, end, w.c_str());
#endif
            }
            break;
          }
      ...
      if (w.empty()) {
        ...
        if (debug_) {
#if __OHOS__
          EDGEVOX_ONNX_LOGE("single word %{public}d-%{public}d: %{public}s", i,
                           i, w.c_str());
#else
          EDGEVOX_ONNX_LOGE("single word %d-%d: %s", i, i, w.c_str());
#endif
        }
