// edgevox-onnx/csrc/offline-tts-pocket-model.cc
//
...
static Ort::Value CreateZeroTensorLike(Ort::Session &sess, int32_t input_index,
  ...
  switch (elem_type) {
    ...
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64:
      ...
      break;
    default:
      EDGEVOX_ONNX_LOGE("Unsupported tensor element type: %d", elem_type);
      EDGEVOX_ONNX_EXIT(-1);
  }
 ...
 public:
  explicit Impl(const OfflineTtsModelConfig &config)
    ...
    lm_flow_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.pocket.lm_flow), sess_opts_);
    InitLmFlow(nullptr, 0);

    lm_main_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.pocket.lm_main), sess_opts_);
    InitLmMain(nullptr, 0);

    mimi_encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.pocket.encoder), sess_opts_);
    InitMimiEncoder(nullptr, 0);

    mimi_decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.pocket.decoder), sess_opts_);
    InitMimiDecoder(nullptr, 0);

    text_conditioner_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.pocket.text_conditioner),
        sess_opts_);
