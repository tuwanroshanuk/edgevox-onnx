// edgevox-onnx/csrc/offline-qwen3-asr-model-config.cc
//
// Copyright (c)  2026  zengyw

#include "edgevox-onnx/csrc/offline-qwen3-asr-model-config.h"
...
bool OfflineQwen3ASRModelConfig::Validate() const {
  if (conv_frontend.empty()) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-conv-frontend is required");
    return false;
  }

  if (!FileExists(conv_frontend)) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-conv-frontend: '%s' does not exist",
                     conv_frontend.c_str());
    return false;
  }

  if (encoder.empty()) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-encoder is required");
    return false;
  }

  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-encoder: '%s' does not exist",
                     encoder.c_str());
    return false;
  }

  if (decoder.empty()) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-decoder is required");
    return false;
  }

  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-decoder: '%s' does not exist",
                     decoder.c_str());
    return false;
  }

  if (tokenizer.empty()) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-tokenizer is required");
    return false;
  }
  ...
  if (max_total_len <= 0) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-max-total-len should be > 0. Given: %d",
                     max_total_len);
    return false;
  }

  if (max_new_tokens <= 0) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-max-new-tokens should be > 0. Given: %d",
                     max_new_tokens);
    return false;
  }

  if (temperature < 0.0f) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-temperature should be >= 0.0. Given: %f",
                     temperature);
    return false;
  }

  if (top_p < 0.0f || top_p > 1.0f) {
    EDGEVOX_ONNX_LOGE("--qwen3-asr-top-p should be in [0.0, 1.0]. Given: %f",
                     top_p);
    return false;
  }
