// edgevox-onnx/csrc/online-ebranchformer-transducer-model.cc
//
// Copyright (c)  2023  Xiaomi Corporation
//                2025  Brno University of Technology (author: Karel Vesely)

#include "edgevox-onnx/csrc/online-ebranchformer-transducer-model.h"
...
#include "edgevox-onnx/csrc/cat.h"
...
#include "edgevox-onnx/csrc/unbind.h"
...
OnlineEbranchformerTransducerModel::OnlineEbranchformerTransducerModel(
  ...
  encoder_sess_ = std::make_unique<Ort::Session>(
      env_, EDGEVOX_ONNX_TO_ORT_PATH(config.transducer.encoder), encoder_sess_opts_);
  InitEncoder(nullptr, 0);

  decoder_sess_ = std::make_unique<Ort::Session>(
      env_, EDGEVOX_ONNX_TO_ORT_PATH(config.transducer.decoder), decoder_sess_opts_);
  InitDecoder(nullptr, 0);

  joiner_sess_ = std::make_unique<Ort::Session>(
      env_, EDGEVOX_ONNX_TO_ORT_PATH(config.transducer.joiner), joiner_sess_opts_);
...
void OnlineEbranchformerTransducerModel::InitEncoder(void *model_data,
  ...
  EDGEVOX_ONNX_READ_META_DATA(decode_chunk_len_, "decode_chunk_len");
  EDGEVOX_ONNX_READ_META_DATA(T_, "T");

  EDGEVOX_ONNX_READ_META_DATA(num_hidden_layers_, "num_hidden_layers");
  EDGEVOX_ONNX_READ_META_DATA(hidden_size_, "hidden_size");
  EDGEVOX_ONNX_READ_META_DATA(intermediate_size_, "intermediate_size");
  EDGEVOX_ONNX_READ_META_DATA(csgu_kernel_size_, "csgu_kernel_size");
  EDGEVOX_ONNX_READ_META_DATA(merge_conv_kernel_, "merge_conv_kernel");
  EDGEVOX_ONNX_READ_META_DATA(left_context_len_, "left_context_len");
  EDGEVOX_ONNX_READ_META_DATA(num_heads_, "num_heads");
  EDGEVOX_ONNX_READ_META_DATA(head_dim_, "head_dim");

  if (config_.debug) {
#if __OHOS__
    EDGEVOX_ONNX_LOGE("T: %{public}d", T_);
    EDGEVOX_ONNX_LOGE("decode_chunk_len_: %{public}d", decode_chunk_len_);

    EDGEVOX_ONNX_LOGE("num_hidden_layers_: %{public}d", num_hidden_layers_);
    EDGEVOX_ONNX_LOGE("hidden_size_: %{public}d", hidden_size_);
    EDGEVOX_ONNX_LOGE("intermediate_size_: %{public}d", intermediate_size_);
    EDGEVOX_ONNX_LOGE("csgu_kernel_size_: %{public}d", csgu_kernel_size_);
    EDGEVOX_ONNX_LOGE("merge_conv_kernel_: %{public}d", merge_conv_kernel_);
    EDGEVOX_ONNX_LOGE("left_context_len_: %{public}d", left_context_len_);
    EDGEVOX_ONNX_LOGE("num_heads_: %{public}d", num_heads_);
    EDGEVOX_ONNX_LOGE("head_dim_: %{public}d", head_dim_);
#else
    EDGEVOX_ONNX_LOGE("T: %d", T_);
    EDGEVOX_ONNX_LOGE("decode_chunk_len_: %d", decode_chunk_len_);

    EDGEVOX_ONNX_LOGE("num_hidden_layers_: %d", num_hidden_layers_);
    EDGEVOX_ONNX_LOGE("hidden_size_: %d", hidden_size_);
    EDGEVOX_ONNX_LOGE("intermediate_size_: %d", intermediate_size_);
    EDGEVOX_ONNX_LOGE("csgu_kernel_size_: %d", csgu_kernel_size_);
    EDGEVOX_ONNX_LOGE("merge_conv_kernel_: %d", merge_conv_kernel_);
    EDGEVOX_ONNX_LOGE("left_context_len_: %d", left_context_len_);
    EDGEVOX_ONNX_LOGE("num_heads_: %d", num_heads_);
    EDGEVOX_ONNX_LOGE("head_dim_: %d", head_dim_);
#endif
  }
}
