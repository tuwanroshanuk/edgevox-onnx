// edgevox-onnx/csrc/ten-vad-model.cc
//
 ...
 public:
  explicit Impl(const VadModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.ten_vad.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if (config_.ten_vad.window_size > 768) {
      EDGEVOX_ONNX_LOGE("Windows size %d for ten-vad is too large",
  ...
  void Check() {
    ...
    if (model_type != "ten-vad") {
      EDGEVOX_ONNX_LOGE("Expect model type 'ten-vad', given '%s'",
                       model_type.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(mean_, "mean");
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(inv_stddev_, "inv_stddev");
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(window_, "window");
