// edgevox-onnx/csrc/edgevox-onnx-online-denoiser.cc
//
...
Streaming speech denoising with edgevox-onnx.
...
./bin/edgevox-onnx-online-denoiser \
...
./bin/edgevox-onnx-online-denoiser \
  ...
  edgevox_onnx::OnlineSpeechDenoiserConfig config;
  ...
  std::vector<float> samples =
      edgevox_onnx::ReadWave(input_wave, &sampling_rate, &is_ok);
  if (!is_ok) {
  ...
  edgevox_onnx::OnlineSpeechDenoiser denoiser(config);
  ...
  fprintf(stderr, "Done\n");
  is_ok = edgevox_onnx::WriteWave(output_wave, denoiser.GetSampleRate(),
