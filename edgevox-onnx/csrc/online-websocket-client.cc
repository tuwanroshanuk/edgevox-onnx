...
Automatic speech recognition with edgevox-onnx using websocket.

Usage:

./bin/edgevox-onnx-online-websocket-client --help

./bin/edgevox-onnx-online-websocket-client \
  --server-ip=127.0.0.1 \
  --server-port=6006 \
...
class Client {
 public:
  Client(asio::io_context &io,  // NOLINT
    ...
    c_.set_close_handler(
        [](connection_hdl /*hdl*/) { EDGEVOX_ONNX_LOGE("Disconnected"); });
 ...
 private:
  void Run() {
    ...
    if (ec) {
      EDGEVOX_ONNX_LOGE("Could not create connection to %s because %s",
  ...
  void OnMessage(connection_hdl hdl, message_ptr msg) {
    ...
    if (payload == "Done!") {
      ...
      if (ec) {
        EDGEVOX_ONNX_LOGE("Failed to close because %s", ec.message().c_str());
        EDGEVOX_ONNX_EXIT(EXIT_FAILURE);
      }
    } else {
      EDGEVOX_ONNX_LOGE("%s", payload.c_str());
    }
  }
  ...
  void SendMessage(
    ...
    if (num_sent_messages_ < 1) {
      EDGEVOX_ONNX_LOGE("Starting to send audio");
    }
    ...
    if (num_sent_messages_ < num_messages) {
      ...
      if (ec) {
        EDGEVOX_ONNX_LOGE("Failed to send audio samples because %s",
                         ec.message().c_str());
        EDGEVOX_ONNX_EXIT(EXIT_FAILURE);
      }
    ...
    if (num_sent_messages_ == num_messages) {
      ...
      EDGEVOX_ONNX_LOGE("Sent Done Signal");
...
int32_t main(int32_t argc, char *argv[]) {
  ...
  if (!websocketpp::uri_helper::ipv4_literal(server_ip.begin(),
                                             server_ip.end())) {
    EDGEVOX_ONNX_LOGE("Invalid server IP: %s", server_ip.c_str());
    return -1;
  }

  if (server_port <= 0 || server_port > 65535) {
    EDGEVOX_ONNX_LOGE("Invalid server port: %d", server_port);
    return -1;
  }

  // 0.01 is an arbitrary value. You can change it.
  if (samples_per_message <= 0.01 * sample_rate) {
    EDGEVOX_ONNX_LOGE("--samples-per-message is too small: %d",
                     samples_per_message);
    return -1;
  }
  ...
  if (seconds_per_message < 0) {
    EDGEVOX_ONNX_LOGE("--seconds-per-message is too small: %.3f",
                     seconds_per_message);
    return -1;
  }
  ...
  std::vector<float> samples =
      edgevox_onnx::ReadWave(wave_filename, &actual_sample_rate, &is_ok);

  if (!is_ok) {
    EDGEVOX_ONNX_LOGE("Failed to read '%s'", wave_filename.c_str());
    return -1;
  }

  if (actual_sample_rate != sample_rate) {
    EDGEVOX_ONNX_LOGE("Expected sample rate: %d, given %d", sample_rate,
                     actual_sample_rate);
    return -1;
  }
