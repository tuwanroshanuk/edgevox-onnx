// edgevox-onnx/csrc/edgevox-onnx-tts-play-alsa.cc
//
...
#include "edgevox-onnx/csrc/alsa-play.h"
...
static void StartPlayback(const std::string &device_name, int32_t sample_rate) {
  edgevox_onnx::AlsaPlay alsa(device_name.c_str(), sample_rate);
...
Offline text-to-speech with edgevox-onnx.
...
./bin/edgevox-onnx-offline-tts-play-alsa \
...
or details.
  ...
  edgevox_onnx::GenerationConfig gen_config;
  std::string lang;
  ...
  edgevox_onnx::OfflineTtsConfig config;
  ...
  edgevox_onnx::OfflineTts tts(config);
  ...
  if (is_pocket_tts || is_zipvoice_tts) {
    ...
    bool is_ok = false;
    auto samples =
        edgevox_onnx::ReadWave(reference_audio, &sample_rate, &is_ok);
    if (!is_ok) {
  ...
  bool ok = edgevox_onnx::WriteWave(output_filename, audio.sample_rate,
