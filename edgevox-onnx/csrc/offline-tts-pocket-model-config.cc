// edgevox-onnx/csrc/offline-tts-pocket-model-config.cc
//
// Copyright (c)  2026  Xiaomi Corporation

#include "edgevox-onnx/csrc/offline-tts-pocket-model-config.h"

#include <sstream>
#include <string>

#include "edgevox-onnx/csrc/file-utils.h"
#include "edgevox-onnx/csrc/macros.h"

namespace edgevox_onnx {

void OfflineTtsPocketModelConfig::Register(ParseOptions *po) {
  po->Register("pocket-lm-flow", &lm_flow, "Path to PocketTTS lm flow model");
  po->Register("pocket-lm-main", &lm_main, "Path to PocketTTS lm main model");
  po->Register("pocket-encoder", &encoder, "Path to PocketTTS encoder model");
  po->Register("pocket-decoder", &decoder, "Path to PocketTTS decoder model");
  po->Register("pocket-text-conditioner", &text_conditioner,
               "Path to PocketTTS text conditioner model");
  po->Register("pocket-vocab-json", &vocab_json,
               "Path to PocketTTS vocab.json");
  po->Register("pocket-token-scores-json", &token_scores_json,
               "Path to PocketTTS token_scores.json");
  po->Register("pocket-voice-embedding-cache-capacity",
               &voice_embedding_cache_capacity,
               "Capacity of the voice embedding cache (number of items). "
               "Default: 50. 0 disables caching.");
}

bool OfflineTtsPocketModelConfig::Validate() const {
  if (lm_flow.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --pocket-lm-flow");
    return false;
  }

  if (!FileExists(lm_flow)) {
    EDGEVOX_ONNX_LOGE("--pocket-lm-flow '%s' does not exist", lm_flow.c_str());
    return false;
  }

  if (lm_main.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --pocket-lm-main");
    return false;
  }

  if (!FileExists(lm_main)) {
    EDGEVOX_ONNX_LOGE("--pocket-lm-main '%s' does not exist", lm_main.c_str());
    return false;
  }

  if (encoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --pocket-encoder");
    return false;
  }

  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("--pocket-encoder '%s' does not exist", encoder.c_str());
    return false;
  }

  if (decoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --pocket-decoder");
    return false;
  }

  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("--pocket-decoder '%s' does not exist", decoder.c_str());
    return false;
  }

  if (text_conditioner.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --pocket-text-conditioner");
    return false;
  }

  if (!FileExists(text_conditioner)) {
    EDGEVOX_ONNX_LOGE("--pocket-text-conditioner '%s' does not exist",
                     text_conditioner.c_str());
    return false;
  }

  if (vocab_json.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --pocket-vocab-json");
    return false;
  }

  if (!FileExists(vocab_json)) {
    EDGEVOX_ONNX_LOGE("--pocket-vocab-json '%s' does not exist",
                     vocab_json.c_str());
    return false;
  }

  if (token_scores_json.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --pocket-token-scores-json");
    return false;
  }

  if (!FileExists(token_scores_json)) {
    EDGEVOX_ONNX_LOGE("--pocket-token-scores-json '%s' does not exist",
                     token_scores_json.c_str());
    return false;
  }

  if (voice_embedding_cache_capacity < 0) {
    EDGEVOX_ONNX_LOGE(
        "voice_embedding_cache_capacity must be non-negative. Given: %d",
        voice_embedding_cache_capacity);
    return false;
  }

  return true;
}

std::string OfflineTtsPocketModelConfig::ToString() const {
  std::ostringstream os;

  os << "OfflineTtsPocketModelConfig(";
  os << "lm_flow=\"" << lm_flow << "\", ";
  os << "lm_main=\"" << lm_main << "\", ";
  os << "encoder=\"" << encoder << "\", ";
  os << "decoder=\"" << decoder << "\", ";
  os << "text_conditioner=\"" << text_conditioner << "\", ";
  os << "vocab_json=\"" << vocab_json << "\", ";
  os << "token_scores_json=\"" << token_scores_json << "\", ";
  os << "voice_embedding_cache_capacity=" << voice_embedding_cache_capacity
     << ")";

  return os.str();
}

}  // namespace edgevox_onnx
