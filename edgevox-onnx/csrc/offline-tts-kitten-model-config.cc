// edgevox-onnx/csrc/offline-tts-kitten-model-config.cc
//
...
bool OfflineTtsKittenModelConfig::Validate() const {
  if (model.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --kitten-model");
    return false;
  }

  if (!FileExists(model)) {
    EDGEVOX_ONNX_LOGE("--kitten-model: '%s' does not exist", model.c_str());
    return false;
  }

  if (voices.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --kitten-voices");
    return false;
  }

  if (!FileExists(voices)) {
    EDGEVOX_ONNX_LOGE("--kitten-voices: '%s' does not exist", voices.c_str());
    return false;
  }

  if (tokens.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --kitten-tokens");
    return false;
  }

  if (!FileExists(tokens)) {
    EDGEVOX_ONNX_LOGE("--kitten-tokens: '%s' does not exist", tokens.c_str());
    return false;
  }

  if (data_dir.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --kitten-data-dir");
    return false;
  }
