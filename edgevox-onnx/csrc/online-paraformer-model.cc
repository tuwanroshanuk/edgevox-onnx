// edgevox-onnx/csrc/online-paraformer-model.cc
//
// Copyright (c)  2022-2023  Xiaomi Corporation

#include "edgevox-onnx/csrc/online-paraformer-model.h"
...
class OnlineParaformerModel::Impl {
 public:
  explicit Impl(const OnlineModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.paraformer.encoder), sess_opts_);
    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.paraformer.decoder), sess_opts_);
 ...
 private:
  void InitEncoder(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA(encoder_output_size_, "encoder_output_size");
    EDGEVOX_ONNX_READ_META_DATA(decoder_num_blocks_, "decoder_num_blocks");
    EDGEVOX_ONNX_READ_META_DATA(decoder_kernel_size_, "decoder_kernel_size");
