// edgevox-onnx/csrc/onnx-utils.cc
//
...
Ort::Value Clone(OrtAllocator *allocator, const Ort::Value *v) {
  ...
  switch (type_and_shape.GetElementType()) {
    ...
    default:
      EDGEVOX_ONNX_LOGE("Unsupported type: %d\n",
...
Ort::Value View(Ort::Value *v) {
  ...
  if (status != nullptr) {
    ...
    EDGEVOX_ONNX_LOGE("Failed to get tensor memory info with error: '%s'", msg);
    EDGEVOX_ONNX_EXIT(-1);
  }
#endif
