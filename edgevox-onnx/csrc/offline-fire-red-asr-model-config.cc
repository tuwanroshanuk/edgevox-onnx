// edgevox-onnx/csrc/offline-fire-red-asr-model-config.cc
//
...
bool OfflineFireRedAsrModelConfig::Validate() const {
  if (encoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --fire-red-asr-encoder");
    return false;
  }

  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("FireRedAsr encoder file '%s' does not exist",
                     encoder.c_str());
    return false;
  }

  if (decoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --fire-red-asr-decoder");
    return false;
  }

  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("FireRedAsr decoder file '%s' does not exist",
                     decoder.c_str());
    return false;
  }
