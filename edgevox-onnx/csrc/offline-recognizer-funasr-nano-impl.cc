// edgevox-onnx/csrc/offline-recognizer-funasr-nano-impl.cc
//
...
OfflineRecognitionResult OfflineRecognizerFunASRNanoImpl::GenerateText(
  ...
  if (max_seq_len <= 0) {
    EDGEVOX_ONNX_LOGE("Invalid max_seq_len=%d", max_seq_len);
    result.text = "";
    return result;
  }
  ...
  if (copy_len > 0 && fbank_beg_idx >= 0) {
    if (enc_et == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16) {
      ...
      EDGEVOX_ONNX_LOGE("encoder_out elem_type=%d not supported", (int)enc_et);
      result.text = "";
      return result;
    }
  }
  ...
  for (int32_t step = 0; step < max_new_tokens; ++step) {
    ...
    if (log_shape.size() < 3) {
      EDGEVOX_ONNX_LOGE("Unexpected logits rank=%zu", log_shape.size());
      result.text = "";
      return result;
    }
    ...
    if (time_dim <= 0 || vocab_size <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid logits shape [%d,%d,%d]",
  ...
  if (config_.model_config.debug) {
    EDGEVOX_ONNX_LOGE("GenerateText: generated %zu tokens: %s",
    ...
    EDGEVOX_ONNX_LOGE("GenerateText: token ids: %s%s", token_str.c_str(),
...
void OfflineRecognizerFunASRNanoImpl::DecodeStreams(OfflineStream **ss,
  ...
  for (int32_t i = 0; i != n; ++i) {
    ...
    if (config_.model_config.debug) {
      ...
      EDGEVOX_ONNX_LOGE("DecodeStreams: user_prompt_dyn=%s",
