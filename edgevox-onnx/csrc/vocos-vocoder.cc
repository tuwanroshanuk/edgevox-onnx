// edgevox-onnx/csrc/vocos-vocoder.cc
//
// Copyright (c)  2025  Xiaomi Corporation

#include "edgevox-onnx/csrc/vocos-vocoder.h"
...
class VocosVocoder::Impl {
  ...
  explicit Impl(const OfflineTtsModelConfig &config)
    ...
    if (!config.matcha.vocoder.empty()) {
      sess_ = std::make_unique<Ort::Session>(
          env_, EDGEVOX_ONNX_TO_ORT_PATH(config.matcha.vocoder), sess_opts_);
    } else if (!config.zipvoice.vocoder.empty()) {
      sess_ = std::make_unique<Ort::Session>(
          env_, EDGEVOX_ONNX_TO_ORT_PATH(config.zipvoice.vocoder), sess_opts_);
    } else {
      EDGEVOX_ONNX_LOGE("No vocoder model provided in the config!");
      EDGEVOX_ONNX_EXIT(-1);
    }
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.n_fft, "n_fft", 1024);
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.hop_length, "hop_length",
                                            256);
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.win_length, "win_length",
                                            1024);
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.center, "center", 1);
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.normalized, "normalized", 0);
    EDGEVOX_ONNX_READ_META_DATA_STR_WITH_DEFAULT(meta_.window_type,
                                                "window_type", "hann");
    EDGEVOX_ONNX_READ_META_DATA_STR_WITH_DEFAULT(meta_.pad_mode, "pad_mode",
                                                "reflect");
  }
