// edgevox-onnx/csrc/funasr-nano-tokenizer.cc
//
...
void FunASRNanoTokenizer::Init(const std::string &tokenizer_dir) {
  ...
  if (tok_json.empty()) {
    EDGEVOX_ONNX_LOGE("Cannot find tokenizer.json in: %s",
                     tokenizer_dir.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (!ParseAddedTokensFromTokenizerJson(tok_blob, &added_tokens_)) {
    EDGEVOX_ONNX_LOGE("Failed to parse added_tokens from tokenizer.json: %s",
                     tok_json.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
...
std::vector<int64_t> FunASRNanoTokenizer::Encode(const std::string &text) {
  if (token2id_.empty()) {
    EDGEVOX_ONNX_LOGE("Tokenizer not initialized");
    EDGEVOX_ONNX_EXIT(-1);
  }
