// edgevox-onnx/csrc/qwen-asr-tokenizer.cc
//
...
void QwenAsrTokenizer::InitFromContents(const std::string &vocab_content,
  ...
  if (vocab_content.empty()) {
    EDGEVOX_ONNX_LOGE("Failed to read vocab.json from: %s",
                     tokenizer_dir.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (!ParseVocab(vocab_content, &token2id_, &id2token_)) {
    EDGEVOX_ONNX_LOGE("Failed to parse vocab.json from: %s",
                     tokenizer_dir.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }

  if (!merges_content.empty() && !ParseMerges(merges_content, &merges_rank_)) {
    EDGEVOX_ONNX_LOGE("Failed to parse merges.txt from: %s",
                     tokenizer_dir.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
