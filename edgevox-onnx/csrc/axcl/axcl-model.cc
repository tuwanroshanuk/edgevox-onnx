// edgevox-onnx/csrc/axcl/axcl-model.cc
//
// Copyright (c)  2025  Xiaomi Corporation

#include "edgevox-onnx/csrc/axcl/axcl-model.h"
...
#include "edgevox-onnx/csrc/axcl/utils.h"
...
class AxclModel::Impl {
 public:
  ...
  Impl(const void *cpu_buf, size_t buf_len_in_bytes, int32_t device_id) {
...

    {
      ...
      if (ret != 0) {
        EDGEVOX_ONNX_LOGE("Failed to call axclrtMemcpy(). Return code is: %d",
  ...
  std::vector<int32_t> TensorShape(const std::string &name) const {
    ...
    EDGEVOX_ONNX_LOGE("Found no tensor with name: '%s'", name.c_str());
    return {};
  }
    ...
    for (size_t i = 0; i < input_tensor_names_.size(); ++i) {
      if (input_tensor_names_[i] == name) {
        if (n * sizeof(T) != input_tensors_[i].Size()) {
          EDGEVOX_ONNX_LOGE("Expected size: %zu, given: %zu",
  ...
  bool Run() const {
    ...
    if (ret != 0) {
      EDGEVOX_ONNX_LOGE("Failed to call axclrtEngineExecute(), return code: %d",
                       static_cast<int32_t>(ret));
      return false;
    }
    return true;
 ...
 private:
  bool SetDevice(int32_t device_id) {
    ...
    if (lst.num == 0) {
      EDGEVOX_ONNX_LOGE("Found 0 device.");
      return false;
    }

    // device_id counts from 0
    if (device_id < 0 || device_id >= lst.num) {
      EDGEVOX_ONNX_LOGE("Invalid device_id: %d. Valid range: 0-%d", device_id,
                       lst.num - 1);
      return false;
    }
    ...
    if (ret != 0) {
      EDGEVOX_ONNX_LOGE("Failed to call axclrtSetDevice(). Return code is: %d",
                       static_cast<int32_t>(ret));
      return false;
    }
  ...
  void PostInit() {
    ...
    if (count != 1) {
      EDGEVOX_ONNX_LOGE("Only support 1 group at present. Given: %d", count);
      EDGEVOX_ONNX_EXIT(-1);
    }
