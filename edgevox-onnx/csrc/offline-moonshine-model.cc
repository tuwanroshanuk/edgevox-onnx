// edgevox-onnx/csrc/offline-moonshine-model.cc
//
 ...
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    preprocessor_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.moonshine.preprocessor),
        sess_opts_);
    ...
    uncached_decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.moonshine.uncached_decoder),
        sess_opts_);
    InitUnCachedDecoder(nullptr, 0);

    cached_decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.moonshine.cached_decoder),
        sess_opts_);
