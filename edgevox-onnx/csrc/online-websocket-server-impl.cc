// edgevox-onnx/csrc/online-websocket-server-impl.cc
//
// Copyright (c)  2022-2023  Xiaomi Corporation

#include "edgevox-onnx/csrc/online-websocket-server-impl.h"
...
void OnlineWebsocketDecoderConfig::Validate() const {
  ...
  EDGEVOX_ONNX_CHECK_GT(loop_interval_ms, 0);
  EDGEVOX_ONNX_CHECK_GT(max_batch_size, 0);
  EDGEVOX_ONNX_CHECK_GT(end_tail_padding, 0);
}

void OnlineWebsocketServerConfig::Register(edgevox_onnx::ParseOptions *po) {
...
void OnlineWebsocketDecoder::ProcessConnections(const asio::error_code &ec) {
  if (ec) {
    EDGEVOX_ONNX_LOG(FATAL) << "The decoder loop is aborted!";
  }
...
void OnlineWebsocketServer::Run(uint16_t port) {
  ...
  if (0 < warm_up && warm_up < 100) {
    if (model_type == "zipformer2") {
      decoder_.Warmup();
      EDGEVOX_ONNX_LOGE("Warm up completed : %d times.", warm_up);
    } else {
      EDGEVOX_ONNX_LOGE("Only Zipformer2 has warmup support for now.");
      EDGEVOX_ONNX_LOGE("Given: %s", model_type.c_str());
      EDGEVOX_ONNX_EXIT(0);
    }
  } else if (warm_up == 0) {
    EDGEVOX_ONNX_LOGE("Starting without warmup!");
  } else {
    EDGEVOX_ONNX_LOGE("Invalid Warm up Value!. Expected 0 < warm_up < 100");
    EDGEVOX_ONNX_EXIT(0);
  }
  decoder_.Run();
}
...
void OnlineWebsocketServer::OnClose(connection_hdl hdl) {
  ...
  EDGEVOX_ONNX_LOG(INFO) << "Number of active connections: "
                        << connections_.size() << "\n";
}
