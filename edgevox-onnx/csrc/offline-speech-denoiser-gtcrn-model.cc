// edgevox-onnx/csrc/offline-speech-denoiser-gtcrn-model.cc
//
...
class OfflineSpeechDenoiserGtcrnModel::Impl {
 public:
  explicit Impl(const OfflineSpeechDenoiserModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.gtcrn.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if (model_type != "gtcrn") {
      EDGEVOX_ONNX_LOGE("Expect model type 'gtcrn'. Given: '%s'",
                       model_type.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    EDGEVOX_ONNX_READ_META_DATA(meta_.version, "version");

    EDGEVOX_ONNX_READ_META_DATA_VEC(meta_.conv_cache_shape, "conv_cache_shape");
    EDGEVOX_ONNX_READ_META_DATA_VEC(meta_.tra_cache_shape, "tra_cache_shape");
    EDGEVOX_ONNX_READ_META_DATA_VEC(meta_.inter_cache_shape,
                                   "inter_cache_shape");
  }
