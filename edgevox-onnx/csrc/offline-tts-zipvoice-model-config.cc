// edgevox-onnx/csrc/offline-tts-zipvoice-model-config.cc
//
...
bool OfflineTtsZipvoiceModelConfig::Validate() const {
  if (tokens.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --zipvoice-tokens");
    return false;
  }
  if (!FileExists(tokens)) {
    EDGEVOX_ONNX_LOGE("--zipvoice-tokens: '%s' does not exist", tokens.c_str());
    return false;
  }

  if (encoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --zipvoice-encoder");
    return false;
  }
  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("--zipvoice-encoder: '%s' does not exist",
                     encoder.c_str());
    return false;
  }

  if (decoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --zipvoice-decoder");
    return false;
  }
  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("--zipvoice-decoder: '%s' does not exist",
                     decoder.c_str());
    return false;
  }

  if (vocoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --zipvoice-vocoder");
    return false;
  }

  if (!FileExists(vocoder)) {
    EDGEVOX_ONNX_LOGE("--zipvoice-vocoder: '%s' does not exist",
                     vocoder.c_str());
    return false;
  }
  ...
  if (feat_scale <= 0) {
    EDGEVOX_ONNX_LOGE("--zipvoice-feat-scale must be positive. Given: %f",
                     feat_scale);
    return false;
  }

  if (t_shift < 0) {
    EDGEVOX_ONNX_LOGE("--zipvoice-t-shift must be non-negative. Given: %f",
                     t_shift);
    return false;
  }

  if (target_rms <= 0) {
    EDGEVOX_ONNX_LOGE("--zipvoice-target-rms must be positive. Given: %f",
                     target_rms);
    return false;
  }

  if (guidance_scale <= 0) {
    EDGEVOX_ONNX_LOGE("--zipvoice-guidance-scale must be positive. Given: %f",
                     guidance_scale);
    return false;
  }
