// edgevox-onnx/csrc/edgevox-onnx-offline-source-separation.cc
//
...
./bin/edgevox-onnx-offline-source-separation \
  ...
  edgevox_onnx::OfflineSourceSeparationConfig config;
  ...
  input.samples.data =
      edgevox_onnx::ReadWaveMultiChannel(input_wave, &input.sample_rate, &is_ok);
  if (!is_ok) {
  ...
  edgevox_onnx::OfflineSourceSeparation sp(config);
  ...
  is_ok = edgevox_onnx::WriteWave(output_accompaniment_wave, output.sample_rate,
