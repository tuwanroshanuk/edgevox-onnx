// edgevox-onnx/csrc/offline-recognizer-transducer-nemo-impl.h
//
// Copyright (c)  2022-2024  Xiaomi Corporation

#ifndef EDGEVOX_ONNX_CSRC_OFFLINE_RECOGNIZER_TRANSDUCER_NEMO_IMPL_H_
#define EDGEVOX_ONNX_CSRC_OFFLINE_RECOGNIZER_TRANSDUCER_NEMO_IMPL_H_
...
#include "edgevox-onnx/csrc/offline-transducer-greedy-search-nemo-decoder.h"
#include "edgevox-onnx/csrc/offline-transducer-modified-beam-search-nemo-decoder.h"
...
#include "edgevox-onnx/csrc/utils.h"
 ...
 public:
  explicit OfflineRecognizerTransducerNeMoImpl(
    ...
    if (config_.decoding_method == "greedy_search") {
      ...
      EDGEVOX_ONNX_LOGE("Unsupported decoding method: %s",
  ...
  std::unique_ptr<OfflineStream> CreateStream(
    ...
    if (!EncodeHotwords(is, config_.model_config.modeling_unit, symbol_table_,
      ...
      EDGEVOX_ONNX_LOGE("Encode hotwords failed, skipping, hotwords are : '%s'",
                       hotwords.c_str());
    }
 ...
 private:
  void PostInit() {
    ...
    if (!symbol_table_.Contains("<blk>")) {
      EDGEVOX_ONNX_LOGE("tokens.txt does not include the blank token <blk>");
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (symbol_table_["<blk>"] != vocab_size - 1) {
      EDGEVOX_ONNX_LOGE("<blk> is not the last token!");
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (symbol_table_.NumSymbols() != vocab_size) {
      EDGEVOX_ONNX_LOGE("number of lines in tokens.txt %d != %d (vocab_size)",
  ...
  void InitHotwords() {
    ...
    if (!is) {
      EDGEVOX_ONNX_LOGE("Open hotwords file failed: '%s'",
...
#endif  // EDGEVOX_ONNX_CSRC_OFFLINE_RECOGNIZER_TRANSDUCER_NEMO_IMPL_H_
