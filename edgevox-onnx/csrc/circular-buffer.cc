// edgevox-onnx/csrc/circular-buffer.cc
//
...
CircularBuffer::CircularBuffer(int32_t capacity) {
  if (capacity <= 0) {
    EDGEVOX_ONNX_LOGE("Please specify a positive capacity. Given: %d\n",
                     capacity);
    EDGEVOX_ONNX_EXIT(-1);
  }
...
#else
    EDGEVOX_ONNX_LOGE("new_capacity (%d) <= original capacity (%d). Skip it.",
                     new_capacity, capacity);
#endif
    return;
  }
...
std::vector<float> CircularBuffer::Get(int32_t start_index, int32_t n) const {
  if (start_index < head_ || start_index >= tail_) {
    EDGEVOX_ONNX_LOGE("Invalid start_index: %d. head_: %d, tail_: %d",
  ...
  int32_t size = Size();
  if (n < 0 || n > size) {
    EDGEVOX_ONNX_LOGE("Invalid n: %d. size: %d", n, size);
    return {};
  }
  ...
  if (start_index - head_ + n > size) {
    EDGEVOX_ONNX_LOGE("Invalid start_index: %d and n: %d. head_: %d, size: %d",
