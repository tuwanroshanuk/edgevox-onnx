// edgevox-onnx/csrc/on-rnn-lm.cc
//
...
#include "edgevox-onnx/csrc/lodr-fst.h"
 ...
 private:
  void Init(const OnlineLMConfig &config) {
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.model), sess_opts_);
    ...
    EDGEVOX_ONNX_READ_META_DATA(rnn_num_layers_, "num_layers");
    EDGEVOX_ONNX_READ_META_DATA(rnn_hidden_size_, "hidden_size");
    EDGEVOX_ONNX_READ_META_DATA(sos_id_, "sos_id");
