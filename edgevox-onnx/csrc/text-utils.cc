// edgevox-onnx/csrc/text-utils.cc
//
...
std::vector<std::string> SplitUtf8(const std::string &text) {
  ...
  while (start < end) {
    ...
    if (num_bytes == 0) {
      ...
      EDGEVOX_ONNX_LOGE("Invalid byte at position: %d",
...
std::string RemoveInvalidUtf8Sequences(const std::string &text,
  ...
  while (i < n) {
    ...
    if (show_debug_msg) {
      EDGEVOX_ONNX_LOGE("Ignore invalid utf8 sequence at pos: %d, value: %02x",
                       i, p[i]);
    }
...
std::string Gb2312ToUtf8(const std::string &text) {
  ...
  EDGEVOX_ONNX_LOGE("num of wchars: %d", num_wchars);
  if (num_wchars == 0) {
    return {};
  }
...
void LengthsToMask(const std::vector<int64_t> &lengths,
  ...
  if (max_len < 0) {
    EDGEVOX_ONNX_LOGE("LengthsToMask: max_len (%" PRId64 ") < 0", max_len);
    EDGEVOX_ONNX_EXIT(-1);
  }
