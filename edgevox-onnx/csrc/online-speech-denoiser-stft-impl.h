// edgevox-onnx/csrc/online-speech-denoiser-stft-impl.h
//
// Copyright (c)  2026  Xiaomi Corporation

#ifndef EDGEVOX_ONNX_CSRC_ONLINE_SPEECH_DENOISER_STFT_IMPL_H_
#define EDGEVOX_ONNX_CSRC_ONLINE_SPEECH_DENOISER_STFT_IMPL_H_
...
class OnlineSpeechDenoiserStftImpl {
 public:
  ...
  DenoisedAudio Run(const float *samples, int32_t n, int32_t sample_rate,
    ...
    if (sample_rate <= 0) {
      EDGEVOX_ONNX_LOGE("Expected sample_rate > 0. Given: %d", sample_rate);
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (n < 0) {
      EDGEVOX_ONNX_LOGE("Expected n >= 0. Given: %d", n);
      EDGEVOX_ONNX_EXIT(-1);
    }
...
#endif  // EDGEVOX_ONNX_CSRC_ONLINE_SPEECH_DENOISER_STFT_IMPL_H_
