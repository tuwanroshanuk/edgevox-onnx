// edgevox-onnx/csrc/offline-paraformer-model-config.cc
//
...
bool OfflineParaformerModelConfig::Validate() const {
  if (EndsWith(model, ".onnx")) {
    if (!FileExists(model)) {
      EDGEVOX_ONNX_LOGE("Paraformer model '%s' does not exist", model.c_str());
      return false;
    }
    return true;
  }
  ...
  if (EndsWith(model, ".om")) {
    ...
    for (const auto &name : filenames) {
      if (!FileExists(name)) {
        EDGEVOX_ONNX_LOGE("Paraformer model '%s' does not exist", name.c_str());
        return false;
      }
    }
  ...
  if (model.empty() && !qnn_config.context_binary.empty()) {
    ...
    for (const auto &name : filenames) {
      if (!FileExists(name)) {
        EDGEVOX_ONNX_LOGE("Paraformer context binary '%s' does not exist",
                         name.c_str());
        return false;
      }
    }
