// edgevox-onnx/csrc/online-wenet-ctc-model.cc
//
// Copyright (c)  2023  Xiaomi Corporation

#include "edgevox-onnx/csrc/online-wenet-ctc-model.h"
...
class OnlineWenetCtcModel::Impl {
 public:
  explicit Impl(const OnlineModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.wenet_ctc.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA(head_, "head");
    EDGEVOX_ONNX_READ_META_DATA(num_blocks_, "num_blocks");
    EDGEVOX_ONNX_READ_META_DATA(output_size_, "output_size");
    EDGEVOX_ONNX_READ_META_DATA(cnn_module_kernel_, "cnn_module_kernel");
    EDGEVOX_ONNX_READ_META_DATA(right_context_, "right_context");
...
std::vector<Ort::Value> OnlineWenetCtcModel::StackStates(
  ...
  if (states.size() != 1) {
    EDGEVOX_ONNX_LOGE("wenet CTC model supports only batch_size==1. Given: %d",
