// edgevox-onnx/csrc/offline-funasr-nano-model.cc
//
...
static inline void AssertTensorIsCpu(const Ort::Value &v, const char *what) {
  ...
  if (status) {
    ...
    EDGEVOX_ONNX_LOGE("%s: failed to get tensor memory info: %s", what, msg);
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (status) {
    const char *msg = Ort::GetApi().GetErrorMessage(status);
    Ort::GetApi().ReleaseStatus(status);
    EDGEVOX_ONNX_LOGE("%s: failed to get mem type: %s", what, msg);
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (mem_type != OrtMemTypeCPU) {
    ...
    if (status) {
      const char *msg = Ort::GetApi().GetErrorMessage(status);
      Ort::GetApi().ReleaseStatus(status);
      EDGEVOX_ONNX_LOGE("%s: failed to get device id: %s", what, msg);
      EDGEVOX_ONNX_EXIT(-1);
    }

    EDGEVOX_ONNX_LOGE("%s: expected CPU tensor but got mem_type=%d device_id=%d",
...
static inline Ort::Value AllocTensorByElemType(
  ...
  EDGEVOX_ONNX_LOGE("AllocTensorByElemType: unsupported elem_type=%d", (int)t);
  EDGEVOX_ONNX_EXIT(-1);
  return AllocTensor<float>(alloc, shape);
}
...
static Ort::Value CastToFloat32(Ort::Value in, OrtAllocator *alloc) {
  ...
  EDGEVOX_ONNX_LOGE("CastToFloat32: unsupported input elem_type=%d", (int)et);
  return in;
}
...
static Ort::Value CastToFloat16(Ort::Value in, OrtAllocator *alloc) {
  ...
  EDGEVOX_ONNX_LOGE("CastToFloat16: unsupported input elem_type=%d", (int)et);
  return in;
}
...
static Ort::Value CastMaskToInt64IfNeeded(Ort::Value in, OrtAllocator *alloc) {
  ...
  EDGEVOX_ONNX_LOGE("attention_mask elem_type=%d not supported, expected int64",
                   (int)et);
  return in;
}
...
class OfflineFunASRNanoModel::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    if (c.encoder_adaptor.empty()) {
      EDGEVOX_ONNX_LOGE("funasr_nano.encoder_adaptor is empty");
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (c.llm.empty()) {
      EDGEVOX_ONNX_LOGE("funasr_nano.llm is required for KV cache mode");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void InitEncoderAdaptorFromMemory(void *model_data,
    ...
    EDGEVOX_ONNX_READ_META_DATA(hidden_size_, "llm_dim");
  }
  ...
  void SetupLlmFromSession() {
    ...
    if (llm_embeds_in_type_ != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {
      EDGEVOX_ONNX_LOGE("LLM inputs_embeds must be float32, got elem_type=%d",
    ...
    if (config_.debug) {
      ...
      PrintModelMetadata(os, meta_data);
#if __OHOS__
      EDGEVOX_ONNX_LOGE("LLM model metadata:\n%{public}s\n", os.str().c_str());
#else
      EDGEVOX_ONNX_LOGE("LLM model metadata:\n%s\n", os.str().c_str());
#endif
    }
    ...
    if (hidden_size_ == 0) {
      EDGEVOX_ONNX_READ_META_DATA(hidden_size_, "hidden_size");
    }
    ...
    if (!num_layers_value.empty()) {
      ...
      if (num_layers_ <= 0) {
        EDGEVOX_ONNX_LOGE("Invalid num_layers=%d from metadata", num_layers_);
        EDGEVOX_ONNX_EXIT(-1);
      }
      if (num_layers_ != inferred_layers) {
        EDGEVOX_ONNX_LOGE("LLM num_layers mismatch: metadata=%d, inferred=%d",
    ...
    if (!(kv_in_type_ == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT ||
      ...
      EDGEVOX_ONNX_LOGE("LLM past_key elem_type=%d not supported",
                       (int)kv_in_type_);
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (!(kv_in_type_v_ == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT ||
      ...
      EDGEVOX_ONNX_LOGE("LLM past_value elem_type=%d not supported",
                       (int)kv_in_type_v_);
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void InitLLMFromMemory(void *model_data, size_t model_data_length) {
    try {
      ...
      EDGEVOX_ONNX_LOGE("InitLLMFromMemory: failed to create session: %s",
                       e.what());
  ...
  Ort::Value ForwardEncoderAdaptor(Ort::Value features) {
    ...
    if (use_cuda_iobinding_) {
      ...
      if (outs.empty()) {
        EDGEVOX_ONNX_LOGE("ForwardEncoderAdaptor: empty outputs");
        EDGEVOX_ONNX_EXIT(-1);
      }
  ...
  std::vector<std::pair<Ort::Value, Ort::Value>> CreateEmptyKVCache(
    ...
    if (tpl.size() < 4) {
      EDGEVOX_ONNX_LOGE("Invalid KV cache shape template, expected >=4 dims");
      EDGEVOX_ONNX_EXIT(-1);
    }
    int64_t kv_h = tpl[2];
    int64_t hd = tpl[3];
  ...
  ForwardLLM(Ort::Value inputs_embeds, Ort::Value attention_mask,
    ...
    if (static_cast<int32_t>(cache_kv.size()) != num_layers_) {
      EDGEVOX_ONNX_LOGE("ForwardLLM: cache_kv size (%zu) != num_layers (%d)",
                       cache_kv.size(), num_layers_);
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (!inputs_embeds.IsTensor()) {
      EDGEVOX_ONNX_LOGE("ForwardLLM: inputs_embeds is not a tensor");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (use_cpu_decode_buffers) {
      ...
      if (outputs.empty()) {
        EDGEVOX_ONNX_LOGE("ForwardLLM: empty outputs");
        EDGEVOX_ONNX_EXIT(-1);
      }
    ...
    if (!logits.IsTensor()) {
      EDGEVOX_ONNX_LOGE("ForwardLLM: logits is not a tensor");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (logits_type != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {
      EDGEVOX_ONNX_LOGE("ForwardLLM: logits must be float32, got elem_type=%d",
                       (int)logits_type);
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (use_cpu_decode_buffers) {
      ...
      if ((outputs.size() - 1) % 2 != 0) {
        EDGEVOX_ONNX_LOGE("ForwardLLM: invalid KV cache outputs size=%d",
  ...
  void ApplyKvDeltaInplace(
    ...
    if (S <= 0) {
      EDGEVOX_ONNX_LOGE("ApplyKvDeltaInplace: cache_position has invalid shape");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (pos0 < 0) {
      EDGEVOX_ONNX_LOGE("ApplyKvDeltaInplace: pos0 < 0 (%d)",
    ...
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
      ...
      switch (elem_type) {
        ...
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT16:
          ...
          break;
        default:
          EDGEVOX_ONNX_LOGE("ApplyKvDeltaInplace: unsupported elem_type=%d",
                           elem_type);
          EDGEVOX_ONNX_EXIT(-1);
      }
  ...
  Ort::Value ForwardEmbedding(Ort::Value input_ids) {
    ...
    if (use_cuda_iobinding_) {
      ...
      if (outs.empty()) {
        EDGEVOX_ONNX_LOGE("ForwardEmbedding: empty outputs");
        EDGEVOX_ONNX_EXIT(-1);
      }
      return std::move(outs[0]);
    }
 ...
 private:
  ...
  void InitEncoderAdaptor(const std::string &model_path) {
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(model_path), sess_opts_encoder_);
    GetInputNames(encoder_sess_.get(), &encoder_input_names_,
                  &encoder_input_names_ptr_);
    GetOutputNames(encoder_sess_.get(), &encoder_output_names_,
                   &encoder_output_names_ptr_);
  ...
  void InitLLM(const std::string &model_path) {
    ...
    if (has_external_data) {
      ...
      llm_sess_ = std::make_unique<Ort::Session>(
          env_, EDGEVOX_ONNX_TO_ORT_PATH(abs_model_path), sess_opts_llm_);
    } else {
  ...
  void InitEmbedding(const std::string &model_path) {
    embedding_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(model_path), sess_opts_embedding_);
    GetInputNames(embedding_sess_.get(), &embedding_input_names_,
                  &embedding_input_names_ptr_);
    GetOutputNames(embedding_sess_.get(), &embedding_output_names_,
                   &embedding_output_names_ptr_);
