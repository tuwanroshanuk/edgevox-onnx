// edgevox-onnx/csrc/provider-config.cc
//
...
bool TensorrtConfig::Validate() const {
  ...
  if (trt_max_partition_iterations < 0) {
    EDGEVOX_ONNX_LOGE("trt_max_partition_iterations: %d is not valid.",
                     trt_max_partition_iterations);
    return false;
  }
  if (trt_min_subgraph_size < 0) {
    EDGEVOX_ONNX_LOGE("trt_min_subgraph_size: %d is not valid.",
...
bool ProviderConfig::Validate() const {
  if (device < 0) {
    EDGEVOX_ONNX_LOGE("device: '%d' is invalid.", device);
    return false;
  }
