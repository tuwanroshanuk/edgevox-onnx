// edgevox-onnx/csrc/offline-catt-model.cc
//
 ...
 public:
  explicit Impl(const OfflineDiacritizationModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.catt_encoder), sess_opts_);
    InitEncoder();

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.catt_decoder), sess_opts_);
    InitDecoder();
  }
