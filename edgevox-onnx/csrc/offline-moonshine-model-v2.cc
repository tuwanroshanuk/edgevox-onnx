// edgevox-onnx/csrc/offline-moonshine-model-v2.cc
//
...
class OfflineMoonshineModelV2::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.moonshine.encoder), sess_opts_);
    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.moonshine.merged_decoder),
        sess_opts_);
 ...
 private:
  ...
  void InitDecoder(void *model_data, size_t model_data_length) {
    ...
    for (const auto &s : decoder_input_names_) {
      if (Contains(s, "key_values")) {
        ...
        if (static_cast<int32_t>(shape.size()) != 4) {
          EDGEVOX_ONNX_LOGE("The shape for %s should be 4-d. Given: %d-d",
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("need attention mask: %d",
                       static_cast<int32_t>(decoder_needs_mask_));
      EDGEVOX_ONNX_LOGE("num_head: %d", num_head_);
      EDGEVOX_ONNX_LOGE("head_dim: %d", head_dim_);
      EDGEVOX_ONNX_LOGE("num_layers: %d", num_layers_);
    }
