// edgevox-onnx/csrc/offline-tts-supertonic-impl.cc
//
...
#include "edgevox-onnx/csrc/offline-tts-supertonic-impl.h"
...
#include "edgevox-onnx/csrc/normal-data-generator.h"
...
SupertonicStyle ParseVoiceStyleFromBinary(const std::vector<char> &buf) {
  ...
  for (int i = 0; i < 6; ++i) {
    if (dims[i] <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid voice style .bin: dims[%d]=%" PRId64 " <= 0", i,
                       dims[i]);
      EDGEVOX_ONNX_EXIT(-1);
    }
  }
  ...
  auto mul3 = [](int64_t a, int64_t b, int64_t c, const char *name) -> size_t {
    ...
    if (a <= 0 || b <= 0 || c <= 0 || a > kMax / b) {
      EDGEVOX_ONNX_LOGE("Invalid voice style .bin: %s dims overflow", name);
      EDGEVOX_ONNX_EXIT(-1);
    }
    int64_t ab = a * b;
    if (ab > kMax / c) {
  ...
  if (ttl_bytes / sizeof(float) != ttl_elems ||
    ...
    EDGEVOX_ONNX_LOGE("Invalid voice style .bin: byte size overflow");
    EDGEVOX_ONNX_EXIT(-1);
  }
  size_t payload_bytes = ttl_bytes + dp_bytes;
  if (payload_bytes < ttl_bytes || payload_bytes < dp_bytes) {
    EDGEVOX_ONNX_LOGE("Invalid voice style .bin: payload size overflow");
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (expected_total < kHeaderSize) {
    EDGEVOX_ONNX_LOGE("Invalid voice style .bin: total size overflow");
    EDGEVOX_ONNX_EXIT(-1);
  }
...
OfflineTtsSupertonicImpl::OfflineTtsSupertonicImpl(
  ...
  if (buf.empty()) {
    EDGEVOX_ONNX_LOGE("Failed to read voice style file: %s",
...
GeneratedAudio OfflineTtsSupertonicImpl::Generate(
  ...
  if (!lang_ok) {
    ...
    EDGEVOX_ONNX_LOGE("Invalid language: %s. Available: %s", lang.c_str(),
  ...
  if (max_len == 0) {
    EDGEVOX_ONNX_LOGE("Max length must be > 0. Given: %zu", max_len);
    return {};
  }
...
GeneratedAudio OfflineTtsSupertonicImpl::Process(
  ...
  if (text_seq_len != text_mask_len) {
    EDGEVOX_ONNX_LOGE("Text sequence length mismatch: text_ids=%" PRId64
  ...
  if (text_emb_size == 0) {
    EDGEVOX_ONNX_LOGE("Text encoder output is empty. Text: \"%s\"",
                     text.c_str());
    return {};
  }
  ...
  if (latent_mask_len != latent_len) {
    EDGEVOX_ONNX_LOGE("Latent mask length mismatch: expected %d, got %" PRId64
                     ". Text: \"%s\"",
  ...
  if (wav_size == 0) {
    EDGEVOX_ONNX_LOGE("Vocoder output is empty. Text: \"%s\"", text.c_str());
    return {};
  }
  ...
  if (config_.model.debug && !result.samples.empty()) {
    ...
    EDGEVOX_ONNX_LOGE("Audio samples: %zu, min_abs=%.6f, max_abs=%.6f",
...
void OfflineTtsSupertonicImpl::InitVoiceStyle(const std::vector<char> &buf) {
  ...
  if (config_.model.debug) {
    EDGEVOX_ONNX_LOGE("Number of speakers: %d", num_speakers_);
  }
}
