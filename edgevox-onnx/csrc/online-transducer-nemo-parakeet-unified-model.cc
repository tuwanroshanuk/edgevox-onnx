// edgevox-onnx/csrc/online-transducer-nemo-parakeet-unified-model.cc
//
...
class OnlineTransducerNeMoParakeetUnifiedModel::Impl {
 public:
  explicit Impl(const OnlineModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.transducer.encoder), sess_opts_);
    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.transducer.decoder), sess_opts_);
    InitDecoder(nullptr, 0);

    joiner_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.transducer.joiner), sess_opts_);
 ...
 private:
  void InitEncoder(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA_STR(streaming_model_type,
                                   "streaming_model_type");
    if (streaming_model_type != kStreamingModelType) {
      EDGEVOX_ONNX_LOGE("Expected streaming_model_type=%s, got: %s",
    ...
    EDGEVOX_ONNX_READ_META_DATA(subsampling_factor_, "subsampling_factor");
    EDGEVOX_ONNX_READ_META_DATA_STR_ALLOW_EMPTY(normalize_type_,
                                               "normalize_type");
    EDGEVOX_ONNX_READ_META_DATA(pred_rnn_layers_, "pred_rnn_layers");
    EDGEVOX_ONNX_READ_META_DATA(pred_hidden_, "pred_hidden");
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(feat_dim_, "feat_dim", -1);

    EDGEVOX_ONNX_READ_META_DATA(left_feature_frames_, "left_feature_frames");
    EDGEVOX_ONNX_READ_META_DATA(chunk_feature_frames_, "chunk_feature_frames");
    EDGEVOX_ONNX_READ_META_DATA(right_feature_frames_, "right_feature_frames");
    EDGEVOX_ONNX_READ_META_DATA(left_encoder_frames_, "left_encoder_frames");
    EDGEVOX_ONNX_READ_META_DATA(chunk_encoder_frames_, "chunk_encoder_frames");
    EDGEVOX_ONNX_READ_META_DATA(right_encoder_frames_, "right_encoder_frames");
  ...
  void ValidateMetadata() const {
    if (vocab_size_ <= 1) {
      EDGEVOX_ONNX_LOGE("Invalid vocab_size: %d", vocab_size_);
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (subsampling_factor_ <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid subsampling_factor: %d", subsampling_factor_);
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (feat_dim_ <= 0) {
      EDGEVOX_ONNX_LOGE(
    ...
    if (left_feature_frames_ < 0 || right_feature_frames_ < 0 ||
      ...
      EDGEVOX_ONNX_LOGE("Left/right context frame counts must be nonnegative");
      EDGEVOX_ONNX_EXIT(-1);
    }
