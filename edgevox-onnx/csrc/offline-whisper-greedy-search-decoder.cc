// edgevox-onnx/csrc/offline-whisper-greedy-search-decoder.cc
//
...
#include "edgevox-onnx/csrc/offline-whisper-timestamp-rules.h"
  ...
  if (model_->IsMultiLingual()) {
    if (!config_.language.empty()) {
      ...
      if (!lang2id.count(config_.language)) {
        EDGEVOX_ONNX_LOGE("Invalid language: %s", config_.language.c_str());
        EDGEVOX_ONNX_EXIT(-1);
      }
