// edgevox-onnx/csrc/sentence-piece-tokenizer-test.cc
//
...
static const char dir[] = "/tmp/edgevox-onnx-test-data";
...
TEST(SpTokenizer, TestEncode) {
  ...
  if (!FileExists(vocab_json) || !FileExists(token_scores_json)) {
    EDGEVOX_ONNX_LOGE(
        ...
        "https://huggingface.co/csukuangfj/edgevox-onnx-test-data/tree/main"
        "and put it inside "
        "/tmp/edgevox-onnx-test-data");
    return;
  }
