// edgevox-onnx/csrc/offline-tts-pocket-impl.h
//
// Copyright (c)  2026  Xiaomi Corporation
#ifndef EDGEVOX_ONNX_CSRC_OFFLINE_TTS_POCKET_IMPL_H_
#define EDGEVOX_ONNX_CSRC_OFFLINE_TTS_POCKET_IMPL_H_
...
#include "edgevox-onnx/csrc/offline-tts-pocket-model.h"
#include "edgevox-onnx/csrc/resample.h"
#include "edgevox-onnx/csrc/sentence-piece-tokenizer.h"
 ...
 public:
  explicit OfflineTtsPocketImpl(const OfflineTtsConfig &config)
...
#if __OHOS__
          EDGEVOX_ONNX_LOGE("rule fst: %{public}s", f.c_str());
#else
    ...
    if (!config.rule_fars.empty()) {
      ...
      for (const auto &f : files) {
        if (config.model.debug) {
#if __OHOS__
          EDGEVOX_ONNX_LOGE("rule far: %{public}s", f.c_str());
#else
  ...
  GeneratedAudio Generate(
    ...
    if (config_.model.debug) {
      EDGEVOX_ONNX_LOGE("%s", gen_config.ToString().c_str());
    }

    std::string text = _text;
    if (config_.model.debug) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("Raw text: %{public}s", text.c_str());
#else
      EDGEVOX_ONNX_LOGE("Raw text: %s", text.c_str());
#endif
    ...
    if (!tn_list_.empty()) {
      for (const auto &tn : tn_list_) {
        ...
        if (config_.model.debug) {
#if __OHOS__
          EDGEVOX_ONNX_LOGE("After normalizing: %{public}s", text.c_str());
#else
          EDGEVOX_ONNX_LOGE("After normalizing: %s", text.c_str());
#endif
        }
      }
    }
 ...
 private:
  ...
  Ort::Value GetVoiceEmbedding(const GenerationConfig &gen_config) const {
    ...
    if (gen_config.reference_audio.empty()) {
      EDGEVOX_ONNX_LOGE("reference audio is empty");
      return Ort::Value{nullptr};
    }
    ...
    if (gen_config.reference_sample_rate != SampleRate()) {
      ...
      auto resampler = std::make_unique<edgevox_onnx::LinearResample>(
    ...
    if (cached_embedding) {
      if (config_.model.debug) {
        EDGEVOX_ONNX_LOGE("CACHE HIT: voice embedding (hash=%zu)", audio_hash);
      }
    ...
    if (config_.model.debug) {
      EDGEVOX_ONNX_LOGE("CACHE MISS: cached embedding (hash=%zu, %zu floats)",
                       audio_hash, total);
    }
...
#endif  // EDGEVOX_ONNX_CSRC_OFFLINE_TTS_POCKET_IMPL_H_
