// edgevox-onnx/csrc/ten-vad-model-config.cc
//
...
bool TenVadModelConfig::Validate() const {
  if (model.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --ten-vad-model");
    return false;
  }

  if (!FileExists(model)) {
    EDGEVOX_ONNX_LOGE("TEN vad model file '%s' does not exist", model.c_str());
    return false;
  }
