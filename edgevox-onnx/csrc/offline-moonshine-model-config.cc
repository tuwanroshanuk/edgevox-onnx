// edgevox-onnx/csrc/offline-moonshine-model-config.cc
//
// Copyright (c)  2024  Xiaomi Corporation

#include "edgevox-onnx/csrc/offline-moonshine-model-config.h"
...
bool OfflineMoonshineModelConfig::Validate() const {
  ...
  if (encoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --moonshine-encoder");
    return false;
  }

  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("moonshine encoder file '%s' does not exist",
                     encoder.c_str());
    return false;
  }
  ...
  if (merged_decoder.empty()) {
    ...
    if (!FileExists(preprocessor)) {
      EDGEVOX_ONNX_LOGE("moonshine preprocessor file '%s' does not exist",
                       preprocessor.c_str());
      return false;
    }

    if (uncached_decoder.empty()) {
      EDGEVOX_ONNX_LOGE("Please provide --moonshine-uncached-decoder for v1");
      return false;
    }

    if (!FileExists(uncached_decoder)) {
      EDGEVOX_ONNX_LOGE("moonshine uncached decoder file '%s' does not exist",
                       uncached_decoder.c_str());
      return false;
    }

    if (cached_decoder.empty()) {
      EDGEVOX_ONNX_LOGE("Please provide --moonshine-cached-decoder for v1");
      return false;
    }

    if (!FileExists(cached_decoder)) {
      EDGEVOX_ONNX_LOGE("moonshine cached decoder file '%s' does not exist",
    ...
    // v2
    if (!preprocessor.empty()) {
      EDGEVOX_ONNX_LOGE("Please don't provide preprocessor for moonshine v2");
      return false;
    }
    ...
    if (!cached_decoder.empty()) {
      EDGEVOX_ONNX_LOGE("Please don't provide cached decoder for moonshine v2");
      return false;
    }
