// edgevox-onnx/csrc/offline-speaker-diarization-pyannote-impl.h
//
// Copyright (c)  2024  Xiaomi Corporation
#ifndef EDGEVOX_ONNX_CSRC_OFFLINE_SPEAKER_DIARIZATION_PYANNOTE_IMPL_H_
#define EDGEVOX_ONNX_CSRC_OFFLINE_SPEAKER_DIARIZATION_PYANNOTE_IMPL_H_
...
#include "Eigen/Dense"
#include "edgevox-onnx/csrc/fast-clustering.h"
 ...
 public:
  ...
  void SetConfig(const OfflineSpeakerDiarizationConfig &config) override {
    if (!config.clustering.Validate()) {
      EDGEVOX_ONNX_LOGE("Invalid clustering config. Skip it");
      return;
    }
  ...
  OfflineSpeakerDiarizationResult Process(
    ...
    if (speakers_per_frame.maxCoeff() == 0) {
      ...
      EDGEVOX_ONNX_LOGE("No speakers found in the audio samples");
      return {};
    }
    ...
    if (valid_indexes.size() != chunk_speaker_samples_list_pair.second.size()) {
      ...
      for (auto i : valid_indexes) {
        if (i < 0 ||
          ...
          EDGEVOX_ONNX_LOGE("valid_indexes: index %d out of range [0, %d)",
                           i,
                           static_cast<int32_t>(
                               chunk_speaker_samples_list_pair.first.size()));
          continue;
  ...
  std::unordered_map<Int32Pair, int32_t, PairHash> ConvertChunkSpeakerToCluster(
    ...
    for (const auto &p : chunk_speaker_pair) {
      if (k >= static_cast<int32_t>(cluster_labels.size())) {
        EDGEVOX_ONNX_LOGE("cluster_labels size mismatch: k=%d, size=%d", k,
...
#endif  // EDGEVOX_ONNX_CSRC_OFFLINE_SPEAKER_DIARIZATION_PYANNOTE_IMPL_H_
