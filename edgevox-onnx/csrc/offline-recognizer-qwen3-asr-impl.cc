// edgevox-onnx/csrc/offline-recognizer-qwen3-asr-impl.cc
//
...
void OfflineRecognizerQwen3ASRImpl::InitPromptTemplateIds() {
  ...
  if (audio_pad_ids_.empty()) {
    EDGEVOX_ONNX_LOGE("Failed to tokenize <|audio_pad|> for qwen3-asr prompt");
    EDGEVOX_ONNX_EXIT(-1);
  }

  asr_text_token_id_ = tokenizer_->GetTokenId("<asr_text>");
  if (asr_text_token_id_ < 0) {
    EDGEVOX_ONNX_LOGE("Failed to locate <asr_text> token id for qwen3-asr");
    EDGEVOX_ONNX_EXIT(-1);
  }
}
