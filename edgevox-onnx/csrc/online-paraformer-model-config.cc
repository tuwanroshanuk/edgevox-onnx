// edgevox-onnx/csrc/online-paraformer-model-config.cc
//
...
bool OnlineParaformerModelConfig::Validate() const {
  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("Paraformer encoder '%s' does not exist", encoder.c_str());
    return false;
  }

  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("Paraformer decoder '%s' does not exist", decoder.c_str());
    return false;
  }
