// edgevox-onnx/csrc/speaker-embedding-extractor.cc
//
...
bool SpeakerEmbeddingExtractorConfig::Validate() const {
  if (model.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide a speaker embedding extractor model");
    return false;
  }

  if (!FileExists(model)) {
    EDGEVOX_ONNX_LOGE("speaker embedding extractor model: '%s' does not exist",
                     model.c_str());
    return false;
  }
