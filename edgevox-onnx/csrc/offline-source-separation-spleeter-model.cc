// edgevox-onnx/csrc/offline-source-separation-spleeter-model.cc
//
 ...
 public:
  explicit Impl(const OfflineSourceSeparationModelConfig &config)
    ...
    vocals_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.spleeter.vocals), sess_opts_);
    InitVocals(nullptr, 0);

    accompaniment_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.spleeter.accompaniment),
        sess_opts_);
 ...
 private:
  void InitVocals(void *model_data, size_t model_data_length) {
    ...
    if (model_type != "spleeter") {
      EDGEVOX_ONNX_LOGE("Expect model type 'spleeter'. Given: '%s'",
                       model_type.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
