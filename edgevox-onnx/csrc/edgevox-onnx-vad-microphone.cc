// edgevox-onnx/csrc/edgevox-onnx-vad-microphone.cc
//
...
bool stop = false;
std::mutex mutex;
edgevox_onnx::CircularBuffer buffer(16000 * 60);
...
This program shows how to use VAD in edgevox-onnx.

  ./bin/edgevox-onnx-vad-microphone \
...
wget https://github.com/k2-fsa/sherpa-onnx/releases/download/asr-models/silero_vad.onnx
  ...
  edgevox_onnx::VadModelConfig config;
  ...
  if (device_index == paNoDevice) {
    ...
    fprintf(stderr, " ./bin/edgevox-onnx-vad-alsa \n");
    EDGEVOX_ONNX_EXIT(EXIT_FAILURE);
  }
  ...
  auto vad = std::make_unique<edgevox_onnx::VoiceActivityDetector>(config);
  ...
  while (!stop) {
    {
      ...
      while (buffer.Size() >= window_size) {
        ...
        while (!vad->Empty()) {
          ...
          k += 1;
          edgevox_onnx::WriteWave(filename, sample_rate, segment.samples.data(),
