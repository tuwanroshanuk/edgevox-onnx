// edgevox-onnx/csrc/offline-dolphin-model.cc
//
...
class OfflineDolphinModel::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.dolphin.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(meta_data_.mean, "mean");
    EDGEVOX_ONNX_READ_META_DATA_VEC_FLOAT(meta_data_.inv_stddev, "invstd");
  }
