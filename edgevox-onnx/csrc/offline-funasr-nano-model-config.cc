// edgevox-onnx/csrc/offline-funasr-nano-model-config.cc
//
// Copyright (c)  2025  zengyw

#include "edgevox-onnx/csrc/offline-funasr-nano-model-config.h"
...
bool OfflineFunASRNanoModelConfig::Validate() const {
  if (encoder_adaptor.empty()) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-encoder-adaptor is required");
    return false;
  }

  if (!FileExists(encoder_adaptor)) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-encoder-adaptor: '%s' does not exist",
                     encoder_adaptor.c_str());
    return false;
  }

  if (llm.empty()) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-llm is required");
    return false;
  }

  if (!FileExists(llm)) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-llm: '%s' does not exist", llm.c_str());
    return false;
  }

  if (tokenizer.empty()) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-tokenizer is required");
    return false;
  }
  ...
  if (embedding.empty()) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-embedding is required");
    return false;
  }

  if (!FileExists(embedding)) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-embedding: '%s' does not exist",
                     embedding.c_str());
    return false;
  }

  if (max_new_tokens <= 0) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-max-new-tokens should be > 0. Given: %d",
                     max_new_tokens);
    return false;
  }

  if (temperature < 0.0f) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-temperature should be >= 0.0. Given: %f",
                     temperature);
    return false;
  }

  if (top_p < 0.0f || top_p > 1.0f) {
    EDGEVOX_ONNX_LOGE("--funasr-nano-top-p should be in [0.0, 1.0]. Given: %f",
                     top_p);
    return false;
  }
