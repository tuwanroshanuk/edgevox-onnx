// edgevox-onnx/csrc/offline-canary-model.cc
//
// Copyright (c)  2025  Xiaomi Corporation

#include "edgevox-onnx/csrc/offline-canary-model.h"
...
#include "edgevox-onnx/csrc/offline-canary-model-meta-data.h"
...
class OfflineCanaryModel::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.canary.encoder), sess_opts_);
    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.canary.decoder), sess_opts_);
 ...
 private:
  void InitEncoder(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA(meta_.vocab_size, "vocab_size");
    EDGEVOX_ONNX_READ_META_DATA_STR_ALLOW_EMPTY(meta_.normalize_type,
                                               "normalize_type");
    EDGEVOX_ONNX_READ_META_DATA(meta_.subsampling_factor, "subsampling_factor");
    EDGEVOX_ONNX_READ_META_DATA(meta_.feat_dim, "feat_dim");

    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.num_decoder_layers,
                                            "num_decoder_layers", 6);
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_.decoder_hidden_size,
