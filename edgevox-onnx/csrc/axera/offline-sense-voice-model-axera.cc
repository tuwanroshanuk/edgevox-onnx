// edgevox-onnx/csrc/axera/offline-sense-voice-model-axera.cc
//
...
class OfflineSenseVoiceModelAxera::Impl {
 public:
  ...
  std::vector<float> Run(std::vector<float> features, int32_t language,
    ...
    if (ret != 0) {
      EDGEVOX_ONNX_LOGE("AX_ENGINE_RunSync failed, ret = %d", ret);
      EDGEVOX_ONNX_EXIT(-1);
    }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if (!io_info_ || io_info_->nInputSize != 2 || !io_info_->pInputs) {
      EDGEVOX_ONNX_LOGE("No input tensor in Axera model");
      EDGEVOX_ONNX_EXIT(-1);
    }

    auto &in0 = io_info_->pInputs[0];
    if (in0.nShapeSize < 2) {
      EDGEVOX_ONNX_LOGE("Input tensor rank is too small (nShapeSize = %u)",
                       in0.nShapeSize);
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (io_info_->nOutputSize != 1) {
      EDGEVOX_ONNX_LOGE("Axera sense voice model expected only 1 output tensor");
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("Axera SenseVoice model init done.");
