// edgevox-onnx/csrc/axera/utils.cc
//
// Copyright (c)  2025  M5Stack Technology CO LTD

#include "edgevox-onnx/csrc/axera/utils.h"
...
#define EDGEVOX_ONNX_TO_STRING(type) \
  case type:                        \
    return #type
...
static const char *kEdgevoxOnnxAxeraSessionName = "edgevox-onnx-axera";
...
static const char *AxEngineDataTypeToString(AX_ENGINE_DATA_TYPE_T type) {
  switch (type) {
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UNKNOWN);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UINT8);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UINT16);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_FLOAT32);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_SINT16);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_SINT8);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_SINT32);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UINT32);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_FLOAT64);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UINT10_PACKED);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UINT12_PACKED);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UINT14_PACKED);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_DT_UINT16_PACKED);
    default:
...
static const char *AxEngineTensorLayoutToString(
  ...
  switch (layout) {
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_TENSOR_LAYOUT_UNKNOWN);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_TENSOR_LAYOUT_NHWC);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_TENSOR_LAYOUT_NCHW);
    default:
...
static const char *AxEngineMemoryTypeToString(AX_ENGINE_MEMORY_TYPE_T type) {
  switch (type) {
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_MT_PHYSICAL);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_MT_VIRTUAL);
    EDGEVOX_ONNX_TO_STRING(AX_ENGINE_MT_OCM);
    default:
...
void InitContext(const void *model_data, size_t model_data_length, bool debug,
  ...
  if (!handle) {
    EDGEVOX_ONNX_LOGE("InitContext: handle is null");
    EDGEVOX_ONNX_EXIT(-1);
  }

  auto ret = AX_ENGINE_CreateHandle(handle, model_data, model_data_length);
  if (ret != 0) {
    EDGEVOX_ONNX_LOGE("AX_ENGINE_CreateHandle failed, ret = %d", ret);
    EDGEVOX_ONNX_EXIT(-1);
  }

  if (debug) {
    EDGEVOX_ONNX_LOGE("AX_ENGINE_CreateHandle done. handle = %p", *handle);
  }

  ret = AX_ENGINE_CreateContext(*handle);
  if (ret != 0) {
    EDGEVOX_ONNX_LOGE("AX_ENGINE_CreateContext failed, ret = %d", ret);
    EDGEVOX_ONNX_EXIT(-1);
  }

  if (debug) {
    EDGEVOX_ONNX_LOGE("AX_ENGINE_CreateContext done.");
  }
}
...
void InitInputOutputAttrs(AX_ENGINE_HANDLE handle, bool debug,
  ...
  if (!io_info) {
    EDGEVOX_ONNX_LOGE("InitInputOutputAttrs: io_info is null");
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (ret != 0) {
    EDGEVOX_ONNX_LOGE("AX_ENGINE_GetIOInfo failed, ret = %d", ret);
    EDGEVOX_ONNX_EXIT(-1);
  }

  if (debug) {
    EDGEVOX_ONNX_LOGE("AX_ENGINE_GetIOInfo done.");
    EDGEVOX_ONNX_LOGE("IO_INFO:\n%s", ToString(*io_info).c_str());
  }
}
...
void PrepareIO(AX_ENGINE_IO_INFO_T *io_info, AX_ENGINE_IO_T *io_data,
               bool debug) {
  if (!io_info || !io_data) {
    EDGEVOX_ONNX_LOGE("PrepareIO: io_info or io_data is null");
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  for (AX_U32 i = 0; i < io_info->nInputSize; ++i) {
    ...
    auto ret = AX_SYS_MemAlloc(
        ...
        reinterpret_cast<const AX_S8 *>(kEdgevoxOnnxAxeraSessionName));

    if (ret != 0) {
      EDGEVOX_ONNX_LOGE("Failed to allocate memory for Input %d",
  ...
  for (AX_U32 i = 0; i < io_info->nOutputSize; ++i) {
    ...
    if (ret != 0) {
      EDGEVOX_ONNX_LOGE("Failed to allocate memory for Output %d",
                       static_cast<int32_t>(i));
      EDGEVOX_ONNX_EXIT(-1);
    }
  }
