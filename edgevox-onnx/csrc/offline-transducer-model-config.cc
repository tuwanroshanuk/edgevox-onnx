// edgevox-onnx/csrc/offline-transducer-model-config.cc
//
  ...
  if (uses_qnn) {
    ...
    if (need_model_libs) {
      ...
      if (!FileExists(encoder_filename)) {
        EDGEVOX_ONNX_LOGE("transducer encoder: '%s' does not exist",
                         encoder_filename.c_str());
        return false;
      }

      if (!FileExists(decoder_filename)) {
        EDGEVOX_ONNX_LOGE("transducer decoder: '%s' does not exist",
                         decoder_filename.c_str());
        return false;
      }

      if (!FileExists(joiner_filename)) {
        EDGEVOX_ONNX_LOGE("transducer joiner: '%s' does not exist",
                         joiner_filename.c_str());
        return false;
      }
    }
