// edgevox-onnx/csrc/offline-tts-supertonic-model.cc
//
...
#include "edgevox-onnx/csrc/offline-tts-supertonic-model.h"
 ...
 private:
  ...
  void LoadModels() {
    dp_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.supertonic.duration_predictor),
        sess_opts_);
    InitDurationPredictor(nullptr, 0);

    text_enc_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.supertonic.text_encoder),
        sess_opts_);
    InitTextEncoder(nullptr, 0);

    vector_est_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.supertonic.vector_estimator),
        sess_opts_);
    InitVectorEstimator(nullptr, 0);

    vocoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.supertonic.vocoder), sess_opts_);
  ...
  void LoadOneModel(Manager *mgr, const std::string &path,
    ...
    if (buf.empty()) {
      EDGEVOX_ONNX_LOGE("Failed to read %s model: %s", model_name, path.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void ParseConfig(const json &j) {
    if (j.find("ae") == j.end() || j.find("ttl") == j.end()) {
      EDGEVOX_ONNX_LOGE("Invalid config file: missing 'ae' or 'ttl' section");
      EDGEVOX_ONNX_EXIT(-1);
    }
    const auto &ae = j["ae"];
    ...
    auto get_int = [](const json &obj, const char *key,
      ...
      if (obj.find(key) == obj.end()) {
        EDGEVOX_ONNX_LOGE("Invalid config: %s.%s missing", section, key);
        EDGEVOX_ONNX_EXIT(-1);
      }
      if (!obj[key].is_number_integer()) {
        EDGEVOX_ONNX_LOGE("Invalid config: %s.%s must be integer", section, key);
        EDGEVOX_ONNX_EXIT(-1);
      }
    ...
    if (cfg_.ae.sample_rate <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid sample_rate: %d", cfg_.ae.sample_rate);
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (cfg_.ae.base_chunk_size <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid base_chunk_size: %d", cfg_.ae.base_chunk_size);
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (cfg_.ttl.chunk_compress_factor <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid chunk_compress_factor: %d",
                       cfg_.ttl.chunk_compress_factor);
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (cfg_.ttl.latent_dim <= 0) {
      EDGEVOX_ONNX_LOGE("Invalid latent_dim: %d", cfg_.ttl.latent_dim);
      EDGEVOX_ONNX_EXIT(-1);
    }
  }
  ...
  static json LoadJsonFromBuffer(const std::vector<char> &buf) {
    if (buf.empty()) {
      EDGEVOX_ONNX_LOGE("Empty json buffer");
      EDGEVOX_ONNX_EXIT(-1);
    }
    try {
      return json::parse(buf.begin(), buf.end());
    } catch (const std::exception &e) {
      EDGEVOX_ONNX_LOGE("Failed to parse JSON buffer: %s", e.what());
      EDGEVOX_ONNX_EXIT(-1);
    }
    return json{};
  }
  ...
  void LoadConfig(const std::string &config_path) {
    ...
    if (buf.empty()) {
      EDGEVOX_ONNX_LOGE("Failed to read config: %s", config_path.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
