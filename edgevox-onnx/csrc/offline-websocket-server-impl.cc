// edgevox-onnx/csrc/offline-websocket-server-impl.cc
//
...
void OfflineWebsocketDecoderConfig::Validate() const {
  if (!recognizer_config.Validate()) {
    EDGEVOX_ONNX_LOGE("Error in recognizer config");
    EDGEVOX_ONNX_EXIT(-1);
  }

  if (max_batch_size <= 0) {
    EDGEVOX_ONNX_LOGE("Expect --max-batch-size > 0. Given: %d", max_batch_size);
    EDGEVOX_ONNX_EXIT(-1);
  }

  if (max_utterance_length <= 0) {
    EDGEVOX_ONNX_LOGE("Expect --max-utterance-length > 0. Given: %f",
...
void OfflineWebsocketServer::OnOpen(connection_hdl hdl) {
  ...
  EDGEVOX_ONNX_LOGE("Number of active connections: %d",
