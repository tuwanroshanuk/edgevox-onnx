// edgevox-onnx/csrc/edgevox-onnx-vad-alsa.cc
//
...
as the device_name.
  ...
  edgevox_onnx::Alsa alsa(device_name.c_str());
  ...
  while (!stop) {
    ...
    while (!vad->Empty()) {
      ...
      os << "seg-" << k << "-" << std::fixed << std::setprecision(3) << duration
         << "s.wav";
      k += 1;
      edgevox_onnx::WriteWave(os.str(), 16000, segment.samples.data(),
