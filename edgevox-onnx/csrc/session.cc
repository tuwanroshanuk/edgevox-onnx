...
#if defined(_WIN32) && EDGEVOX_ONNX_ENABLE_DIRECTML == 1
#include "dml_provider_factory.h"  // NOLINT
#endif

#if defined(EDGEVOX_ONNX_ENABLE_SPACEMIT)
...
static void ParseConfigFile(
  ...
  if (!is.is_open()) {
    EDGEVOX_ONNX_LOGE("Failed to open provider config file: %s",
                     config_path.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  while (std::getline(is, line)) {
    ...
    if (sep_pos == std::string::npos) {
      EDGEVOX_ONNX_LOGE("Ignore invalid provider config line %d in %s: %s",
    ...
    if (key.empty()) {
      EDGEVOX_ONNX_LOGE("Ignore provider config line %d with empty key in %s",
...
static void SplitProviderAndConfig(
  ...
  if (config_path.empty()) {
    EDGEVOX_ONNX_LOGE("Provider config path is empty: %s", s.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (config.find("DEBUG") != config.end()) {
    ...
    if (config_debug) {
      EDGEVOX_ONNX_LOGE("Provider string: %s. Config path: %s",
                       provider_str.c_str(), config_path.c_str());
      for (const auto &kv : config) {
        EDGEVOX_ONNX_LOGE("Provider config: %s=%s", kv.first.c_str(),
                         kv.second.c_str());
      }
    }
  }
...
Ort::SessionOptions GetSessionOptionsImpl(
  ...
  if (config.find("ProfilingFilePrefix") != config.end()) {
    sess_opts.EnableProfiling(EDGEVOX_ONNX_TO_ORT_PATH(config["ProfilingFilePrefix"]));
  ...
  switch (p) {
    ...
    case Provider::kXnnpack: {
      ...
      if (std::find(available_providers.begin(), available_providers.end(),
        ...
        EDGEVOX_ONNX_LOGE("Available providers: %s. Fallback to cpu!",
                         os.str().c_str());
      }
#else
      EDGEVOX_ONNX_LOGE(
    ...
    case Provider::kCUDA: {
      if (std::find(available_providers.begin(), available_providers.end(),
        ...
        EDGEVOX_ONNX_LOGE(
            "Please compile with -DEDGEVOX_ONNX_ENABLE_GPU=ON. Available "
    ...
    case Provider::kDirectML: {
      ...
      if (status) {
        const auto &api = Ort::GetApi();
        const char *msg = api.GetErrorMessage(status);
        EDGEVOX_ONNX_LOGE("Failed to enable DirectML: %s. Fallback to cpu", msg);
        api.ReleaseStatus(status);
      }
#else
      EDGEVOX_ONNX_LOGE("DirectML is for Windows only. Fallback to cpu!");
#endif
      break;
    }
...
#if __ANDROID_API__ >= 27
      EDGEVOX_ONNX_LOGE("Current API level %d ", (int32_t)__ANDROID_API__);
...
#else
      EDGEVOX_ONNX_LOGE("NNAPI is for Android only. Fallback to cpu");
#endif
      break;
    }
    case Provider::kSpacemiT: {
#if defined(EDGEVOX_ONNX_ENABLE_SPACEMIT)
      EDGEVOX_ONNX_LOGE("Use SpacemiT Execution Provider");
      ...
      EDGEVOX_ONNX_LOGE("Set IntraOpNumThreads to 1");
      sess_opts.SetIntraOpNumThreads(1);
      EDGEVOX_ONNX_LOGE("Set InterOpNumThreads to 1");
      sess_opts.SetInterOpNumThreads(1);

      if (provider_options.find("SPACEMIT_EP_INTRA_THREAD_NUM") ==
          provider_options.end()) {
        EDGEVOX_ONNX_LOGE("Set SPACEMIT_EP_INTRA_THREAD_NUM to %d", num_threads);
