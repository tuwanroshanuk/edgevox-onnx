// edgevox-onnx/csrc/offline-tts-supertonic-unicode-processor.cc
//
...
#include "edgevox-onnx/csrc/offline-tts-supertonic-nfkd-table.h"
...
static std::vector<int32_t> LoadIndexerFromPathImpl(
  ...
  if (buf.empty()) {
    EDGEVOX_ONNX_LOGE("Failed to read unicode indexer: %s", path.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
...
SupertonicUnicodeProcessor::SupertonicUnicodeProcessor(
  ...
  if (!EndsWith(unicode_indexer_path, ".bin")) {
    EDGEVOX_ONNX_LOGE("Unicode indexer path must be end with .bin. Given: '%s'",
