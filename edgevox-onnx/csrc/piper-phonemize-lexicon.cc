// edgevox-onnx/csrc/piper-phonemize-lexicon.cc
//
...
static std::string ToString(char32_t cp) {
  ...
  if (cp <= 0x7F) {
    ...
    EDGEVOX_ONNX_LOGE("Invalid Unicode code point: %d",
...
void CallPhonemizeEspeak(const std::string &text,
  ...
  try {
    ...
    EDGEVOX_ONNX_LOGE("Failed to phonemize '%s' with espeak-ng voice '%s': %s",
...
static std::unordered_map<char32_t, int32_t> ReadTokens(std::istream &is) {
  ...
  while (std::getline(is, line)) {
    ...
    iss >> std::ws;
    if (!iss.eof()) {
      EDGEVOX_ONNX_LOGE("Error when reading tokens: %s", line.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (s.size() != 1) {
      ...
      EDGEVOX_ONNX_LOGE("Error when reading tokens at Line %s. size: %d",
    ...
    if (token2id.count(c)) {
      EDGEVOX_ONNX_LOGE("Duplicated token %s. Line %s. Existing ID: %d",
...
static std::vector<int64_t> PiperPhonemesToIdsVits(
  ...
  for (auto p : phonemes) {
    if (token2id.count(p)) {
      ...
      EDGEVOX_ONNX_LOGE("Skip unknown phonemes. Unicode codepoint: \\U+%04x.",
...
static std::vector<std::vector<int64_t>> PiperPhonemesToIdsKokoroOrKitten(
  ...
  for (auto p : phonemes) {
    // EDGEVOX_ONNX_LOGE("%d %s", static_cast<int32_t>(p), ToString(p).c_str());
    if (token2id.count(p)) {
...
std::vector<TokenIDs> PiperPhonemizeLexicon::ConvertTextToTokenIdsVits(
  ...
  if (vits_meta_data_.is_piper || vits_meta_data_.is_icefall) {
    ...
    EDGEVOX_ONNX_LOGE("Unsupported model");
    EDGEVOX_ONNX_EXIT(-1);
  }
