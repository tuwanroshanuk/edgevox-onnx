// edgevox-onnx/csrc/offline-tts-matcha-model.cc
//
...
class OfflineTtsMatchaModel::Impl {
 public:
  explicit Impl(const OfflineTtsModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.matcha.acoustic_model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA_WITH_DEFAULT(meta_data_.has_espeak, "has_espeak",
                                            0);
    EDGEVOX_ONNX_READ_META_DATA(meta_data_.use_eos_bos, "use_eos_bos");
    EDGEVOX_ONNX_READ_META_DATA(meta_data_.pad_id, "pad_id");
