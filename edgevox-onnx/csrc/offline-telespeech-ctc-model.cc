// edgevox-onnx/csrc/offline-telespeech-ctc-model.cc
//
 ...
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.telespeech_ctc), sess_opts_);
    Init(nullptr, 0);
  }
  ...
  std::vector<Ort::Value> Forward(Ort::Value features,
    ...
    if (static_cast<int32_t>(shape[0]) != 1) {
      EDGEVOX_ONNX_LOGE("This model supports only batch size 1. Given %d",
