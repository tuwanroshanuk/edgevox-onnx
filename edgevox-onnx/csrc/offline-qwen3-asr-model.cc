// edgevox-onnx/csrc/offline-qwen3-asr-model.cc
//
// Copyright (c)  2026  zengyw

#include "edgevox-onnx/csrc/offline-qwen3-asr-model.h"
...
inline size_t ElemBytesFromTensorType(ONNXTensorElementDataType t) {
  switch (t) {
    ...
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT16:
      return 2;
    default:
      EDGEVOX_ONNX_LOGE("Unsupported tensor element type: %d",
                       static_cast<int>(t));
      EDGEVOX_ONNX_EXIT(-1);
      return 0;
  }
...
inline Ort::Value AllocTensorByElemType(OrtAllocator *alloc,
  ...
  switch (t) {
    ...
    default:
      EDGEVOX_ONNX_LOGE("AllocTensorByElemType: unsupported elem_type=%d",
                       static_cast<int>(t));
      EDGEVOX_ONNX_EXIT(-1);
      return AllocTensor<float>(alloc, shape);
  }
 ...
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    conv_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(c.conv_frontend), sess_opts_conv_);

    InitConvFrontend(nullptr, 0);

    encoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(c.encoder), sess_opts_encoder_);

    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(c.decoder), sess_opts_decoder_);
  ...
  Impl(Manager *mgr, const OfflineModelConfig &config)
...

    {
      ...
      if (buf.empty()) {
        EDGEVOX_ONNX_LOGE("Failed to read qwen3_asr.conv_frontend: %s",
    ...
    }
    {
      ...
      if (buf.empty()) {
        EDGEVOX_ONNX_LOGE("Failed to read qwen3_asr.encoder: %s",
                         c.encoder.c_str());
        EDGEVOX_ONNX_EXIT(-1);
      }
    ...
    }
    {
      ...
      if (buf.empty()) {
        EDGEVOX_ONNX_LOGE("Failed to read qwen3_asr.decoder: %s",
                         c.decoder.c_str());
        EDGEVOX_ONNX_EXIT(-1);
      }
 ...
 private:
  ...
  void InitConvFrontend(void *model_data, size_t model_data_length) {
    ...
    if (conv_input_names_.size() != 1) {
      EDGEVOX_ONNX_LOGE("conv_frontend must have exactly 1 input, got %zu",
                       conv_input_names_.size());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (conv_output_names_.size() != 1) {
      EDGEVOX_ONNX_LOGE("conv_frontend must have exactly 1 output, got %zu",
  ...
  void InitEncoderSession() {
    ...
    if (encoder_input_names_.size() != 2) {
      EDGEVOX_ONNX_LOGE("encoder must have exactly 2 inputs, got %zu",
                       encoder_input_names_.size());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (encoder_output_names_.size() != 1) {
      EDGEVOX_ONNX_LOGE("encoder must have exactly 1 output, got %zu",
  ...
  void InitDecoderSession() {
    ...
    if (!(kv_in_type_ == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT ||
      ...
      EDGEVOX_ONNX_LOGE("Decoder past_key elem_type=%d not supported",
                       (int)kv_in_type_);
      EDGEVOX_ONNX_EXIT(-1);
    }
    if (!(kv_in_type_v_ == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT ||
      ...
      EDGEVOX_ONNX_LOGE("Decoder past_value elem_type=%d not supported",
                       (int)kv_in_type_v_);
      EDGEVOX_ONNX_EXIT(-1);
    }
  }
 ...
 public:
  Ort::Value ForwardConvFrontend(Ort::Value input_features) {
    if (use_cuda_iobinding_) {
      ...
      if (outs.empty()) {
        EDGEVOX_ONNX_LOGE("ForwardConvFrontend: empty outputs");
        EDGEVOX_ONNX_EXIT(-1);
      }
  ...
  Ort::Value ForwardEncoder(Ort::Value conv_output,
    ...
    if (use_cuda_iobinding_) {
      ...
      auto outs = binding.GetOutputValues();
      if (outs.empty()) {
        EDGEVOX_ONNX_LOGE("ForwardEncoder: empty outputs");
        EDGEVOX_ONNX_EXIT(-1);
      }
      return std::move(outs[0]);
    }
  ...
  ForwardLLM(Ort::Value input_ids, Ort::Value audio_features,
    ...
    if (!audio_features.IsTensor()) {
      EDGEVOX_ONNX_LOGE("ForwardLLM: audio_features is not a tensor");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void ApplyKvDeltaInplace(
    ...
    if (!cache_position.IsTensor()) {
      EDGEVOX_ONNX_LOGE("ApplyKvDeltaInplace: cache_position is not a tensor");
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
      ...
      if (ck_shape.size() < 4 || cv_shape.size() < 4 || dk_shape.size() < 4 ||
          dv_shape.size() < 4) {
        EDGEVOX_ONNX_LOGE("ApplyKvDeltaInplace: KV tensors must be >= 4-D");
        EDGEVOX_ONNX_EXIT(-1);
      }
      ...
      if (B <= 0 || kv_h <= 0 || hd <= 0) {
        EDGEVOX_ONNX_LOGE("ApplyKvDeltaInplace: invalid cache key shape");
        EDGEVOX_ONNX_EXIT(-1);
      }

      if (dk_shape[0] != B || dv_shape[0] != B || dk_shape[2] != kv_h ||
          dv_shape[2] != kv_h || dk_shape[3] != hd || dv_shape[3] != hd) {
        EDGEVOX_ONNX_LOGE("ApplyKvDeltaInplace: delta shape mismatch");
        EDGEVOX_ONNX_EXIT(-1);
      }
