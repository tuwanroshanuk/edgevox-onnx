// edgevox-onnx/csrc/keyword-spotter-transducer-impl.h
//
// Copyright (c)  2023-2024  Xiaomi Corporation

#ifndef EDGEVOX_ONNX_CSRC_KEYWORD_SPOTTER_TRANSDUCER_IMPL_H_
#define EDGEVOX_ONNX_CSRC_KEYWORD_SPOTTER_TRANSDUCER_IMPL_H_
 ...
 public:
  ...
  std::unique_ptr<OnlineStream> CreateStream(
    ...
    if (!EncodeKeywords(is, sym_, &current_ids, &current_kws, &current_scores,
                        &current_thresholds)) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("Encode keywords '%{public}s' failed.",
                       keywords.c_str());
#else
  ...
  void DecodeStreams(OnlineStream **ss, int32_t n) const override {
    ...
    for (int32_t i = 0; i != n; ++i) {
      EDGEVOX_ONNX_CHECK(ss[i]->GetContextGraph() != nullptr);
...
#endif  // EDGEVOX_ONNX_CSRC_KEYWORD_SPOTTER_TRANSDUCER_IMPL_H_
