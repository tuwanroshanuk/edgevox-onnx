// edgevox-onnx/csrc/offline-model-config.cc
//
...
bool OfflineModelConfig::Validate() const {
  ...
  if (funasr_nano.encoder_adaptor.empty() && qwen3_asr.conv_frontend.empty()) {
    if (!FileExists(tokens)) {
      EDGEVOX_ONNX_LOGE("tokens: '%s' does not exist", tokens.c_str());
      return false;
    }
  }
  ...
  if (!modeling_unit.empty() &&
    ...
    if (!FileExists(bpe_vocab)) {
      EDGEVOX_ONNX_LOGE("bpe_vocab: '%s' does not exist", bpe_vocab.c_str());
      return false;
    }
  }
  ...
  if (!telespeech_ctc.empty() && !FileExists(telespeech_ctc)) {
    EDGEVOX_ONNX_LOGE("telespeech_ctc: '%s' does not exist",
