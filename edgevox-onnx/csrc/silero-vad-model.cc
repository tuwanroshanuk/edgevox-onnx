// edgevox-onnx/csrc/silero-vad-model.cc
//
 ...
 public:
  explicit Impl(const VadModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.silero_vad.model), sess_opts_);
    Init(nullptr, 0);
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if ((input_names_.size() == 4 && output_names_.size() == 3) ||
      ...
      EDGEVOX_ONNX_LOGE("Unsupported silero vad model");
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  void CheckV4() const {
    ...
    if (input_names_.size() != 4) {
      EDGEVOX_ONNX_LOGE("Expect 4 inputs. Given: %d",
                       static_cast<int32_t>(input_names_.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (input_names_[0] != "input") {
      EDGEVOX_ONNX_LOGE("Input[0]: %s. Expected: input",
                       input_names_[0].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (input_names_[1] != "sr") {
      EDGEVOX_ONNX_LOGE("Input[1]: %s. Expected: sr", input_names_[1].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (input_names_[2] != "h") {
      EDGEVOX_ONNX_LOGE("Input[2]: %s. Expected: h", input_names_[2].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (input_names_[3] != "c") {
      EDGEVOX_ONNX_LOGE("Input[3]: %s. Expected: c", input_names_[3].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    // Now for outputs
    if (output_names_.size() != 3) {
      EDGEVOX_ONNX_LOGE("Expect 3 outputs. Given: %d",
                       static_cast<int32_t>(output_names_.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (output_names_[0] != "output") {
      EDGEVOX_ONNX_LOGE("Output[0]: %s. Expected: output",
                       output_names_[0].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (output_names_[1] != "hn") {
      EDGEVOX_ONNX_LOGE("Output[1]: %s. Expected: sr", output_names_[1].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (output_names_[2] != "cn") {
      EDGEVOX_ONNX_LOGE("Output[2]: %s. Expected: sr", output_names_[2].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
  }
  ...
  void CheckV5() const {
    if (input_names_.size() != 3) {
      EDGEVOX_ONNX_LOGE("Expect 3 inputs. Given: %d",
                       static_cast<int32_t>(input_names_.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (input_names_[1] != "state") {
      EDGEVOX_ONNX_LOGE("Input[1]: %s. Expected: state",
                       input_names_[1].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (input_names_[2] != "sr") {
      EDGEVOX_ONNX_LOGE("Input[2]: %s. Expected: sr", input_names_[2].c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    // Now for outputs
    if (output_names_.size() != 2) {
      EDGEVOX_ONNX_LOGE("Expect 2 outputs. Given: %d",
                       static_cast<int32_t>(output_names_.size()));
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (output_names_[1] != "stateN") {
      EDGEVOX_ONNX_LOGE("Output[1]: %s. Expected: stateN",
