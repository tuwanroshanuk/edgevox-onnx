// edgevox-onnx/csrc/context-graph.cc
//
...
void ContextGraph::Build(const std::vector<std::vector<int32_t>> &token_ids,
  ...
  if (!scores.empty()) {
    EDGEVOX_ONNX_CHECK_EQ(token_ids.size(), scores.size());
  }
  if (!phrases.empty()) {
    EDGEVOX_ONNX_CHECK_EQ(token_ids.size(), phrases.size());
  }
  if (!ac_thresholds.empty()) {
    EDGEVOX_ONNX_CHECK_EQ(token_ids.size(), ac_thresholds.size());
  }
...
ContextGraph::ForwardOneStep(const ContextState *state, int32_t token,
  ...
  if (!node) {
    EDGEVOX_ONNX_LOGE("Some bad things happened.");
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (!strict_mode && node->output_score != 0) {
    EDGEVOX_ONNX_CHECK(nullptr != matched_node);
    float output_score =
