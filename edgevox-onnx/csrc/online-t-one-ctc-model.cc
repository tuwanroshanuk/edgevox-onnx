// edgevox-onnx/csrc/online-t-one-ctc-model.cc
//
// Copyright (c)  2025  Xiaomi Corporation

#include "edgevox-onnx/csrc/online-t-one-ctc-model.h"
...
class OnlineToneCtcModel::Impl {
 public:
  explicit Impl(const OnlineModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config.t_one_ctc.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    EDGEVOX_ONNX_READ_META_DATA(frame_length_ms_, "frame_length_ms");
    EDGEVOX_ONNX_READ_META_DATA(state_dim_, "state_dim");
    EDGEVOX_ONNX_READ_META_DATA(sample_rate_, "sample_rate");
