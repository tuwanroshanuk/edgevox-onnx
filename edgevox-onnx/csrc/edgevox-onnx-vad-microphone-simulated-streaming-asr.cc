// edgevox-onnx/csrc/edgevox-onnx-vad-microphone-simulated-streaming-asr.cc
//
...
#include "edgevox-onnx/csrc/sherpa-display.h"
...
edgevox-onnx for real-time speech recognition.
...
Please first use ./bin/edgevox-onnx-offline to test the RTF of the model.
A model with RTF < 0.2 should work with this program.
  ...
  po.Register("mic-device-index", &user_device_index,
              ...
              "You can use edgevox-onnx-pa-devs to list available devices");

  po.Register("mic-sample-rate", &user_sample_rate,
              "If provided, we use it to replace the default sample rate."
              "You can use edgevox-onnx-pa-devs to list sample rate of "
              "available devices");
  ...
  std::unique_ptr<edgevox_onnx::LinearResample> resampler;
  if (mic_sample_rate != sample_rate) {
    float min_freq = std::min(mic_sample_rate, sample_rate);
    float lowpass_cutoff = 0.99 * 0.5 * min_freq;

    int32_t lowpass_filter_width = 6;
    resampler = std::make_unique<edgevox_onnx::LinearResample>(
  ...
  edgevox_onnx::SherpaDisplay display;
