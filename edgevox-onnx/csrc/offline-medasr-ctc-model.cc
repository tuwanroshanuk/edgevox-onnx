// edgevox-onnx/csrc/offline-medasr-ctc-model.cc
//
...
std::vector<int64_t> GetMask(Ort::Value length) {
  ...
  if (shape.size() != 1) {
    EDGEVOX_ONNX_LOGE("Invalid length dim %zu", shape.size());
    EDGEVOX_ONNX_EXIT(-1);
  }
 ...
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    sess_ = std::make_unique<Ort::Session>(
        env_, EDGEVOX_ONNX_TO_ORT_PATH(config_.medasr.model), sess_opts_);
    Init(nullptr, 0);
  }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if (model_type != "medasr_ctc") {
      EDGEVOX_ONNX_LOGE("Expect model type medasr_ctc. Given: '%s'",
                       model_type.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
