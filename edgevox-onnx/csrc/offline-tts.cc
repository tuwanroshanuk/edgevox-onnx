// edgevox-onnx/csrc/offline-tts.cc
//
...
GeneratedAudio GeneratedAudio::ScaleSilence(float scale) const {
  ...
  if (!(scale >= kMinSilenceScale && scale <= kMaxSilenceScale)) {
    EDGEVOX_ONNX_LOGE("Silence scale %f is not in [%.2f, %.2f]. Skip scaling.",
...
bool OfflineTtsConfig::Validate() const {
  ...
  if (!rule_fars.empty()) {
    ...
    for (const auto &f : files) {
      if (!FileExists(f)) {
        EDGEVOX_ONNX_LOGE("Rule far '%s' does not exist. ", f.c_str());
        return false;
      }
    }
  }
  ...
  if (!(silence_scale >= kMinSilenceScale &&
    ...
    EDGEVOX_ONNX_LOGE("--tts-silence-scale '%.3f' is not in [%.2f, %.2f]",
...
GeneratedAudio OfflineTts::Generate(
  ...
  if (IsGB2312(text)) {
    ...
    if (!printed) {
      EDGEVOX_ONNX_LOGE("Detected GB2312 encoded text! Converting it to UTF8.");
      printed = true;
    }
  }
