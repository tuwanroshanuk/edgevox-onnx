// edgevox-onnx/csrc/rknn/offline-paraformer-model-rknn.cc
//
...
class OfflineParaformerModelRknn::Impl {
 public:
  ~Impl() {
    ...
    if (ret != RKNN_SUCC) {
      EDGEVOX_ONNX_LOGE("Failed to destroy the predictor context");
    }
  ...
  explicit Impl(const OfflineModelConfig &config) : config_(config) {
    ...
    if (filenames.size() != 3) {
      EDGEVOX_ONNX_LOGE("Invalid Paraformer RK NPU model '%s'",
  ...
  std::vector<float> Run(std::vector<float> features) {
    ...
    if (acoustic_embedding.empty()) {
      if (config_.debug) {
        EDGEVOX_ONNX_LOGE("No speech found in the input audio");
      }
 ...
 private:
  std::vector<float> RunEncoder(std::vector<float> features) {
    ...
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to run the encoder model");
  ...
  std::vector<float> RunPredictor(const std::vector<float> &encoder_out) {
    ...
    auto ret = rknn_inputs_set(ctx, inputs.size(), inputs.data());
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to set predictor inputs");

    ret = rknn_run(ctx, nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to run the predictor model");

    ret = rknn_outputs_get(ctx, outputs.size(), outputs.data(), nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get predictor output");
  ...
  std::vector<float> RunDecoder(std::vector<float> encoder_out,
    ...
    ret = rknn_run(ctx, nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to run the decoder model");
  ...
  void InitEncoder(void *model_data, size_t model_data_length) {
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("num_input_frames_: %d", num_input_frames_);
      EDGEVOX_ONNX_LOGE("encoder_out_dim:: %d", encoder_out_dim_);
    }
  }
