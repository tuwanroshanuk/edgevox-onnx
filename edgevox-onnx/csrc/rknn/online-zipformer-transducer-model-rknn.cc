// edgevox-onnx/csrc/rknn/online-zipformer-transducer-model-rknn.cc
//
...
class OnlineZipformerTransducerModelRknn::Impl {
 public:
  ~Impl() {
    ...
    if (ret != RKNN_SUCC) {
      EDGEVOX_ONNX_LOGE("Failed to destroy the encoder context");
    }

    ret = rknn_destroy(decoder_ctx_);
    if (ret != RKNN_SUCC) {
      EDGEVOX_ONNX_LOGE("Failed to destroy the decoder context");
    }

    ret = rknn_destroy(joiner_ctx_);
    if (ret != RKNN_SUCC) {
      EDGEVOX_ONNX_LOGE("Failed to destroy the joiner context");
    }
  }
  ...
  std::vector<std::vector<uint8_t>> GetEncoderInitStates() const {
    ...
    for (auto &attr : encoder_input_attrs_) {
      ...
      if (attr.type == RKNN_TENSOR_FLOAT16) {
        ...
        EDGEVOX_ONNX_LOGE("Unsupported tensor type: %d, %s", attr.type,
  ...
  std::pair<std::vector<float>, std::vector<std::vector<uint8_t>>> RunEncoder(
    ...
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to set encoder inputs");

    ret = rknn_run(encoder_ctx, nullptr);

    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to run encoder");

    ret =
        rknn_outputs_get(encoder_ctx, outputs.size(), outputs.data(), nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get encoder output");
  ...
  std::vector<float> RunDecoder(std::vector<int64_t> decoder_input) {
    ...
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to set decoder inputs");

    ret = rknn_run(decoder_ctx, nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to run decoder");

    ret = rknn_outputs_get(decoder_ctx, 1, &output, nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get decoder output");
  ...
  std::vector<float> RunJoiner(const float *encoder_out,
    ...
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to set joiner inputs");

    ret = rknn_run(joiner_ctx, nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to run joiner");

    ret = rknn_outputs_get(joiner_ctx, 1, &output, nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get joiner output");
 ...
 private:
  ...
  void InitDecoder(void *model_data, size_t model_data_length) {
    ...
    if (decoder_input_attrs_[0].type != RKNN_TENSOR_INT64) {
      EDGEVOX_ONNX_LOGE("Expect int64 for decoder input. Given: %d, %s",
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("context_size: %d", context_size_);
    }
  }
