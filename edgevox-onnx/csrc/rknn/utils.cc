...
std::unordered_map<std::string, std::string> Parse(
  ...
  for (const auto &f : fields) {
    ...
    if (tmp.size() != 2) {
      EDGEVOX_ONNX_LOGE("Invalid custom string %s for %s", custom_string.string,
                       f.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
...
void InitContext(void *model_data, size_t model_data_length, bool debug,
  ...
  EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to init rknn");

  if (debug) {
    rknn_sdk_version v;
    ret = rknn_query(*ctx, RKNN_QUERY_SDK_VERSION, &v, sizeof(v));
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get rknn sdk version");

    EDGEVOX_ONNX_LOGE("sdk api version: %s, driver version: %s", v.api_version,
                     v.drv_version);
  }
}
...
void InitInputOutputAttrs(rknn_context ctx, bool debug,
  ...
  EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get I/O information for the model");

  if (debug) {
    EDGEVOX_ONNX_LOGE("model: %d inputs, %d outputs",
  ...
  for (auto &attr : *input_attrs) {
    ...
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get attr for model input %d", i);
    i += 1;
  }
  ...
  for (auto &attr : *output_attrs) {
    ...
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get attr for model output %d", i);
    i += 1;
  }
...
rknn_custom_string GetCustomString(rknn_context ctx, bool debug) {
  ...
  EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to read custom string from the model");
  if (debug) {
    EDGEVOX_ONNX_LOGE("customs string: %s", custom_string.string);
  }
