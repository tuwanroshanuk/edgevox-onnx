// edgevox-onnx/csrc/rknn/offline-sense-voice-model-rknn.cc
//
...
#include "edgevox-onnx/csrc/rknn/context-blocking-queue-rknn.h"
#include "edgevox-onnx/csrc/rknn/macros.h"
#include "edgevox-onnx/csrc/rknn/utils.h"
...
class OfflineSenseVoiceModelRknn::Impl {
 public:
  ~Impl() {
    ...
    if (ret != RKNN_SUCC) {
      EDGEVOX_ONNX_LOGE("Failed to destroy the context");
    }
  }
  ...
  std::vector<float> Run(std::vector<float> features, int32_t language,
    ...
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to set inputs");

    ret = rknn_run(ctx, nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to run the model");

    ret = rknn_outputs_get(ctx, outputs.size(), outputs.data(), nullptr);
    EDGEVOX_ONNX_RKNN_CHECK(ret, "Failed to get model output");
...
#define EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(dst, src_key)                     \
  do {                                                                        \
    if (!meta.count(#src_key)) {                                              \
      EDGEVOX_ONNX_LOGE("'%s' does not exist in the custom_string", #src_key); \
      EDGEVOX_ONNX_EXIT(-1);                                                   \
    }                                                                         \
                                                                              \
    ...
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(meta_data_.with_itn_id, with_itn);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(meta_data_.without_itn_id, without_itn);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(meta_data_.window_size,
                                        lfr_window_size);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(meta_data_.window_shift,
                                        lfr_window_shift);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(meta_data_.vocab_size, vocab_size);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(meta_data_.normalize_samples,
                                        normalize_samples);
    ...
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(lang_auto, lang_auto);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(lang_zh, lang_zh);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(lang_en, lang_en);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(lang_ja, lang_ja);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(lang_ko, lang_ko);
    EDGEVOX_ONNX_RKNN_READ_META_DATA_INT(lang_yue, lang_yue);
...
#undef EDGEVOX_ONNX_RKNN_READ_META_DATA_INT
