// edgevox-onnx/csrc/rknn/keyword-spotter-transducer-rknn-impl.h
//
// Copyright (c)  2025  Xiaomi Corporation

#ifndef EDGEVOX_ONNX_CSRC_RKNN_KEYWORD_SPOTTER_TRANSDUCER_RKNN_IMPL_H_
#define EDGEVOX_ONNX_CSRC_RKNN_KEYWORD_SPOTTER_TRANSDUCER_RKNN_IMPL_H_
...
#include "edgevox-onnx/csrc/rknn/transducer-keyword-decoder-rknn.h"
 ...
 public:
  ...
  std::unique_ptr<OnlineStream> CreateStream(
    ...
    if (!EncodeKeywords(is, sym_, &current_ids, &current_kws, &current_scores,
                        &current_thresholds)) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("Encode keywords %{public}s failed.", keywords.c_str());
#else
      EDGEVOX_ONNX_LOGE("Encode keywords %s failed.", keywords.c_str());
#endif
      return nullptr;
    }
 ...
 private:
  void InitKeywords(std::istream &is) {
    if (!EncodeKeywords(is, sym_, &keywords_id_, &keywords_, &boost_scores_,
                        &thresholds_)) {
      EDGEVOX_ONNX_LOGE("Encode keywords failed.");
      exit(-1);
    }
  ...
  void InitKeywords() {
#ifdef EDGEVOX_ONNX_ENABLE_WASM_KWS
    ...
    if (!is) {
#if __OHOS__
      EDGEVOX_ONNX_LOGE("Open keywords file failed: %{public}s",
                       config_.keywords_file.c_str());
#else
      EDGEVOX_ONNX_LOGE("Open keywords file failed: %s",
                       config_.keywords_file.c_str());
#endif
      exit(-1);
    }
  ...
  void InitOnlineStream(OnlineStreamRknn *stream) const {
    ...
    EDGEVOX_ONNX_CHECK_EQ(r.hyps.Size(), 1);

    EDGEVOX_ONNX_CHECK(stream->GetContextGraph() != nullptr);
...
#endif  // EDGEVOX_ONNX_CSRC_RKNN_KEYWORD_SPOTTER_TRANSDUCER_RKNN_IMPL_H_
