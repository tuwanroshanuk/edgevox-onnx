// edgevox-onnx/csrc/rknn/silero-vad-model-rknn.cc
//
 ...
 public:
  ...
  explicit Impl(const VadModelConfig &config)
    ...
    if (sample_rate_ != 16000) {
      EDGEVOX_ONNX_LOGE("Expected sample rate 16000. Given: %d",
                       config.sample_rate);
      EDGEVOX_ONNX_EXIT(-1);
    }
  ...
  bool IsSpeech(const float *samples, int32_t n) {
    if (n != WindowSize()) {
      EDGEVOX_ONNX_LOGE("n: %d != window_size: %d", n, WindowSize());
      EDGEVOX_ONNX_EXIT(-1);
    }
 ...
 private:
  void Init(void *model_data, size_t model_data_length) {
    ...
    if (config_.silero_vad.window_size != 512) {
      EDGEVOX_ONNX_LOGE("we require window_size to be 512. Given: %d",
    ...
    if (config_.debug) {
      for (const auto &p : meta) {
        EDGEVOX_ONNX_LOGE("%s: %s", p.first.c_str(), p.second.c_str());
      }
    }

    if (meta.count("model_type") == 0) {
      EDGEVOX_ONNX_LOGE("No model type found in '%s'",
                       config_.silero_vad.model.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (meta.at("model_type") != "silero-vad-v4") {
      EDGEVOX_ONNX_LOGE("Expect model type silero-vad-v4 in '%s', given: '%s'",
                       config_.silero_vad.model.c_str(),
    ...
    if (meta.count("sample_rate") == 0) {
      EDGEVOX_ONNX_LOGE("No sample_rate found in '%s'",
                       config_.silero_vad.model.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (meta.at("sample_rate") != "16000") {
      EDGEVOX_ONNX_LOGE("Expect sample rate 16000 in '%s', given: '%s'",
                       config_.silero_vad.model.c_str(),
    ...
    if (meta.count("version") == 0) {
      EDGEVOX_ONNX_LOGE("No version found in '%s'",
                       config_.silero_vad.model.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (meta.at("version") != "4") {
      EDGEVOX_ONNX_LOGE("Expect version 4 in '%s', given: '%s'",
                       config_.silero_vad.model.c_str(),
    ...
    if (meta.count("h_shape") == 0) {
      EDGEVOX_ONNX_LOGE("No h_shape found in '%s'",
                       config_.silero_vad.model.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (meta.count("c_shape") == 0) {
      EDGEVOX_ONNX_LOGE("No c_shape found in '%s'",
                       config_.silero_vad.model.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    if (h_shape.size() != 3 || c_shape.size() != 3) {
      EDGEVOX_ONNX_LOGE("Incorrect shape for h (%d) or c (%d)",
