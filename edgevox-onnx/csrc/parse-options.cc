// edgevox-onnx/csrc/parse-options.cc
/**
...
#include "edgevox-onnx/csrc/log.h"
...
void ParseOptions::RegisterCommon(const std::string &name, T *ptr,
  ...
  EDGEVOX_ONNX_CHECK(ptr != nullptr);
  std::string idx = name;
  NormalizeArgName(&idx);
  if (doc_map_.find(idx) != doc_map_.end()) {
    EDGEVOX_ONNX_LOGE("Registering option twice, ignoring second time: %s",
                     name.c_str());
  } else {
...
void ParseOptions::DisableOption(const std::string &name) {
  if (argv_ != nullptr) {
    EDGEVOX_ONNX_LOGE("DisableOption must not be called after calling Read().");
    EDGEVOX_ONNX_EXIT(-1);
  }
  if (doc_map_.erase(name) == 0) {
    EDGEVOX_ONNX_LOGE("Option %s was not registered so cannot be disabled: ",
                     name.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
...
std::string ParseOptions::GetArg(int32_t i) const {
  if (i < 1 || i > static_cast<int32_t>(positional_args_.size())) {
    EDGEVOX_ONNX_LOGE("ParseOptions::GetArg, invalid index %d", i);
    EDGEVOX_ONNX_EXIT(-1);
  }
...
static bool MustBeQuoted(const std::string &str, ShellType st) {
  ...
  EDGEVOX_ONNX_CHECK_EQ(st, kBash) << "Invalid shell type.";
  ...
  if (*c == '\0') {
    ...
    EDGEVOX_ONNX_CHECK(!strchr(ok_chars[kBash], ' '));
...
int32_t ParseOptions::Read(int32_t argc, const char *const *argv) {
  ...
  for (i = 1; i < argc; ++i) {
    if (std::strncmp(argv[i], "--", 2) == 0) {
      ...
      if (!SetOption(key, value, has_equal_sign)) {
        PrintUsage(true);
        EDGEVOX_ONNX_LOGE("Invalid option %s", argv[i]);
        EDGEVOX_ONNX_EXIT(-1);
      }
    } else {
      break;
  ...
  if (print_args_) {
    ...
    strm << '\n';
    EDGEVOX_ONNX_LOGE("%s", strm.str().c_str());
  }
  return i;
}
...
void ParseOptions::PrintConfig(std::ostream &os) const {
  ...
  for (const auto &it : doc_map_) {
    ...
    if (bool_map_.end() != bool_map_.find(key)) {
      ...
      EDGEVOX_ONNX_LOGE("PrintConfig: unrecognized option %s [code error]",
                       key.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    os << '\n';
...
void ParseOptions::ReadConfigFile(const std::string &filename) {
  ...
  if (!is.good()) {
    EDGEVOX_ONNX_LOGE("Cannot open config file: %s", filename.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  while (std::getline(is, line)) {
    ...
    if (line.substr(0, 2) != "--") {
      EDGEVOX_ONNX_LOGE(
          ...
          "from a edgevox-onnx command-line program's config file: should "
    ...
    if (!SetOption(key, value, has_equal_sign)) {
      PrintUsage(true);
      EDGEVOX_ONNX_LOGE("Invalid option %s in config file %s: line %d",
...
void ParseOptions::SplitLongArg(const std::string &in, std::string *key,
  ...
  EDGEVOX_ONNX_CHECK(in.substr(0, 2) == "--") << in;  // precondition.
  ...
  if (pos == std::string::npos) {  // we allow --option for bools
    ...
    *has_equal_sign = false;
    ...
    PrintUsage(true);
    EDGEVOX_ONNX_LOGE("Invalid option (no key): %s", in.c_str());
    EDGEVOX_ONNX_EXIT(-1);
...
void ParseOptions::NormalizeArgName(std::string *str) const {
  ...
  *str = out;
  ...
  EDGEVOX_ONNX_CHECK_GT(str->length(), 0);
}
...
bool ParseOptions::SetOption(const std::string &key, const std::string &value,
  ...
  if (bool_map_.end() != bool_map_.find(key)) {
    if (has_equal_sign && value.empty()) {
      EDGEVOX_ONNX_LOGE("Invalid option --%s=", key.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
    ...
    *(double_map_[key]) = ToDouble(value);
    ...
    if (!has_equal_sign) {
      EDGEVOX_ONNX_LOGE("Invalid option --%s (option format is --x=y).",
                       key.c_str());
      EDGEVOX_ONNX_EXIT(-1);
    }
...
int32_t ParseOptions::ToInt(const std::string &str) const {
  ...
  if (!ConvertStringToInteger(str, &ret)) {
    EDGEVOX_ONNX_LOGE("Invalid integer option \"%s\"", str.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  return ret;
}
...
int64_t ParseOptions::ToInt64(const std::string &str) const {
  int64_t ret = 0;
  if (!ConvertStringToInteger(str, &ret)) {
    EDGEVOX_ONNX_LOGE("Invalid integer int64 option \"%s\"", str.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  return ret;
}
...
float ParseOptions::ToFloat(const std::string &str) const {
  ...
  if (!ConvertStringToReal(str, &ret)) {
    EDGEVOX_ONNX_LOGE("Invalid floating-point option \"%s\"", str.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  return ret;
}
