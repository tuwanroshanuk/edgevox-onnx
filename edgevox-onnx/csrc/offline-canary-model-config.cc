// edgevox-onnx/csrc/offline-canary-model-config.cc
//
...
bool OfflineCanaryModelConfig::Validate() const {
  if (encoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --canary-encoder");
    return false;
  }

  if (!FileExists(encoder)) {
    EDGEVOX_ONNX_LOGE("Canary encoder file '%s' does not exist",
                     encoder.c_str());
    return false;
  }

  if (decoder.empty()) {
    EDGEVOX_ONNX_LOGE("Please provide --canary-decoder");
    return false;
  }

  if (!FileExists(decoder)) {
    EDGEVOX_ONNX_LOGE("Canary decoder file '%s' does not exist",
                     decoder.c_str());
    return false;
  }
  ...
  if (!src_lang.empty()) {
    if (src_lang != "en" && src_lang != "de" && src_lang != "es" &&
        src_lang != "fr") {
      EDGEVOX_ONNX_LOGE("Please use en, de, es, or fr for --canary-src-lang");
      return false;
    }
  }
  ...
  if (!tgt_lang.empty()) {
    if (tgt_lang != "en" && tgt_lang != "de" && tgt_lang != "es" &&
        tgt_lang != "fr") {
      EDGEVOX_ONNX_LOGE("Please use en, de, es, or fr for --canary-tgt-lang");
      return false;
    }
  }
