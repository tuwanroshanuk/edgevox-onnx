// edgevox-onnx/csrc/ascend/utils.cc
//
...
Acl::Acl() {
  ...
  EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclInit");
  initialized_ = true;
}
...
Acl::~Acl() {
  if (initialized_) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclFinalize");
  }
}

AclContext::AclContext(int32_t device_id) {
  aclError ret = aclrtCreateContext(&context_, device_id);
  EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclrtCreateContext");
}

AclContext::~AclContext() {
  if (context_) {
    aclError ret = aclrtDestroyContext(context_);
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclrtDestroyContext");
  }
}
...
AclDevicePtr::AclDevicePtr(
  ...
  if (size > 0) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclrtMalloc with size: %zu",
                             size);
  }
  size_ = size;
}
...
AclDevicePtr::~AclDevicePtr() {
  if (p_) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclrtFree");
  }
}
...
AclModel::AclModel(const void *model, size_t model_size) {
  ...
  EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlLoadFromMem");
...
AclModel::~AclModel() {
  if (model_id_ != 0) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlUnload");
  }
}
...
void AclModel::InitInputShapes() {
  ...
  for (int32_t i = 0; i < num_inputs; ++i) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlGetInputDims");
...
void AclModel::InitOutputShapes() {
  ...
  for (int32_t i = 0; i < num_outputs; ++i) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlGetOutputDims");
...
AclMdlDataset::AclMdlDataset() {
  ...
  if (!p_) {
    EDGEVOX_ONNX_LOGE("Failed to call aclmdlCreateDataset");
    EDGEVOX_ONNX_EXIT(-1);
  }
}
...
AclMdlDataset::~AclMdlDataset() {
  if (p_) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlDestroyDataset");
  }
}

void AclMdlDataset::AddBuffer(aclDataBuffer *buffer) const {
  aclError ret = aclmdlAddDatasetBuffer(p_, buffer);
  EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlAddDatasetBuffer");
}
...
AclDataBuffer::AclDataBuffer(void *data, size_t size) {
  ...
  if (!p_) {
    EDGEVOX_ONNX_LOGE("Failed to call aclCreateDataBuffer");
    EDGEVOX_ONNX_EXIT(-1);
  }
}
...
void AclDataBuffer::Release() {
  if (p_) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclDestroyDataBuffer");
  }
  p_ = nullptr;
}
...
AclTensorDesc::AclTensorDesc(aclDataType data_type, int num_dims,
  ...
  if (!p_) {
    EDGEVOX_ONNX_LOGE("Failed to call aclCreateTensorDesc");
    EDGEVOX_ONNX_EXIT(-1);
  }
}
