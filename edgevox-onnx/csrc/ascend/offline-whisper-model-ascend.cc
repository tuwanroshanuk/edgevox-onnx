// edgevox-onnx/csrc/ascend/offline-whisper-model-ascend.cc
//
...
#include "edgevox-onnx/csrc/ascend/macros.h"
#include "edgevox-onnx/csrc/ascend/utils.h"
...
static WhisperModelType ParseWhisperModelFromString(const std::string &s) {
  ...
  if (pos == std::string::npos) {
    EDGEVOX_ONNX_LOGE("Unexpected model input '%s'", s.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
  ...
  if (pos == 0) {
    EDGEVOX_ONNX_LOGE("Empty model name in '%s'", s.c_str());
    EDGEVOX_ONNX_EXIT(-1);
  }
...
class OfflineWhisperModelAscend::Impl {
 public:
  ...
  OfflineWhisperDecoderResult Run(std::vector<float> features) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret_set_ctx, "Failed to call aclrtSetCurrentContext");
 ...
 private:
  void RunEncoder(std::vector<float> features) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclrtMemcpy");
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlExecute");
  }
  ...
  void PreInit() {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclrtSetCurrentContext");
  }
  ...
  void Preallocate() {
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("Allocated %d bytes, or %.3f MB", total,
                       total / 1024. / 1024.);
    }
  }
  ...
  void PostInitEncoder() {
    ...
    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("model type: %s", ToString(model_type_).c_str());
    }
    ...
    if (mel_shape[0] != 1) {
      EDGEVOX_ONNX_LOGE("It supports only batch size == 1. Given: %d",
  ...
  void PostInitDecoder() {
    ...
    if (input_shapes.size() != expected_num_inputs) {
      EDGEVOX_ONNX_LOGE("Expect %d inputs. Actual: %d", expected_num_inputs,
    ...
    if (s[0] != 1) {
      EDGEVOX_ONNX_LOGE("Support only batch size 1. Given: %d",
    ...
    if (s[2] != n_text_state_) {
      EDGEVOX_ONNX_LOGE("Expect n_text_state_ %d. Given: %d", n_text_state_,
                       static_cast<int32_t>(s[2]));
      EDGEVOX_ONNX_EXIT(-1);
    }

    if (config_.debug) {
      EDGEVOX_ONNX_LOGE("n_text_ctx_: %d", n_text_ctx_);
    }
