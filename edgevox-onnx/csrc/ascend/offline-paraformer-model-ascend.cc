// edgevox-onnx/csrc/ascend/offline-paraformer-model-ascend.cc
//
...
class OfflineParaformerModelAscend::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config) : config_(config) {
    ...
    if (filenames.size() != 3) {
      EDGEVOX_ONNX_LOGE("Invalid paraformer ascend NPU model '%s'",
 ...
 private:
  void RunEncoder(std::vector<float> features) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlExecute for encoder");
  }
  ...
  void RunPredictor(int32_t num_frames) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlExecute for predictor");
  }
  ...
  void RunDecoder(int32_t num_frames, std::vector<float> acoustic_embedding) {
    ...
    EDGEVOX_ONNX_ASCEND_CHECK(ret, "Failed to call aclmdlExecute for decoder");
  }
