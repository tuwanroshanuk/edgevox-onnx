...
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct EdgevoxOnnxGeneratedAudio {
...
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct EdgevoxOnnxGenerationConfig {
    pub silence_scale: c_float,
    pub speed: c_float,
    pub sid: i32,
...
pub type EdgevoxOnnxGeneratedAudioProgressCallbackWithArg =
    Option<unsafe extern "C" fn(samples: *const f32, n: i32, progress: c_float, arg: *mut c_void) -> i32>;

#[repr(C)]
pub struct EdgevoxOnnxOfflineTts {
    _private: [u8; 0],
}

extern "C" {
    pub fn EdgevoxOnnxCreateOfflineTts(
        config: *const OfflineTtsConfig,
    ) -> *const EdgevoxOnnxOfflineTts;

    pub fn EdgevoxOnnxDestroyOfflineTts(tts: *const EdgevoxOnnxOfflineTts);

    pub fn EdgevoxOnnxOfflineTtsSampleRate(tts: *const EdgevoxOnnxOfflineTts) -> i32;

    pub fn EdgevoxOnnxOfflineTtsNumSpeakers(tts: *const EdgevoxOnnxOfflineTts) -> i32;

    pub fn EdgevoxOnnxOfflineTtsGenerateWithConfig(
        tts: *const EdgevoxOnnxOfflineTts,
        text: *const c_char,
        config: *const EdgevoxOnnxGenerationConfig,
        callback: EdgevoxOnnxGeneratedAudioProgressCallbackWithArg,
        arg: *mut c_void,
    ) -> *const EdgevoxOnnxGeneratedAudio;

    pub fn EdgevoxOnnxDestroyOfflineTtsGeneratedAudio(p: *const EdgevoxOnnxGeneratedAudio);
}
