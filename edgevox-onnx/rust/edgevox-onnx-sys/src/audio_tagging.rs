...
extern "C" {
    pub fn EdgevoxOnnxCreateAudioTagging(config: *const AudioTaggingConfig) -> *const AudioTagging;

    pub fn EdgevoxOnnxDestroyAudioTagging(tagger: *const AudioTagging);

    pub fn EdgevoxOnnxAudioTaggingCreateOfflineStream(
        tagger: *const AudioTagging,
    ) -> *const crate::offline_asr::OfflineStream;

    pub fn EdgevoxOnnxAudioTaggingCompute(
        tagger: *const AudioTagging,
        s: *const crate::offline_asr::OfflineStream,
        top_k: i32,
    ) -> *const *const AudioEvent;

    pub fn EdgevoxOnnxAudioTaggingFreeResults(p: *const *const AudioEvent);
}
