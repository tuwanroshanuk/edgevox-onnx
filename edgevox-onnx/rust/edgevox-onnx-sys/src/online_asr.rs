...
extern "C" {
    pub fn EdgevoxOnnxCreateOnlineRecognizer(
        config: *const OnlineRecognizerConfig,
    ) -> *const OnlineRecognizer;

    pub fn EdgevoxOnnxDestroyOnlineRecognizer(recognizer: *const OnlineRecognizer);

    pub fn EdgevoxOnnxCreateOnlineStream(recognizer: *const OnlineRecognizer)
        -> *const OnlineStream;

    pub fn EdgevoxOnnxCreateOnlineStreamWithHotwords(
        recognizer: *const OnlineRecognizer,
        hotwords: *const c_char,
    ) -> *const OnlineStream;

    pub fn EdgevoxOnnxDestroyOnlineStream(stream: *const OnlineStream);

    pub fn EdgevoxOnnxOnlineStreamAcceptWaveform(
    ...
    pub fn EdgevoxOnnxIsOnlineStreamReady(
        recognizer: *const OnlineRecognizer,
        stream: *const OnlineStream,
    ) -> i32;

    pub fn EdgevoxOnnxDecodeOnlineStream(
        recognizer: *const OnlineRecognizer,
        stream: *const OnlineStream,
    );

    pub fn EdgevoxOnnxDecodeMultipleOnlineStreams(
        recognizer: *const OnlineRecognizer,
        streams: *const *const OnlineStream,
        n: i32,
    );

    pub fn EdgevoxOnnxGetOnlineStreamResultAsJson(
        recognizer: *const OnlineRecognizer,
        stream: *const OnlineStream,
    ) -> *const c_char;

    pub fn EdgevoxOnnxDestroyOnlineStreamResultJson(s: *const c_char);

    pub fn EdgevoxOnnxOnlineStreamReset(
        recognizer: *const OnlineRecognizer,
        stream: *const OnlineStream,
    );

    pub fn EdgevoxOnnxOnlineStreamInputFinished(stream: *const OnlineStream);

    pub fn EdgevoxOnnxOnlineStreamSetOption(
        stream: *const OnlineStream,
        key: *const c_char,
        value: *const c_char,
    );

    pub fn EdgevoxOnnxOnlineStreamGetOption(
        stream: *const OnlineStream,
        key: *const c_char,
    ) -> *const c_char;

    pub fn EdgevoxOnnxOnlineStreamHasOption(
        stream: *const OnlineStream,
        key: *const c_char,
    ) -> i32;

    pub fn EdgevoxOnnxOnlineStreamIsEndpoint(
        recognizer: *const OnlineRecognizer,
        stream: *const OnlineStream,
    ) -> i32;
}
