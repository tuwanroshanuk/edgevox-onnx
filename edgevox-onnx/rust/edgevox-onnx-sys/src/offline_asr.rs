...
extern "C" {
    pub fn EdgevoxOnnxCreateOfflineRecognizer(
        config: *const OfflineRecognizerConfig,
    ) -> *const OfflineRecognizer;

    pub fn EdgevoxOnnxDestroyOfflineRecognizer(recognizer: *const OfflineRecognizer);

    pub fn EdgevoxOnnxCreateOfflineStream(
        recognizer: *const OfflineRecognizer,
    ) -> *const OfflineStream;

    pub fn EdgevoxOnnxCreateOfflineStreamWithHotwords(
        recognizer: *const OfflineRecognizer,
        hotwords: *const c_char,
    ) -> *const OfflineStream;

    pub fn EdgevoxOnnxDestroyOfflineStream(stream: *const OfflineStream);

    pub fn EdgevoxOnnxAcceptWaveformOffline(
    ...
    pub fn EdgevoxOnnxOfflineStreamSetOption(
        stream: *const OfflineStream,
        key: *const c_char,
        value: *const c_char,
    );

    pub fn EdgevoxOnnxOfflineStreamGetOption(
        stream: *const OfflineStream,
        key: *const c_char,
    ) -> *const c_char;

    pub fn EdgevoxOnnxOfflineStreamHasOption(
        stream: *const OfflineStream,
        key: *const c_char,
    ) -> i32;

    pub fn EdgevoxOnnxDecodeOfflineStream(
        recognizer: *const OfflineRecognizer,
        stream: *const OfflineStream,
    );

    pub fn EdgevoxOnnxDecodeMultipleOfflineStreams(
        recognizer: *const OfflineRecognizer,
        streams: *const *const OfflineStream,
        n: i32,
    );

    pub fn EdgevoxOnnxGetOfflineStreamResultAsJson(stream: *const OfflineStream) -> *const c_char;

    pub fn EdgevoxOnnxDestroyOfflineStreamResultJson(s: *const c_char);
}
