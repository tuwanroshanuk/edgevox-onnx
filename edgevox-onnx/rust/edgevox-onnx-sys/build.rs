...
fn try_main() -> Result<(), DynError> {
    println!("cargo:rerun-if-env-changed=EDGEVOX_ONNX_LIB_DIR");
    println!("cargo:rerun-if-env-changed=EDGEVOX_ONNX_ARCHIVE_DIR");
...
fn resolve_lib_dir(
    ...
    if let Some(path) = env::var_os("EDGEVOX_ONNX_LIB_DIR") {
        let path = PathBuf::from(path);
        if !path.is_dir() {
            return Err(format!(
                "EDGEVOX_ONNX_LIB_DIR does not exist or is not a directory: {}",
                path.display()
            )
            .into());
        }
...
fn download_prebuilt_libs(
    ...
    let cache_root = target_dir_from_out_dir(&out_dir)?.join("edgevox-onnx-prebuilt");
    ...
    if !archive_path.is_file() {
        if let Some(local_archive_dir) = env::var_os("EDGEVOX_ONNX_ARCHIVE_DIR") {
            let local_archive_path = PathBuf::from(local_archive_dir).join(&archive_name);
            if !local_archive_path.is_file() {
                return Err(format!(
                    "EDGEVOX_ONNX_ARCHIVE_DIR does not contain expected archive: {}",
            ...
            eprintln!("Downloading edgevox-onnx libs from {url}");
            ...
            let response = ureq::builder()
                ...
                .get(&url)
                .call()
                .map_err(|e| format!("Failed to download edgevox-onnx archive from {url}: {e}"))?;
    ...
    if !lib_dir.is_dir() {
        ...
        if android_lib_dir.is_dir() {
            eprintln!("Downloaded edgevox-onnx Android libs to {}", android_lib_dir.display());
            return Ok(android_lib_dir);
        }
        return Err(format!(
    ...
    eprintln!("Downloaded edgevox-onnx libs to {}", extracted_dir.display());
...
fn archive_name(
    ...
    let name = match (link_mode, target_os, target_arch) {
        (LinkMode::Static, "linux", "x86_64") => {
            format!("edgevox-onnx-v{version}-linux-x64-static-lib.tar.bz2")
        }
        (LinkMode::Static, "linux", "aarch64") => {
            format!("edgevox-onnx-v{version}-linux-aarch64-static-lib.tar.bz2")
        }
        (LinkMode::Static, "macos", "x86_64") => {
            format!("edgevox-onnx-v{version}-osx-x64-static-lib.tar.bz2")
        }
        (LinkMode::Static, "macos", "aarch64") => {
            format!("edgevox-onnx-v{version}-osx-arm64-static-lib.tar.bz2")
        }
        (LinkMode::Static, "windows", "x86_64") => {
            format!("edgevox-onnx-v{version}-win-x64-static-MT-Release-lib.tar.bz2")
        }
        (LinkMode::Shared, "linux", "x86_64") => {
            format!("edgevox-onnx-v{version}-linux-x64-shared-lib.tar.bz2")
        }
        (LinkMode::Shared, "linux", "aarch64") => {
            format!("edgevox-onnx-v{version}-linux-aarch64-shared-cpu-lib.tar.bz2")
        }
        (LinkMode::Shared, "macos", "x86_64") => {
            format!("edgevox-onnx-v{version}-osx-x64-shared-lib.tar.bz2")
        }
        (LinkMode::Shared, "macos", "aarch64") => {
            format!("edgevox-onnx-v{version}-osx-arm64-shared-lib.tar.bz2")
        }
        (LinkMode::Shared, "windows", "x86_64") => {
            format!("edgevox-onnx-v{version}-win-x64-shared-MT-Release-lib.tar.bz2")
        }
        // Android: one archive with all ABIs under jniLibs/{abi}/.
        (LinkMode::Shared, "android", "aarch64" | "arm" | "x86" | "x86_64") => {
            format!("edgevox-onnx-v{version}-android.tar.bz2")
        }
        _ => return Err(format!(
            "Unsupported target for edgevox-onnx prebuilt libs: os={target_os}, arch={target_arch}"
        )
        .into()),
    };
...
fn emit_shared_link_directives() {
    println!("cargo:rustc-link-lib=dylib=edgevox-onnx-c-api");
    println!("cargo:rustc-link-lib=dylib=onnxruntime");
}

fn emit_static_link_directives(target_os: &str) {
    for lib in EDGEVOX_ONNX_STATIC_LIBS {
