...
//!
//! ```no_run
//! use edgevox_onnx::{OnlinePunctuation, OnlinePunctuationConfig, OnlinePunctuationModelConfig};
//!
...
impl OnlinePunctuation {
    pub fn create(config: &OnlinePunctuationConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOnlinePunctuation(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn add_punctuation(&self, text: &str) -> Option<String> {
        ...
        unsafe {
            let p = sys::EdgevoxOnnxOnlinePunctuationAddPunct(self.ptr, text.as_ptr());
            if p.is_null() {
                return None;
            }
            ...
            let ans = CStr::from_ptr(p)
                .to_string_lossy()
                .into_owned();
            sys::EdgevoxOnnxOnlinePunctuationFreeText(p);
            Some(ans)
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroyOnlinePunctuation(self.ptr);
            }
        }
    }
}
