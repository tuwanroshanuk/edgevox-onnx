...
//!
//! ```no_run
//! use edgevox_onnx::{KeywordSpotter, KeywordSpotterConfig, Wave};
//!
...
impl KeywordSpotter {
    ...
    pub fn create(config: &KeywordSpotterConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateKeywordSpotter(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn create_stream(&self) -> OnlineStream {
        let ptr = unsafe { sys::EdgevoxOnnxCreateKeywordStream(self.ptr) };
        OnlineStream { ptr }
    }
    ...
    pub fn create_stream_with_keywords(&self, keywords: &str) -> OnlineStream {
        ...
        let ptr =
            unsafe { sys::EdgevoxOnnxCreateKeywordStreamWithKeywords(self.ptr, keywords.as_ptr()) };
        OnlineStream { ptr }
    }

    /// Return `true` if `stream` has enough audio for another decode step.
    pub fn is_ready(&self, stream: &OnlineStream) -> bool {
        unsafe { sys::EdgevoxOnnxIsKeywordStreamReady(self.ptr, stream.ptr) != 0 }
    }

    /// Decode one incremental step for `stream`.
    pub fn decode(&self, stream: &OnlineStream) {
        unsafe { sys::EdgevoxOnnxDecodeKeywordStream(self.ptr, stream.ptr) }
    }
    ...
    pub fn decode_multiple_streams(&self, streams: &[&OnlineStream]) {
        let ptrs: Vec<*const sys::OnlineStream> = streams
            ...
            .collect();
        unsafe {
            sys::EdgevoxOnnxDecodeMultipleKeywordStreams(self.ptr, ptrs.as_ptr(), ptrs.len() as i32)
        }
    }

    /// Reset the detector state for `stream`.
    pub fn reset(&self, stream: &OnlineStream) {
        unsafe { sys::EdgevoxOnnxResetKeywordStream(self.ptr, stream.ptr) }
    }

    /// Get the structured keyword spotting result for `stream`.
    pub fn get_result(&self, stream: &OnlineStream) -> Option<KeywordResult> {
        unsafe {
            let p = sys::EdgevoxOnnxGetKeywordResult(self.ptr, stream.ptr);
            if p.is_null() {
                return None;
            }
            ...
            sys::EdgevoxOnnxDestroyKeywordResult(p);
            Some(ans)
        }
    }
    ...
    pub fn get_result_as_json(&self, stream: &OnlineStream) -> Option<String> {
        unsafe {
            let p = sys::EdgevoxOnnxGetKeywordResultAsJson(self.ptr, stream.ptr);
            if p.is_null() {
                return None;
            }
            ...
            let ans = CStr::from_ptr(p)
                .to_string_lossy()
                .into_owned();
            sys::EdgevoxOnnxFreeKeywordResultJson(p);
            Some(ans)
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroyKeywordSpotter(self.ptr);
            }
        }
    }
}
