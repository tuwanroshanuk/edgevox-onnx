...
impl Wave {
    ...
    pub fn read(filename: &str) -> Option<Self> {
        ...
        let wave_ptr = unsafe { sys::EdgevoxOnnxReadWave(c_filename.as_ptr()) };
        if wave_ptr.is_null() {
            None
        } else {
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxFreeWave(self.inner);
            }
        }
    }
}
