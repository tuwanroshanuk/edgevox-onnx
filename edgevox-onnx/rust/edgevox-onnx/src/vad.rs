...
impl CircularBuffer {
    ...
    pub fn new(capacity: i32) -> Option<Self> {
        let ptr = unsafe { sys::EdgevoxOnnxCreateCircularBuffer(capacity) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn push(&self, samples: &[f32]) {
        unsafe {
            sys::EdgevoxOnnxCircularBufferPush(self.ptr, samples.as_ptr(), samples.len() as i32)
        }
    }
    ...
    pub fn get(&self, start_index: i32, n: i32) -> Vec<f32> {
        unsafe {
            let p = sys::EdgevoxOnnxCircularBufferGet(self.ptr, start_index, n);
            if p.is_null() {
                return vec![];
            }
            let slice = slice::from_raw_parts(p, n as usize);
            let result = slice.to_vec();
            sys::EdgevoxOnnxCircularBufferFree(p);
            result
        }
    }
    ...
    pub fn pop(&self, n: i32) {
        unsafe { sys::EdgevoxOnnxCircularBufferPop(self.ptr, n) }
    }

    /// Return the number of samples currently stored.
    pub fn size(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxCircularBufferSize(self.ptr) }
    }

    /// Return the logical head position.
    pub fn head(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxCircularBufferHead(self.ptr) }
    }

    /// Clear the buffer.
    pub fn reset(&self) {
        unsafe { sys::EdgevoxOnnxCircularBufferReset(self.ptr) }
    }
}

impl Drop for CircularBuffer {
    fn drop(&mut self) {
        unsafe { sys::EdgevoxOnnxDestroyCircularBuffer(self.ptr) }
    }
}
...
impl Drop for SpeechSegment {
    fn drop(&mut self) {
        unsafe { sys::EdgevoxOnnxDestroySpeechSegment(self.ptr) }
    }
}
...
impl VoiceActivityDetector {
    ...
    pub fn create(config: &VadModelConfig, buffer_size_in_seconds: f32) -> Option<Self> {
        ...
        let ptr = unsafe {
            sys::EdgevoxOnnxCreateVoiceActivityDetector(&sys_config, buffer_size_in_seconds)
        };
    ...
    pub fn accept_waveform(&self, samples: &[f32]) {
        unsafe {
            sys::EdgevoxOnnxVoiceActivityDetectorAcceptWaveform(
                self.ptr,
                samples.as_ptr(),
                samples.len() as i32,
            )
    ...
    pub fn is_empty(&self) -> bool {
        unsafe { sys::EdgevoxOnnxVoiceActivityDetectorEmpty(self.ptr) != 0 }
    }

    /// Return `true` if speech is currently being detected.
    pub fn detected(&self) -> bool {
        unsafe { sys::EdgevoxOnnxVoiceActivityDetectorDetected(self.ptr) != 0 }
    }

    /// Drop the front speech segment, if any.
    pub fn pop(&self) {
        unsafe { sys::EdgevoxOnnxVoiceActivityDetectorPop(self.ptr) }
    }

    /// Remove all queued segments.
    pub fn clear(&self) {
        unsafe { sys::EdgevoxOnnxVoiceActivityDetectorClear(self.ptr) }
    }
    ...
    pub fn front(&self) -> Option<SpeechSegment> {
        ...
        unsafe {
            let ptr = sys::EdgevoxOnnxVoiceActivityDetectorFront(self.ptr);
            if ptr.is_null() {
                None
            } else {
    ...
    pub fn reset(&self) {
        unsafe { sys::EdgevoxOnnxVoiceActivityDetectorReset(self.ptr) }
    }

    /// Flush any buffered trailing speech into the output queue.
    pub fn flush(&self) {
        unsafe { sys::EdgevoxOnnxVoiceActivityDetectorFlush(self.ptr) }
    }
}

impl Drop for VoiceActivityDetector {
    fn drop(&mut self) {
        unsafe { sys::EdgevoxOnnxDestroyVoiceActivityDetector(self.ptr) }
    }
}
