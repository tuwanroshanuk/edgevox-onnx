...
/// Return the edgevox-onnx version string compiled into the native library.
pub fn version() -> &'static str {
    let ptr = unsafe { sys::EdgevoxOnnxGetVersionStr() };
    c_str_to_static_str(ptr)
}

/// Return the Git SHA1 of the native library build.
pub fn git_sha1() -> &'static str {
    let ptr = unsafe { sys::EdgevoxOnnxGetGitSha1() };
    c_str_to_static_str(ptr)
}

/// Return the Git date of the native library build.
pub fn git_date() -> &'static str {
    let ptr = unsafe { sys::EdgevoxOnnxGetGitDate() };
    c_str_to_static_str(ptr)
}
...
pub fn file_exists(filename: &str) -> bool {
    ...
    unsafe { sys::EdgevoxOnnxFileExists(cstr.as_ptr()) != 0 }
}
