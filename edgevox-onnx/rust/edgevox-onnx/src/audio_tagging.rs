...
impl AudioTagging {
    ...
    pub fn create(config: &AudioTaggingConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateAudioTagging(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn create_stream(&self) -> AudioTaggingOfflineStream {
        let ptr = unsafe { sys::EdgevoxOnnxAudioTaggingCreateOfflineStream(self.ptr) };
    ...
    pub fn compute(&self, stream: &AudioTaggingOfflineStream, top_k: i32) -> Vec<AudioEvent> {
        unsafe {
            let p = sys::EdgevoxOnnxAudioTaggingCompute(self.ptr, stream.ptr, top_k);
            if p.is_null() {
                return Vec::new();
            }
            ...
            sys::EdgevoxOnnxAudioTaggingFreeResults(p);
            ans
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroyAudioTagging(self.ptr);
            }
        }
    }
}
