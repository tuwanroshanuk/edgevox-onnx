...
impl SpokenLanguageIdentification {
    ...
    pub fn create(config: &SpokenLanguageIdentificationConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateSpokenLanguageIdentification(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn create_stream(&self) -> OfflineStream {
        let ptr =
            unsafe { sys::EdgevoxOnnxSpokenLanguageIdentificationCreateOfflineStream(self.ptr) };
        OfflineStream { ptr }
    }
    ...
    pub fn compute(&self, stream: &OfflineStream) -> Option<SpokenLanguageIdentificationResult> {
        unsafe {
            let p = sys::EdgevoxOnnxSpokenLanguageIdentificationCompute(self.ptr, stream.ptr);
            if p.is_null() {
                return None;
            }
            ...
            sys::EdgevoxOnnxDestroySpokenLanguageIdentificationResult(p);
            Some(ans)
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroySpokenLanguageIdentification(self.ptr);
            }
        }
    }
}
