...
impl OfflineSpeakerDiarization {
    ...
    pub fn create(config: &OfflineSpeakerDiarizationConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOfflineSpeakerDiarization(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn sample_rate(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOfflineSpeakerDiarizationGetSampleRate(self.ptr) }
    }
    ...
    pub fn set_config(&self, config: &OfflineSpeakerDiarizationConfig) {
        let mut cstrings = Vec::new();
        let sys_config = config.to_sys(&mut cstrings);
        unsafe { sys::EdgevoxOnnxOfflineSpeakerDiarizationSetConfig(self.ptr, &sys_config) }
    }
    ...
    pub fn process(&self, samples: &[f32]) -> Option<OfflineSpeakerDiarizationResult> {
        let ptr = unsafe {
            sys::EdgevoxOnnxOfflineSpeakerDiarizationProcess(
                self.ptr,
                samples.as_ptr(),
                samples.len() as i32,
            )
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroyOfflineSpeakerDiarization(self.ptr);
            }
        }
    }
}
...
impl OfflineSpeakerDiarizationResult {
    ...
    pub fn num_speakers(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOfflineSpeakerDiarizationResultGetNumSpeakers(self.ptr) }
    }

    /// Return the number of diarization segments.
    pub fn num_segments(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOfflineSpeakerDiarizationResultGetNumSegments(self.ptr) }
    }
    ...
    pub fn sort_by_start_time(&self) -> Vec<OfflineSpeakerDiarizationSegment> {
        ...
        unsafe {
            let p = sys::EdgevoxOnnxOfflineSpeakerDiarizationResultSortByStartTime(self.ptr);
            if p.is_null() {
                return Vec::new();
            }
            ...
            let segments = slice::from_raw_parts(p, n as usize)
                ...
                .collect::<Vec<_>>();
            sys::EdgevoxOnnxOfflineSpeakerDiarizationDestroySegment(p);
            segments
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxOfflineSpeakerDiarizationDestroyResult(self.ptr);
            }
        }
    }
}
