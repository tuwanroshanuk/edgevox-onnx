...
//!
//! ```no_run
//! use edgevox_onnx::{OfflinePunctuation, OfflinePunctuationConfig};
//!
//! let mut config = OfflinePunctuationConfig::default();
//! config.model.ct_transformer = Some("./edgevox-onnx-offline-punctuation/model.onnx".into());
//!
...
impl OfflinePunctuation {
    ...
    pub fn create(config: &OfflinePunctuationConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOfflinePunctuation(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroyOfflinePunctuation(self.ptr);
            }
        }
    }
}
