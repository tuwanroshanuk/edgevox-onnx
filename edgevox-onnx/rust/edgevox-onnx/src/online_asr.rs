...
//!
//! ```no_run
//! use edgevox_onnx::{OnlineRecognizer, OnlineRecognizerConfig, Wave};
//!
...
impl OnlineRecognizer {
    ...
    pub fn create(config: &OnlineRecognizerConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOnlineRecognizer(&sys_config) };
    ...
    pub fn create_stream(&self) -> OnlineStream {
        let ptr = unsafe { sys::EdgevoxOnnxCreateOnlineStream(self.ptr) };
        OnlineStream { ptr }
    }
    ...
    pub fn create_stream_with_hotwords(&self, hotwords: &str) -> OnlineStream {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOnlineStreamWithHotwords(self.ptr, c.as_ptr()) };
        OnlineStream { ptr }
    }

    /// Decode one step for `stream`.
    pub fn decode(&self, stream: &OnlineStream) {
        unsafe { sys::EdgevoxOnnxDecodeOnlineStream(self.ptr, stream.ptr) }
    }
    ...
    pub fn get_result(&self, stream: &OnlineStream) -> Option<RecognizerResult> {
        unsafe {
            ...
            let s = CStr::from_ptr(cstr)
                .to_string_lossy()
                .into_owned();
            sys::EdgevoxOnnxDestroyOnlineStreamResultJson(cstr);
...
impl Drop for OnlineRecognizer {
    fn drop(&mut self) {
        unsafe {
            sys::EdgevoxOnnxDestroyOnlineRecognizer(self.ptr);
        }
    }
}
...
impl OnlineStream {
    ...
    pub fn accept_waveform(&self, sample_rate: i32, samples: &[f32]) {
        unsafe {
            sys::EdgevoxOnnxOnlineStreamAcceptWaveform(
                self.ptr,
                sample_rate,
                samples.as_ptr(),
                samples.len() as i32,
    ...
    pub fn input_finished(&self) {
        unsafe { sys::EdgevoxOnnxOnlineStreamInputFinished(self.ptr) }
    }

    pub fn set_option(&self, key: &str, value: &str) {
        let key = CString::new(key).unwrap();
        let value = CString::new(value).unwrap();
        unsafe { sys::EdgevoxOnnxOnlineStreamSetOption(self.ptr, key.as_ptr(), value.as_ptr()) }
    }

    pub fn get_option(&self, key: &str) -> String {
        let key = CString::new(key).unwrap();
        unsafe {
            let p = sys::EdgevoxOnnxOnlineStreamGetOption(self.ptr, key.as_ptr());
            if p.is_null() {
                String::new()
            } else {
                CStr::from_ptr(p)
    ...
    pub fn has_option(&self, key: &str) -> bool {
        let key = CString::new(key).unwrap();
        unsafe { sys::EdgevoxOnnxOnlineStreamHasOption(self.ptr, key.as_ptr()) != 0 }
    }
}

impl Drop for OnlineStream {
    fn drop(&mut self) {
        unsafe { sys::EdgevoxOnnxDestroyOnlineStream(self.ptr) }
    }
}
