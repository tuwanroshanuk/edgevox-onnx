...
impl OfflineRecognizer {
    ...
    pub fn create(config: &OfflineRecognizerConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOfflineRecognizer(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn create_stream(&self) -> OfflineStream {
        let ptr = unsafe { sys::EdgevoxOnnxCreateOfflineStream(self.ptr) };
        OfflineStream { ptr }
    }
    ...
    pub fn create_stream_with_hotwords(&self, hotwords: &str) -> OfflineStream {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOfflineStreamWithHotwords(self.ptr, c.as_ptr()) };
        OfflineStream { ptr }
    }

    /// Decode one stream.
    pub fn decode(&self, stream: &OfflineStream) {
        unsafe { sys::EdgevoxOnnxDecodeOfflineStream(self.ptr, stream.ptr) }
    }
    ...
    pub fn decode_multiple_streams(&self, streams: &[&OfflineStream]) {
        let ptrs: Vec<*const sys::OfflineStream> = streams
            ...
            .collect();
        unsafe {
            sys::EdgevoxOnnxDecodeMultipleOfflineStreams(self.ptr, ptrs.as_ptr(), ptrs.len() as i32)
        }
    }
}
...
impl Drop for OfflineRecognizer {
    fn drop(&mut self) {
        unsafe {
            sys::EdgevoxOnnxDestroyOfflineRecognizer(self.ptr);
        }
    }
}
...
impl OfflineStream {
    ...
    pub fn accept_waveform(&self, sample_rate: i32, samples: &[f32]) {
        unsafe {
            sys::EdgevoxOnnxAcceptWaveformOffline(
                self.ptr,
                sample_rate,
                samples.as_ptr(),
                samples.len() as i32,
    ...
    pub fn get_result(&self) -> Option<OfflineRecognizerResult> {
        unsafe {
            let cstr = sys::EdgevoxOnnxGetOfflineStreamResultAsJson(self.ptr);
            if cstr.is_null() {
                return None;
            }
            let s = CStr::from_ptr(cstr)
                .to_string_lossy()
                .into_owned();
            sys::EdgevoxOnnxDestroyOfflineStreamResultJson(cstr);
    ...
    pub fn set_option(&self, key: &str, value: &str) {
        ...
        unsafe { sys::EdgevoxOnnxOfflineStreamSetOption(self.ptr, key.as_ptr(), value.as_ptr()) }
    }

    pub fn get_option(&self, key: &str) -> String {
        let key = CString::new(key).unwrap();
        unsafe {
            let p = sys::EdgevoxOnnxOfflineStreamGetOption(self.ptr, key.as_ptr());
            if p.is_null() {
                String::new()
            } else {
                CStr::from_ptr(p)
    ...
    pub fn has_option(&self, key: &str) -> bool {
        let key = CString::new(key).unwrap();
        unsafe { sys::EdgevoxOnnxOfflineStreamHasOption(self.ptr, key.as_ptr()) != 0 }
    }
}

impl Drop for OfflineStream {
    fn drop(&mut self) {
        unsafe { sys::EdgevoxOnnxDestroyOfflineStream(self.ptr) }
    }
}
