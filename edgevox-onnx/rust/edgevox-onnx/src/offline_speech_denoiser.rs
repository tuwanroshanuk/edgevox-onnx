...
impl DenoisedAudio {
    pub(crate) fn from_ptr(ptr: *const sys::DenoisedAudio) -> Self {
        ...
        unsafe {
            ...
            sys::EdgevoxOnnxDestroyDenoisedAudio(ptr);
            Self {
                samples,
                sample_rate,
            }
...
impl OfflineSpeechDenoiser {
    ...
    pub fn create(config: &OfflineSpeechDenoiserConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOfflineSpeechDenoiser(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn run(&self, samples: &[f32], sample_rate: i32) -> DenoisedAudio {
        ...
        let ptr = unsafe {
            sys::EdgevoxOnnxOfflineSpeechDenoiserRun(
                self.ptr,
                samples_ptr,
                samples.len() as i32,
                sample_rate,
    ...
    pub fn sample_rate(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOfflineSpeechDenoiserGetSampleRate(self.ptr) }
    }
}

impl Drop for OfflineSpeechDenoiser {
    fn drop(&mut self) {
        unsafe { sys::EdgevoxOnnxDestroyOfflineSpeechDenoiser(self.ptr) }
    }
}
