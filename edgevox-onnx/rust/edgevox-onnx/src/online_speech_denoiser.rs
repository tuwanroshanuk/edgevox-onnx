...
unsafe impl Send for OnlineSpeechDenoiser {}
unsafe impl Sync for OnlineSpeechDenoiser {}
...
impl OnlineSpeechDenoiser {
    ...
    pub fn create(config: &OnlineSpeechDenoiserConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOnlineSpeechDenoiser(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    pub fn run(&self, samples: &[f32], sample_rate: i32) -> DenoisedAudio {
        ...
        let ptr = unsafe {
            sys::EdgevoxOnnxOnlineSpeechDenoiserRun(
                self.ptr,
                samples_ptr,
                samples.len() as i32,
                sample_rate,
    ...
    pub fn flush(&self) -> DenoisedAudio {
        let ptr = unsafe { sys::EdgevoxOnnxOnlineSpeechDenoiserFlush(self.ptr) };
        DenoisedAudio::from_ptr(ptr)
    }

    /// Reset the streaming state.
    pub fn reset(&self) {
        unsafe { sys::EdgevoxOnnxOnlineSpeechDenoiserReset(self.ptr) }
    }

    /// Return the model sample rate expected by this denoiser.
    pub fn sample_rate(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOnlineSpeechDenoiserGetSampleRate(self.ptr) }
    }

    /// Return the preferred input frame shift, in samples.
    pub fn frame_shift_in_samples(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOnlineSpeechDenoiserGetFrameShiftInSamples(self.ptr) }
    }
}

impl Drop for OnlineSpeechDenoiser {
    fn drop(&mut self) {
        unsafe { sys::EdgevoxOnnxDestroyOnlineSpeechDenoiser(self.ptr) }
    }
}
