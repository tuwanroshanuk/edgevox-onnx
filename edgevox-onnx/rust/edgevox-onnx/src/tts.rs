...
//! };
//! let audio = tts
//!     .generate_with_config("Hello from edgevox-onnx", &generation_config, None)
//!     .expect("generate");
...
impl OfflineTtsZipvoiceModelConfig {
    fn to_sys(&self, cstrings: &mut Vec<CString>) -> sys::OfflineTtsZipvoiceModelConfig {
        sys::OfflineTtsZipvoiceModelConfig {
            ...
            data_dir: to_c_ptr(&self.data_dir, cstrings),
            lexicon: to_c_ptr(&self.lexicon, cstrings),
            espeak_voice: to_c_ptr(&self.espeak_voice, cstrings),
...
pub struct GeneratedAudio {
    ptr: *const sys::EdgevoxOnnxGeneratedAudio,
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroyOfflineTtsGeneratedAudio(self.ptr);
            }
        }
    }
}
...
pub struct OfflineTts {
    ptr: *const sys::EdgevoxOnnxOfflineTts,
}
...
impl OfflineTts {
    ...
    pub fn create(config: &OfflineTtsConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateOfflineTts(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr })
    ...
    /// Return the output sample rate in Hz.
    pub fn sample_rate(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOfflineTtsSampleRate(self.ptr) }
    }

    /// Return the number of built-in speakers reported by the model.
    pub fn num_speakers(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxOfflineTtsNumSpeakers(self.ptr) }
    }
        ...
        F: FnMut(&[f32], f32) -> bool + 'static,
    {
        ...
        let sys_gen_config = sys::EdgevoxOnnxGenerationConfig {
        ...
        let (c_callback, c_arg): (
            sys::EdgevoxOnnxGeneratedAudioProgressCallbackWithArg,
            *mut c_void,
        ...
        let audio_ptr = unsafe {
            sys::EdgevoxOnnxOfflineTtsGenerateWithConfig(
                self.ptr,
                c_text.as_ptr(),
                &sys_gen_config,
                c_callback,
...
impl Drop for OfflineTts {
    fn drop(&mut self) {
        unsafe {
            sys::EdgevoxOnnxDestroyOfflineTts(self.ptr);
        }
    }
}
