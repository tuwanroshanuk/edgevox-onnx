...
// SAFETY: The edgevox-onnx C library is thread-safe for single-object usage.
...
impl SpeakerEmbeddingExtractor {
    ...
    pub fn create(config: &SpeakerEmbeddingExtractorConfig) -> Option<Self> {
        ...
        let ptr = unsafe { sys::EdgevoxOnnxCreateSpeakerEmbeddingExtractor(&sys_config) };
        if ptr.is_null() {
            None
        } else {
            let dim = unsafe { sys::EdgevoxOnnxSpeakerEmbeddingExtractorDim(ptr) };
            Some(Self { ptr, dim })
        }
    }
    ...
    pub fn create_stream(&self) -> Option<OnlineStream> {
        let ptr = unsafe { sys::EdgevoxOnnxSpeakerEmbeddingExtractorCreateStream(self.ptr) };
        if ptr.is_null() {
            None
        } else {
    ...
    pub fn is_ready(&self, stream: &OnlineStream) -> bool {
        unsafe { sys::EdgevoxOnnxSpeakerEmbeddingExtractorIsReady(self.ptr, stream.ptr) == 1 }
    }

    /// Compute the embedding for `stream`.
    pub fn compute(&self, stream: &OnlineStream) -> Option<Vec<f32>> {
        let p = unsafe {
            sys::EdgevoxOnnxSpeakerEmbeddingExtractorComputeEmbedding(self.ptr, stream.ptr)
        };
        if p.is_null() {
            None
        } else {
            let ans = unsafe { slice::from_raw_parts(p, self.dim as usize) }.to_vec();
            unsafe { sys::EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(p) };
            Some(ans)
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroySpeakerEmbeddingExtractor(self.ptr);
            }
        }
    }
}
...
impl SpeakerEmbeddingManager {
    ...
    pub fn create(dim: i32) -> Option<Self> {
        let ptr = unsafe { sys::EdgevoxOnnxCreateSpeakerEmbeddingManager(dim) };
        if ptr.is_null() {
            None
        } else {
            Some(Self { ptr, dim })
    ...
    pub fn add(&self, name: &str, embedding: &[f32]) -> bool {
        ...
        unsafe {
            sys::EdgevoxOnnxSpeakerEmbeddingManagerAdd(self.ptr, c_name.as_ptr(), embedding.as_ptr())
                == 1
        }
    }
    ...
    pub fn add_list(&self, name: &str, embeddings: &[Vec<f32>]) -> bool {
        ...
        unsafe {
            sys::EdgevoxOnnxSpeakerEmbeddingManagerAddList(self.ptr, c_name.as_ptr(), ptrs.as_ptr())
                == 1
        }
    }
    ...
    pub fn add_list_flattened(&self, name: &str, embeddings: &[f32]) -> bool {
        ...
        unsafe {
            sys::EdgevoxOnnxSpeakerEmbeddingManagerAddListFlattened(
                self.ptr,
                c_name.as_ptr(),
                embeddings.as_ptr(),
                n,
    ...
    pub fn remove(&self, name: &str) -> bool {
        ...
        unsafe { sys::EdgevoxOnnxSpeakerEmbeddingManagerRemove(self.ptr, c_name.as_ptr()) == 1 }
    }
    ...
    pub fn search(&self, embedding: &[f32], threshold: f32) -> Option<String> {
        ...
        unsafe {
            let p = sys::EdgevoxOnnxSpeakerEmbeddingManagerSearch(
                self.ptr,
                embedding.as_ptr(),
                threshold,
            );
            if p.is_null() {
                ...
                let ans = CStr::from_ptr(p)
                    .to_string_lossy()
                    .into_owned();
                sys::EdgevoxOnnxSpeakerEmbeddingManagerFreeSearch(p);
                Some(ans)
            }
        }
    }
    ...
    pub fn get_best_matches(
        ...
        unsafe {
            let r = sys::EdgevoxOnnxSpeakerEmbeddingManagerGetBestMatches(
                self.ptr,
                embedding.as_ptr(),
                threshold,
                n,
            ...
            let matches = slice::from_raw_parts(result.matches, result.count as usize)
                ...
                .collect::<Vec<_>>();
            sys::EdgevoxOnnxSpeakerEmbeddingManagerFreeBestMatches(r);
            matches
        }
    }
    ...
    pub fn verify(&self, name: &str, embedding: &[f32], threshold: f32) -> bool {
        ...
        unsafe {
            sys::EdgevoxOnnxSpeakerEmbeddingManagerVerify(
                self.ptr,
                c_name.as_ptr(),
                embedding.as_ptr(),
                threshold,
    ...
    pub fn contains(&self, name: &str) -> bool {
        ...
        unsafe { sys::EdgevoxOnnxSpeakerEmbeddingManagerContains(self.ptr, c_name.as_ptr()) == 1 }
    }

    pub fn num_speakers(&self) -> i32 {
        unsafe { sys::EdgevoxOnnxSpeakerEmbeddingManagerNumSpeakers(self.ptr) }
    }

    pub fn get_all_speakers(&self) -> Vec<String> {
        unsafe {
            let names = sys::EdgevoxOnnxSpeakerEmbeddingManagerGetAllSpeakers(self.ptr);
            if names.is_null() {
                return Vec::new();
            }
            ...
            sys::EdgevoxOnnxSpeakerEmbeddingManagerFreeAllSpeakers(names);
            ans
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if !self
                ...
                .is_null()
            {
                sys::EdgevoxOnnxDestroySpeakerEmbeddingManager(self.ptr);
            }
        }
    }
}
