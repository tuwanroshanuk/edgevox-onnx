# edgevox-onnx/python/tests/test_fast_clustering.py
#
...
class TestFastClustering(unittest.TestCase):
    def test_construct_by_num_clusters(self):
        config = edgevox_onnx.FastClusteringConfig(num_clusters=4)
        assert config.validate() is True

        print(config)

        clustering = edgevox_onnx.FastClustering(config)
        features = np.array(
            [
                [0.2, 0.3],  # cluster 0
                [0.3, -0.4],  # cluster 1
    ...
    def test_construct_by_threshold(self):
        config = edgevox_onnx.FastClusteringConfig(threshold=0.2)
        assert config.validate() is True
    ...
    def test_cluster_speaker_embeddings(self):
        ...
        extractor_config = edgevox_onnx.SpeakerEmbeddingExtractorConfig(
        ...
        extractor = edgevox_onnx.SpeakerEmbeddingExtractor(extractor_config)
        ...
        config = edgevox_onnx.FastClusteringConfig(num_clusters=3)
        #  config = edgevox_onnx.FastClusteringConfig(threshold=0.5)
        clustering = edgevox_onnx.FastClustering(config)
        labels = clustering(features)
