// edgevox-onnx/python/csrc/faked-alsa.cc
//
...
class FakedAlsa {
 public:
  explicit FakedAlsa(const char *) {
    EDGEVOX_ONNX_LOGE("This function is for Linux only.");
#if (EDGEVOX_ONNX_ENABLE_ALSA == 0) && (defined(__unix__) || defined(__unix))
    EDGEVOX_ONNX_LOGE(R"doc(
edgevox-onnx is compiled without alsa support. To enable that, please run
  (1) sudo apt-get install alsa-utils libasound2-dev
  (2) rebuild edgevox-onnx
)doc");
#endif
    exit(-1);
  }
