...
class OfflineRecognizer(
    ...
    companion object {
        init {
            System.loadLibrary("edgevox-onnx-jni")
        }
...
fun getOfflineModelConfig(type: Int): OfflineModelConfig? {
    when (type) {
        ...
        18 -> {
            val modelDir = "edgevox-onnx-small-zipformer-ru-2024-09-18"
            return OfflineModelConfig(
                transducer = OfflineTransducerModelConfig(
                    encoder = "$modelDir/encoder.int8.onnx",
                    decoder = "$modelDir/decoder.onnx",
        ...
        47 -> {
            val modelDir = "edgevox-onnx-wenetspeech-wu-u2pp-conformer-ctc-zh-int8-2026-02-03"
            return OfflineModelConfig(
                wenetCtc = OfflineWenetCtcModelConfig(
                    model = "$modelDir/model.int8.onnx",
                ),
        ...
        48 -> {
            val modelDir = "edgevox-onnx-wenetspeech-wu-u2pp-conformer-ctc-zh-2026-02-03"
            return OfflineModelConfig(
                wenetCtc = OfflineWenetCtcModelConfig(
                    model = "$modelDir/model.onnx",
                ),
