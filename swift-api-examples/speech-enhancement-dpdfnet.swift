...
func run() {
  ...
  var config = edgevoxOnnxOfflineSpeechDenoiserConfig(
    model: edgevoxOnnxOfflineSpeechDenoiserModelConfig(
      dpdfnet: edgevoxOnnxOfflineSpeechDenoiserDpdfNetModelConfig(model: model))
  )

  let sd = EdgevoxOnnxOfflineSpeechDenoiserWrapper(config: &config)
