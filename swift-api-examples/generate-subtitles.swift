...
func run() {
  ...
  var sileroVadConfig = edgevoxOnnxSileroVadModelConfig()
  var tenVadConfig = edgevoxOnnxTenVadModelConfig()

  var windowSize = 0

  if FileManager.default.fileExists(atPath: "./silero_vad.onnx") {
    sileroVadConfig = edgevoxOnnxSileroVadModelConfig(
    ...
    tenVadConfig = edgevoxOnnxTenVadModelConfig(
      model: "./ten-vad.onnx",
      threshold: 0.25,
      windowSize: 256
    )
  ...
  var vadModelConfig = edgevoxOnnxVadModelConfig(
    sileroVad: sileroVadConfig, tenVad: tenVadConfig)

  let vad = EdgevoxOnnxVoiceActivityDetectorWrapper(
