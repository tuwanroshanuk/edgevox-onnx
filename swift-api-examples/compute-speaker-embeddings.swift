...
func computeEmbedding(extractor: EdgevoxOnnxSpeakerEmbeddingExtractorWrapper, waveFilename: String)
  -> [Float]
{
  let audio = EdgevoxOnnxWaveWrapper.readWave(filename: waveFilename)
...
func run() {
  ...
  var config = edgevoxOnnxSpeakerEmbeddingExtractorConfig(model: model)
  let extractor = EdgevoxOnnxSpeakerEmbeddingExtractorWrapper(config: &config)
