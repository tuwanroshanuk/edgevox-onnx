...
func run() {
  ...
  let numSpeakers = 4
  var config = edgevoxOnnxOfflineSpeakerDiarizationConfig(
    segmentation: edgevoxOnnxOfflineSpeakerSegmentationModelConfig(
      pyannote: edgevoxOnnxOfflineSpeakerSegmentationPyannoteModelConfig(model: segmentationModel)),
    embedding: edgevoxOnnxSpeakerEmbeddingExtractorConfig(model: embeddingExtractorModel),
    clustering: edgevoxOnnxFastClusteringConfig(numClusters: numSpeakers)
  )

  let sd = EdgevoxOnnxOfflineSpeakerDiarizationWrapper(config: &config)
