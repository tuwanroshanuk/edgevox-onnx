...
void main(List<String> arguments) async {
  ...
  final segmentationConfig = edgevox_onnx.OfflineSpeakerSegmentationModelConfig(
    pyannote: edgevox_onnx.OfflineSpeakerSegmentationPyannoteModelConfig(
        model: segmentationModel),
  );

  final embeddingConfig =
      edgevox_onnx.SpeakerEmbeddingExtractorConfig(model: embeddingModel);
  ...
  final clusteringConfig =
      edgevox_onnx.FastClusteringConfig(numClusters: 4, threshold: 0.5);

  var config = edgevox_onnx.OfflineSpeakerDiarizationConfig(
  ...
  final sd = edgevox_onnx.OfflineSpeakerDiarization(config);
  if (sd.ptr == nullptr) {
    return;
  }

  final waveData = edgevox_onnx.readWave(waveFilename);
