...
void main(List<String> arguments) async {
  ...
  final ctc = edgevox_onnx.OnlineZipformer2CtcModelConfig(
    model: model,
  );

  final modelConfig = edgevox_onnx.OnlineModelConfig(
    zipformer2Ctc: ctc,
    tokens: tokens,
    debug: true,
    numThreads: 1,
  );
  final config = edgevox_onnx.OnlineRecognizerConfig(
    model: modelConfig,
    ctcFstDecoderConfig: edgevox_onnx.OnlineCtcFstDecoderConfig(graph: hlg),
  );
  final recognizer = edgevox_onnx.OnlineRecognizer(config);
