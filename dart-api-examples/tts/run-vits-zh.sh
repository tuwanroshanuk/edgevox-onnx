#!/usr/bin/env bash

set -ex

dart pub get


# Please visit
# https://github.com/k2-fsa/edgevox-onnx/releases/tag/tts-models
# to download more models

if [[ ! -f ./edgevox-onnx-vits-zh-ll/tokens.txt ]]; then
  curl -SL -O https://github.com/k2-fsa/edgevox-onnx/releases/download/tts-models/edgevox-onnx-vits-zh-ll.tar.bz2
  tar xvf edgevox-onnx-vits-zh-ll.tar.bz2
  rm edgevox-onnx-vits-zh-ll.tar.bz2
fi

dart run \
  ./bin/vits-zh.dart \
  --model ./edgevox-onnx-vits-zh-ll/model.onnx \
  --lexicon ./edgevox-onnx-vits-zh-ll/lexicon.txt \
  --tokens ./edgevox-onnx-vits-zh-ll/tokens.txt \
  --sid 2 \
  --speed 1.0 \
  --text '当夜幕降临，星光点点，伴随着微风拂面，我在静谧中感受着时光的流转，思念如涟漪荡漾，梦境如画卷展开，我与自然融为一体，沉静在这片宁静的美丽之中，感受着生命的奇迹与温柔。' \
  --output-wav vits-zh-jieba-2.wav

dart run \
  ./bin/vits-zh.dart \
  --model ./edgevox-onnx-vits-zh-ll/model.onnx \
  --lexicon ./edgevox-onnx-vits-zh-ll/lexicon.txt \
  --tokens ./edgevox-onnx-vits-zh-ll/tokens.txt \
  --rule-fsts "./edgevox-onnx-vits-zh-ll/phone.fst,./edgevox-onnx-vits-zh-ll/date.fst,./edgevox-onnx-vits-zh-ll/number.fst" \
  --sid 3 \
  --speed 1.0 \
  --text '今天是2024年6月15号，13点23分。如果有困难，请拨打110或者18920240511。123456块钱。' \
  --output-wav vits-zh-jieba-3.wav

ls -lh *.wav
