...
Float32List computeEmbedding(
    {required edgevox_onnx.SpeakerEmbeddingExtractor extractor,
    required String filename}) {
  final waveData = edgevox_onnx.readWave(filename);
...
void main(List<String> arguments) async {
  ...
  */
  ...
  final config = edgevox_onnx.SpeakerEmbeddingExtractorConfig(
    model: model,
    numThreads: 1,
    debug: true,
    provider: 'cpu',
  );
  final extractor = edgevox_onnx.SpeakerEmbeddingExtractor(config: config);

  final manager = edgevox_onnx.SpeakerEmbeddingManager(extractor.dim);
