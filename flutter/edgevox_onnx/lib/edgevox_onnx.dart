...
/// Dart bindings for the public edgevox-onnx inference APIs.
///
...
/// can load the underlying native `edgevox-onnx-c-api` library for the current
/// platform.
///
...
import 'src/edgevox_onnx_bindings.dart';
...
final DynamicLibrary _dylib = () {
  if (Platform.isMacOS) {
    if (_path == null) {
      return DynamicLibrary.open('libedgevox-onnx-c-api.dylib');
    } else {
      return DynamicLibrary.open('$_path/libedgevox-onnx-c-api.dylib');
    }
  }

  if (Platform.isIOS) {
    if (_path == null) {
      return DynamicLibrary.open('edgevox_onnx.framework/edgevox_onnx');
    } else {
      return DynamicLibrary.open('$_path/edgevox_onnx.framework/edgevox_onnx');
    }
  }

  if (Platform.isAndroid || Platform.isLinux) {
    if (_path == null) {
      return DynamicLibrary.open('libedgevox-onnx-c-api.so');
    } else {
      return DynamicLibrary.open('$_path/libedgevox-onnx-c-api.so');
    }
  }

  if (Platform.isWindows) {
    if (_path == null) {
      return DynamicLibrary.open('edgevox-onnx-c-api.dll');
    } else {
      return DynamicLibrary.open('$_path\\edgevox-onnx-c-api.dll');
    }
  }
...
/// Initialize the native edgevox-onnx bindings.
///
...
void initBindings([String? p]) {
  _path ??= p;
  EdgevoxOnnxBindings.init(_dylib);
}
