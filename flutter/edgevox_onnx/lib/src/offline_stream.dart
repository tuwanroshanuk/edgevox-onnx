...
class OfflineStream {
  ...
  void free() {
    if (EdgevoxOnnxBindings.destroyOfflineStream == null) {
    ...
    EdgevoxOnnxBindings.destroyOfflineStream?.call(ptr);
    ptr = nullptr;
  }
  ...
  void acceptWaveform({required Float32List samples, required int sampleRate}) {
    if (EdgevoxOnnxBindings.acceptWaveformOffline == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }
    ...
    EdgevoxOnnxBindings.acceptWaveformOffline?.call(ptr, sampleRate, p, n);
  ...
  void setOption({required String key, required String value}) {
    if (EdgevoxOnnxBindings.offlineStreamSetOption == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }
    ...
    EdgevoxOnnxBindings.offlineStreamSetOption?.call(ptr, pKey, pValue);
    calloc.free(pKey);
    calloc.free(pValue);
  }

  Pointer<EdgevoxOnnxOfflineStream> ptr;
}
