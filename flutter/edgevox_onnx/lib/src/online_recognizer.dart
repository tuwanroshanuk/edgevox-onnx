...
class OnlineRecognizer {
  ...
  factory OnlineRecognizer(OnlineRecognizerConfig config) {
    if (EdgevoxOnnxBindings.createOnlineRecognizer == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }

    final c = calloc<EdgevoxOnnxOnlineRecognizerConfig>();
    ...
    final ptr = EdgevoxOnnxBindings.createOnlineRecognizer?.call(c) ?? nullptr;
  ...
  void free() {
    if (EdgevoxOnnxBindings.destroyOnlineRecognizer == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }
    ...
    EdgevoxOnnxBindings.destroyOnlineRecognizer?.call(ptr);
    ptr = nullptr;
  }
  ...
  OnlineStream createStream({String hotwords = ''}) {
    if (hotwords == '') {
      if (EdgevoxOnnxBindings.createOnlineStream == null) {
        throw Exception("Please initialize edgevox-onnx first");
      }
    } else {
      if (EdgevoxOnnxBindings.createOnlineStreamWithHotwords == null) {
        throw Exception("Please initialize edgevox-onnx first");
      }
    }
    ...
    if (hotwords == '') {
      final p = EdgevoxOnnxBindings.createOnlineStream?.call(ptr) ?? nullptr;
      if (p == nullptr) {
        throw Exception("Failed to create online stream");
      }
    ...
    final p =
        EdgevoxOnnxBindings.createOnlineStreamWithHotwords?.call(ptr, utf8) ??
            nullptr;
    calloc.free(utf8);
  ...
  bool isReady(OnlineStream stream) {
    if (EdgevoxOnnxBindings.isOnlineStreamReady == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }
    ...
    int ready =
        EdgevoxOnnxBindings.isOnlineStreamReady?.call(ptr, stream.ptr) ?? 0;
  ...
  OnlineRecognizerResult getResult(OnlineStream stream) {
    if (EdgevoxOnnxBindings.getOnlineStreamResultAsJson == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }
    ...
    final json =
        EdgevoxOnnxBindings.getOnlineStreamResultAsJson?.call(ptr, stream.ptr) ??
            nullptr;
    if (json == nullptr) {
      return OnlineRecognizerResult(text: '', tokens: [], timestamps: []);
    }
    ...
    EdgevoxOnnxBindings.destroyOnlineStreamResultJson?.call(json);
  ...
  void reset(OnlineStream stream) {
    if (EdgevoxOnnxBindings.reset == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }
    ...
    EdgevoxOnnxBindings.reset?.call(ptr, stream.ptr);
  }

  /// Decode one incremental step for [stream].
  void decode(OnlineStream stream) {
    if (EdgevoxOnnxBindings.decodeOnlineStream == null) {
      throw Exception("Please initialize edgevox-onnx first");
    }
  ...
  bool isEndpoint(OnlineStream stream) {
    ...
    int yes = EdgevoxOnnxBindings.isEndpoint?.call(ptr, stream.ptr) ?? 0;

    return yes == 1;
  }

  Pointer<EdgevoxOnnxOnlineRecognizer> ptr;
