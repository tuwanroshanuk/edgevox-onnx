...
CStreamingSpeechRecognitionDlg::~CStreamingSpeechRecognitionDlg() {
  if (recognizer_) {
    EdgevoxOnnxDestroyOnlineRecognizer(recognizer_);
    recognizer_ = nullptr;
  }
}
...
static int32_t RecordCallback(const void *input_buffer,
  ...
  if (stream) {
    EdgevoxOnnxOnlineStreamAcceptWaveform(stream, 16000, reinterpret_cast<const float *>(input_buffer),
                   frames_per_buffer);
  }
...
void CStreamingSpeechRecognitionDlg::OnBnClickedOk() {
  ...
  if (!started_) {
    ...
    if (stream_) {
      EdgevoxOnnxDestroyOnlineStream(stream_);
      stream_ = nullptr;
    }

    stream_ = EdgevoxOnnxCreateOnlineStream(recognizer_);
...
void CStreamingSpeechRecognitionDlg::InitParaformer() {
  ...
  recognizer_ = EdgevoxOnnxCreateOnlineRecognizer(&config);
}
...
int CStreamingSpeechRecognitionDlg::RunThread() {
  ...
  while (started_) {
    while (EdgevoxOnnxIsOnlineStreamReady(recognizer_, stream_)) {
      EdgevoxOnnxDecodeOnlineStream(recognizer_, stream_);
    }

    auto r = EdgevoxOnnxGetOnlineStreamResult(recognizer_, stream_);
    ...
    int is_endpoint = EdgevoxOnnxOnlineStreamIsEndpoint(recognizer_, stream_);
    if (is_endpoint) {
      EdgevoxOnnxOnlineStreamReset(recognizer_, stream_);
      if (!text.empty()) {
