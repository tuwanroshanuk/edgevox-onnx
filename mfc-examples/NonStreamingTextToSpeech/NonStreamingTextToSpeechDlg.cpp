...
void CNonStreamingTextToSpeechDlg::Init() {
  ...
  tts_ = EdgevoxOnnxCreateOfflineTts(&config);
}

 CNonStreamingTextToSpeechDlg::~CNonStreamingTextToSpeechDlg() {
  if (tts_) {
    EdgevoxOnnxDestroyOfflineTts(tts_);
  }
...
void CNonStreamingTextToSpeechDlg::OnBnClickedOk() {
  ...
  // for simplicity
  play_thread_ = std::make_unique<std::thread>(StartPlayback, EdgevoxOnnxOfflineTtsSampleRate(tts_));
  ...
  generate_thread_ = std::make_unique<std::thread>([ss, this,filename, speaker_id, speed]() {
      ...
      EdgevoxOnnxGenerationConfig config = {0};
      config.silence_scale = 0.2f;
      config.sid = speaker_id;
      config.speed = speed;

      const EdgevoxOnnxGeneratedAudio *audio =
          EdgevoxOnnxOfflineTtsGenerateWithConfig(tts_, text.c_str(), &config,
                                                &AudioGeneratedProgressCallback,
                                                nullptr);
      // generate_btn_.EnableWindow(TRUE);
       g_stopped = true;

      int ok = EdgevoxOnnxWriteWave(audio->samples, audio->n, audio->sample_rate,
                                   filename.c_str());
