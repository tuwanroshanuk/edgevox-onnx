...
CNonStreamingSpeechRecognitionDlg::~CNonStreamingSpeechRecognitionDlg() {
  if (recognizer_) {
    EdgevoxOnnxDestroyOfflineRecognizer(recognizer_);
    recognizer_ = nullptr;
  }
}
...
void CNonStreamingSpeechRecognitionDlg::OnBnClickedOk() {
  ...
  if (!started_) {
    ...
    const EdgevoxOnnxOfflineStream *stream = EdgevoxOnnxCreateOfflineStream(recognizer_);

    EdgevoxOnnxAcceptWaveformOffline(stream, config_.feat_config.sample_rate,
                          samples_.data(), static_cast<int32_t>(samples_.size()));
    EdgevoxOnnxDecodeOfflineStream(recognizer_, stream);
    auto r = EdgevoxOnnxGetOfflineStreamResult(stream);
...
void CNonStreamingSpeechRecognitionDlg::InitWhisper() {
  ...
  recognizer_ = EdgevoxOnnxCreateOfflineRecognizer(&config_);
}
