...
function initEdgevoxOnnxOfflineTtsVitsModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineTtsMatchaModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineTtsKokoroModelConfig(config, Module) {
  const modelLen = Module.lengthBytesUTF8(config.model) + 1;
...
function initEdgevoxOnnxOfflineTtsKittenModelConfig(config, Module) {
  const modelLen = Module.lengthBytesUTF8(config.model) + 1;
  const voicesLen = Module.lengthBytesUTF8(config.voices) + 1;
  const tokensLen = Module.lengthBytesUTF8(config.tokens || '') + 1;
  const dataDirLen = Module.lengthBytesUTF8(config.dataDir || '') + 1;
...
function initEdgevoxOnnxOfflineTtsZipVoiceModelConfig(config, Module) {
  const tokensLen = Module.lengthBytesUTF8(config.tokens || '') + 1;
...
function initEdgevoxOnnxOfflineTtsPocketModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineTtsSupertonicModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineTtsModelConfig(config, Module) {
  ...
  const matchaModelConfig = initEdgevoxOnnxOfflineTtsMatchaModelConfig(
      config.offlineTtsMatchaModelConfig, Module);

  const kokoroModelConfig = initEdgevoxOnnxOfflineTtsKokoroModelConfig(
      config.offlineTtsKokoroModelConfig, Module);

  const kittenModelConfig = initEdgevoxOnnxOfflineTtsKittenModelConfig(
      config.offlineTtsKittenModelConfig, Module);

  const zipVoiceModelConfig = initEdgevoxOnnxOfflineTtsZipVoiceModelConfig(
      config.offlineTtsZipVoiceModelConfig, Module);

  const pocketModelConfig = initEdgevoxOnnxOfflineTtsPocketModelConfig(
...
function initEdgevoxOnnxOfflineTtsConfig(config, Module) {
  const modelConfig =
      initEdgevoxOnnxOfflineTtsModelConfig(config.offlineTtsModelConfig, Module);
...
// Allocate a EdgevoxOnnxGenerationConfig in WASM
function initEdgevoxOnnxGenerationConfig(config, Module) {
  const len = 9 * 4;
  const ptr = Module._malloc(len);
...
// Free the memory allocated for a EdgevoxOnnxGenerationConfig
function freeEdgevoxOnnxGenerationConfig(cfg, Module) {
  if (!cfg) return;
...
class OfflineTts {
  constructor(configObj, Module) {
    const config = initEdgevoxOnnxOfflineTtsConfig(configObj, Module)
    const handle = Module._EdgevoxOnnxCreateOfflineTts(config.ptr);

    freeConfig(config, Module);

    this.handle = handle;
    this.sampleRate = Module._EdgevoxOnnxOfflineTtsSampleRate(this.handle);
    this.numSpeakers = Module._EdgevoxOnnxOfflineTtsNumSpeakers(this.handle);
    this.Module = Module
  }
  ...
  free() {
    ...
    this.Module._EdgevoxOnnxDestroyOfflineTts(this.handle);
    this.handle = 0
  }
  ...
  generate(config) {
    ...
    const cfgWasm = initEdgevoxOnnxGenerationConfig(genConfig, this.Module);

    const h = this.Module._EdgevoxOnnxOfflineTtsGenerateWithConfig(
        this.handle, textPtr, cfgWasm.ptr, 0, 0);

    freeEdgevoxOnnxGenerationConfig(cfgWasm, this.Module);
    ...
    this.Module._EdgevoxOnnxDestroyOfflineTtsGeneratedAudio(h);
  ...
  generateWithConfig(text, genConfig) {
    ...
    let audioPtr = 0;
    try {
      audioPtr = this.Module._EdgevoxOnnxOfflineTtsGenerateWithConfig(
    ...
    this.Module._EdgevoxOnnxDestroyOfflineTtsGeneratedAudio(audioPtr);
  ...
  save(filename, audio) {
    ...
    this.Module._EdgevoxOnnxWriteWave(ptr, samples.length, sampleRate, buffer);
