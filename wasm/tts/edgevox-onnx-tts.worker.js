...
// pthread bootstrap).
import createModule from "./edgevox-onnx-wasm-main-tts.js";
import {
  createOfflineTts,
  getDefaultOfflineTtsModelType,
} from "./edgevox-onnx-tts.js";
...
const Module = await createModule({
  ...
  setStatus: (status) => {
    self.postMessage({ type: "edgevox-onnx-tts-progress", status });
  },
});
  ...
  if (type === "generate") {
    ...
    try {
      ...
      config.callback = (samples, n, progress) => {
        self.postMessage({
          type: "edgevox-onnx-tts-generation-progress",
          progress: progress,
        });
        return 1;
      };
