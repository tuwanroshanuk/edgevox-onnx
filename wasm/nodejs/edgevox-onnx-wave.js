...
function readWave(filename, Module) {
  ...
  const w = Module._EdgevoxOnnxReadWave(pFilename);
...
function readWaveFromBinaryData(uint8Array, Module) {
  ...
  const w =
      Module._EdgevoxOnnxReadWaveFromBinaryData(dataOnHeap.byteOffset, numBytes);
  if (w == 0) {
