...
function initEdgevoxOnnxOfflineSpeakerSegmentationPyannoteModelConfig(
    config, Module) {
...
function initEdgevoxOnnxOfflineSpeakerSegmentationModelConfig(config, Module) {
  ...
  const pyannote = initEdgevoxOnnxOfflineSpeakerSegmentationPyannoteModelConfig(
...
function initEdgevoxOnnxSpeakerEmbeddingExtractorConfig(config, Module) {
  const modelLen = Module.lengthBytesUTF8(config.model || '') + 1;
  const providerLen = Module.lengthBytesUTF8(config.provider || 'cpu') + 1;
...
function initEdgevoxOnnxFastClusteringConfig(config, Module) {
  const len = 2 * 4;
  const ptr = Module._malloc(len);
...
function initEdgevoxOnnxOfflineSpeakerDiarizationConfig(config, Module) {
  if (!('segmentation' in config)) {
    config.segmentation = {
      pyannote: {model: ''},
      numThreads: 1,
  ...
  const segmentation = initEdgevoxOnnxOfflineSpeakerSegmentationModelConfig(
      config.segmentation, Module);

  const embedding =
      initEdgevoxOnnxSpeakerEmbeddingExtractorConfig(config.embedding, Module);

  const clustering =
      initEdgevoxOnnxFastClusteringConfig(config.clustering, Module);
...
class OfflineSpeakerDiarization {
  constructor(configObj, Module) {
    const config =
        initEdgevoxOnnxOfflineSpeakerDiarizationConfig(configObj, Module)
    // Module._MyPrint(config.ptr);

    const handle =
        Module._EdgevoxOnnxCreateOfflineSpeakerDiarization(config.ptr);

    freeConfig(config, Module);

    this.handle = handle;
    this.sampleRate =
        Module._EdgevoxOnnxOfflineSpeakerDiarizationGetSampleRate(this.handle);
    this.Module = Module
  ...
  free() {
    this.Module._EdgevoxOnnxDestroyOfflineSpeakerDiarization(this.handle);
    this.handle = 0
  }
  ...
  setConfig(configObj) {
    ...
    const config =
        initEdgevoxOnnxOfflineSpeakerDiarizationConfig(configObj, this.Module);

    this.Module._EdgevoxOnnxOfflineSpeakerDiarizationSetConfig(
  ...
  process(samples) {
    ...
    let r = this.Module._EdgevoxOnnxOfflineSpeakerDiarizationProcess(
        this.handle, pointer, samples.length);
    this.Module._free(pointer);

    let numSegments =
        this.Module._EdgevoxOnnxOfflineSpeakerDiarizationResultGetNumSegments(r);

    let segments =
        this.Module._EdgevoxOnnxOfflineSpeakerDiarizationResultSortByStartTime(
            r);
    ...
    this.Module._EdgevoxOnnxOfflineSpeakerDiarizationDestroySegment(segments);
    this.Module._EdgevoxOnnxOfflineSpeakerDiarizationDestroyResult(r);
