...
function initEdgevoxOnnxOnlineTransducerModelConfig(config, Module) {
...
function initEdgevoxOnnxOnlineParaformerModelConfig(config, Module) {
  const encoderLen = Module.lengthBytesUTF8(config.encoder || '') + 1;
  const decoderLen = Module.lengthBytesUTF8(config.decoder || '') + 1;
...
function initEdgevoxOnnxOnlineZipformer2CtcModelConfig(config, Module) {
...
function initEdgevoxOnnxOnlineNemoCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
  const buffer = Module._malloc(n);
...
function initEdgevoxOnnxOnlineToneCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
  const buffer = Module._malloc(n);
...
function initEdgevoxOnnxOnlineModelConfig(config, Module) {
  if (!('transducer' in config)) {
    config.transducer = {
      encoder: '',
      decoder: '',
  ...
  const transducer =
      initEdgevoxOnnxOnlineTransducerModelConfig(config.transducer, Module);

  const paraformer =
      initEdgevoxOnnxOnlineParaformerModelConfig(config.paraformer, Module);

  const zipformer2Ctc = initEdgevoxOnnxOnlineZipformer2CtcModelConfig(
      config.zipformer2Ctc, Module);

  const nemoCtc =
      initEdgevoxOnnxOnlineNemoCtcModelConfig(config.nemoCtc, Module);

  const toneCtc =
      initEdgevoxOnnxOnlineToneCtcModelConfig(config.toneCtc, Module);
...
function initEdgevoxOnnxFeatureConfig(config, Module) {
  const len = 2 * 4;  // 2 pointers
  const ptr = Module._malloc(len);
...
function initEdgevoxOnnxHomophoneReplacerConfig(config, Module) {
  const len = 3 * 4;
  const ptr = Module._malloc(len);
...
function initEdgevoxOnnxOnlineCtcFstDecoderConfig(config, Module) {
  const len = 2 * 4;
  const ptr = Module._malloc(len);
...
function initEdgevoxOnnxOnlineRecognizerConfig(config, Module) {
  ...
  const feat = initEdgevoxOnnxFeatureConfig(config.featConfig, Module);
  const model = initEdgevoxOnnxOnlineModelConfig(config.modelConfig, Module);
  const ctcFstDecoder = initEdgevoxOnnxOnlineCtcFstDecoderConfig(
      config.ctcFstDecoderConfig, Module)
  const hr = initEdgevoxOnnxHomophoneReplacerConfig(config.hr, Module);
...
function initEdgevoxOnnxOfflineTransducerModelConfig(config, Module) {
  const encoderLen = Module.lengthBytesUTF8(config.encoder || '') + 1;
  const decoderLen = Module.lengthBytesUTF8(config.decoder || '') + 1;
  const joinerLen = Module.lengthBytesUTF8(config.joiner || '') + 1;
...
function initEdgevoxOnnxOfflineParaformerModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineNemoEncDecCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineDolphinModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineZipformerCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineWenetCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineOmnilingualAsrCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineMedAsrCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineFireRedAsrCtcModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
...
function initEdgevoxOnnxOfflineFunAsrNanoModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineQwen3AsrModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineWhisperModelConfig(config, Module) {
  const encoderLen = Module.lengthBytesUTF8(config.encoder || '') + 1;
  const decoderLen = Module.lengthBytesUTF8(config.decoder || '') + 1;
  const languageLen = Module.lengthBytesUTF8(config.language || '') + 1;
...
function initEdgevoxOnnxOfflineCanaryModelConfig(config, Module) {
  const encoderLen = Module.lengthBytesUTF8(config.encoder || '') + 1;
  const decoderLen = Module.lengthBytesUTF8(config.decoder || '') + 1;
...
function initEdgevoxOnnxOfflineMoonshineModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineFireRedAsrModelConfig(config, Module) {
  const encoderLen = Module.lengthBytesUTF8(config.encoder || '') + 1;
  const decoderLen = Module.lengthBytesUTF8(config.decoder || '') + 1;
...
function initEdgevoxOnnxOfflineTdnnModelConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
  const buffer = Module._malloc(n);
...
function initEdgevoxOnnxOfflineCohereTranscribeModelConfig(config, Module) {
  const encoderLen = Module.lengthBytesUTF8(config.encoder || '') + 1;
  const decoderLen = Module.lengthBytesUTF8(config.decoder || '') + 1;
  const languageLen = Module.lengthBytesUTF8(config.language || '') + 1;
...
function initEdgevoxOnnxOfflineSenseVoiceModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflineLMConfig(config, Module) {
  const n = Module.lengthBytesUTF8(config.model || '') + 1;
  const buffer = Module._malloc(n);
...
function initEdgevoxOnnxOfflineModelConfig(config, Module) {
  if (!('transducer' in config)) {
    config.transducer = {
      encoder: '',
      decoder: '',
  ...
  const transducer =
      initEdgevoxOnnxOfflineTransducerModelConfig(config.transducer, Module);

  const paraformer =
      initEdgevoxOnnxOfflineParaformerModelConfig(config.paraformer, Module);

  const nemoCtc =
      initEdgevoxOnnxOfflineNemoEncDecCtcModelConfig(config.nemoCtc, Module);

  const whisper =
      initEdgevoxOnnxOfflineWhisperModelConfig(config.whisper, Module);

  const tdnn = initEdgevoxOnnxOfflineTdnnModelConfig(config.tdnn, Module);

  const senseVoice =
      initEdgevoxOnnxOfflineSenseVoiceModelConfig(config.senseVoice, Module);

  const moonshine =
      initEdgevoxOnnxOfflineMoonshineModelConfig(config.moonshine, Module);

  const fireRedAsr =
      initEdgevoxOnnxOfflineFireRedAsrModelConfig(config.fireRedAsr, Module);

  const dolphin =
      initEdgevoxOnnxOfflineDolphinModelConfig(config.dolphin, Module);

  const zipformerCtc =
      initEdgevoxOnnxOfflineZipformerCtcModelConfig(config.zipformerCtc, Module);

  const canary = initEdgevoxOnnxOfflineCanaryModelConfig(config.canary, Module);

  const wenetCtc =
      initEdgevoxOnnxOfflineWenetCtcModelConfig(config.wenetCtc, Module);

  const omnilingual = initEdgevoxOnnxOfflineOmnilingualAsrCtcModelConfig(
      config.omnilingual, Module);

  const medasr =
      initEdgevoxOnnxOfflineMedAsrCtcModelConfig(config.medasr, Module);

  const funasrNano =
      initEdgevoxOnnxOfflineFunAsrNanoModelConfig(config.funasrNano, Module);

  const fireRedAsrCtc = initEdgevoxOnnxOfflineFireRedAsrCtcModelConfig(
      config.fireRedAsrCtc, Module);

  const qwen3Asr =
      initEdgevoxOnnxOfflineQwen3AsrModelConfig(config.qwen3Asr, Module);

  const cohereTranscribe = initEdgevoxOnnxOfflineCohereTranscribeModelConfig(
...
function initEdgevoxOnnxOfflineRecognizerConfig(config, Module) {
  if (!('featConfig' in config)) {
    config.featConfig = {
      sampleRate: 16000,
      featureDim: 80,
  ...
  const feat = initEdgevoxOnnxFeatureConfig(config.featConfig, Module);
  const model = initEdgevoxOnnxOfflineModelConfig(config.modelConfig, Module);
  const lm = initEdgevoxOnnxOfflineLMConfig(config.lmConfig, Module);
  const hr = initEdgevoxOnnxHomophoneReplacerConfig(config.hr, Module);
...
class OfflineStream {
  ...
  free() {
    if (this.handle) {
      this.Module._EdgevoxOnnxDestroyOfflineStream(this.handle);
      this.handle = null;
    }
  }
  ...
  acceptWaveform(sampleRate, samples) {
    ...
    this.Module._EdgevoxOnnxAcceptWaveformOffline(
  ...
  setOption(key, value) {
    ...
    this.Module._EdgevoxOnnxOfflineStreamSetOption(this.handle, pKey, pValue);
  ...
  getOption(key) {
    ...
    this.Module.stringToUTF8(key, pKey, keyLen);
    const pValue =
        this.Module._EdgevoxOnnxOfflineStreamGetOption(this.handle, pKey);
...
class OfflineRecognizer {
  constructor(configObj, Module) {
    ...
    const config = initEdgevoxOnnxOfflineRecognizerConfig(configObj, Module);
    const handle = Module._EdgevoxOnnxCreateOfflineRecognizer(config.ptr);
  ...
  setConfig(configObj) {
    const config =
        initEdgevoxOnnxOfflineRecognizerConfig(configObj, this.Module);
    this.Module._EdgevoxOnnxOfflineRecognizerSetConfig(this.handle, config.ptr);
    freeConfig(config, this.Module);
  }

  free() {
    this.Module._EdgevoxOnnxDestroyOfflineRecognizer(this.handle);
    this.handle = 0
  }

  createStream() {
    const handle = this.Module._EdgevoxOnnxCreateOfflineStream(this.handle);
    return new OfflineStream(handle, this.Module);
  }

  decode(stream) {
    this.Module._EdgevoxOnnxDecodeOfflineStream(this.handle, stream.handle);
  }

  getResult(stream) {
    const r =
        this.Module._EdgevoxOnnxGetOfflineStreamResultAsJson(stream.handle);
    const jsonStr = this.Module.UTF8ToString(r);
    const ans = JSON.parse(jsonStr);
    this.Module._EdgevoxOnnxDestroyOfflineStreamResultJson(r);
...
class OnlineStream {
  ...
  free() {
    if (this.handle) {
      this.Module._EdgevoxOnnxDestroyOnlineStream(this.handle);
      this.handle = null;
  ...
  acceptWaveform(sampleRate, samples) {
    ...
    this.Module._EdgevoxOnnxOnlineStreamAcceptWaveform(
        this.handle, sampleRate, this.pointer, samples.length);
  }

  inputFinished() {
    this.Module._EdgevoxOnnxOnlineStreamInputFinished(this.handle);
  }
  ...
  setOption(key, value) {
    ...
    this.Module.stringToUTF8(key, pKey, keyLen);
    this.Module.stringToUTF8(value, pValue, valueLen);
    this.Module._EdgevoxOnnxOnlineStreamSetOption(this.handle, pKey, pValue);
    this.Module._free(pKey);
    this.Module._free(pValue);
  }
  ...
  getOption(key) {
    ...
    this.Module.stringToUTF8(key, pKey, keyLen);
    const pValue =
        this.Module._EdgevoxOnnxOnlineStreamGetOption(this.handle, pKey);
    const value = this.Module.UTF8ToString(pValue);
    this.Module._free(pKey);
    return value;
  }
...
class OnlineRecognizer {
  constructor(configObj, Module) {
    this.config = configObj;
    const config = initEdgevoxOnnxOnlineRecognizerConfig(configObj, Module)
    const handle = Module._EdgevoxOnnxCreateOnlineRecognizer(config.ptr);
  ...
  free() {
    this.Module._EdgevoxOnnxDestroyOnlineRecognizer(this.handle);
    this.handle = 0
  }

  createStream() {
    const handle = this.Module._EdgevoxOnnxCreateOnlineStream(this.handle);
    return new OnlineStream(handle, this.Module);
  }

  isReady(stream) {
    return this.Module._EdgevoxOnnxIsOnlineStreamReady(
               this.handle, stream.handle) == 1;
  }

  decode(stream) {
    this.Module._EdgevoxOnnxDecodeOnlineStream(this.handle, stream.handle);
  }

  isEndpoint(stream) {
    return this.Module._EdgevoxOnnxOnlineStreamIsEndpoint(
               this.handle, stream.handle) == 1;
  }

  reset(stream) {
    this.Module._EdgevoxOnnxOnlineStreamReset(this.handle, stream.handle);
  }

  getResult(stream) {
    const r = this.Module._EdgevoxOnnxGetOnlineStreamResultAsJson(
        this.handle, stream.handle);
    const jsonStr = this.Module.UTF8ToString(r);
    const ans = JSON.parse(jsonStr);
    this.Module._EdgevoxOnnxDestroyOnlineStreamResultJson(r);
