...
function initEdgevoxOnnxOfflineSpeechDenoiserGtcrnModelConfig(config, Module) {
  if (!('model' in config)) {
    config.model = '';
  }
...
function initEdgevoxOnnxOfflineSpeechDenoiserDpdfNetModelConfig(config, Module) {
  if (!('model' in config)) {
    config.model = '';
  }
...
function initEdgevoxOnnxOfflineSpeechDenoiserModelConfig(config, Module) {
  if (!('gtcrn' in config)) {
    config.gtcrn = {model: ''};
  }
  ...
  const gtcrn =
      initEdgevoxOnnxOfflineSpeechDenoiserGtcrnModelConfig(config.gtcrn, Module);
  const dpdfnet =
      initEdgevoxOnnxOfflineSpeechDenoiserDpdfNetModelConfig(
...
function initEdgevoxOnnxOfflineSpeechDenoiserConfig(config, Module) {
  if (!('model' in config)) {
    config.model = {
      gtcrn: {model: ''},
      dpdfnet: {model: ''},
  ...
  const modelConfig =
      initEdgevoxOnnxOfflineSpeechDenoiserModelConfig(config.model, Module);
...
function copyDenoisedAudio(handle, Module) {
  ...
  Module._EdgevoxOnnxDestroyDenoisedAudio(handle);
...
class OfflineSpeechDenoiser extends SpeechDenoiserBase {
  constructor(configObj, Module) {
    super(Module);
    const config = initEdgevoxOnnxOfflineSpeechDenoiserConfig(configObj, Module);
    const handle = Module._EdgevoxOnnxCreateOfflineSpeechDenoiser(config.ptr);

    freeConfig(config, Module);

    this.handle = handle;
    this.sampleRate =
        Module._EdgevoxOnnxOfflineSpeechDenoiserGetSampleRate(this.handle);
  }

  free() {
    this.Module._EdgevoxOnnxDestroyOfflineSpeechDenoiser(this.handle);
    this.handle = 0;
  }
  ...
  run(samples, sampleRate) {
    ...
    const h = this.Module._EdgevoxOnnxOfflineSpeechDenoiserRun(
...
class OnlineSpeechDenoiser extends SpeechDenoiserBase {
  constructor(configObj, Module) {
    super(Module);
    const config = initEdgevoxOnnxOfflineSpeechDenoiserConfig(configObj, Module);
    const handle = Module._EdgevoxOnnxCreateOnlineSpeechDenoiser(config.ptr);

    freeConfig(config, Module);

    this.handle = handle;
    this.sampleRate =
        Module._EdgevoxOnnxOnlineSpeechDenoiserGetSampleRate(this.handle);
    this.frameShiftInSamples =
        Module._EdgevoxOnnxOnlineSpeechDenoiserGetFrameShiftInSamples(
            this.handle);
  }

  free() {
    this.Module._EdgevoxOnnxDestroyOnlineSpeechDenoiser(this.handle);
    this.handle = 0;
  }
  ...
  run(samples, sampleRate) {
    const pointer =
        this.Module._malloc(samples.length * samples.BYTES_PER_ELEMENT);
    this.Module.HEAPF32.set(samples, pointer / samples.BYTES_PER_ELEMENT);
    const h = this.Module._EdgevoxOnnxOnlineSpeechDenoiserRun(
        this.handle, pointer, samples.length, sampleRate);
    this.Module._free(pointer);
  ...
  flush() {
    const h = this.Module._EdgevoxOnnxOnlineSpeechDenoiserFlush(this.handle);
    return copyDenoisedAudio(h, this.Module);
  }

  reset() {
    this.Module._EdgevoxOnnxOnlineSpeechDenoiserReset(this.handle);
  }
}
