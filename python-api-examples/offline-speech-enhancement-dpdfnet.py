...
Please download DPDFNet models from the edgevox-onnx GitHub release
...
def create_speech_denoiser():
    ...
    if not Path(model_filename).is_file():
        ...
        raise ValueError(
            ...
            "the edgevox-onnx GitHub release or the official Hugging Face hub: "
            "https://github.com/k2-fsa/edgevox-onnx/releases/tag/speech-enhancement-models or "
            "https://huggingface.co/Ceva-IP/DPDFNet"
        )

    config = edgevox_onnx.OfflineSpeechDenoiserConfig(
        model=edgevox_onnx.OfflineSpeechDenoiserModelConfig(
            dpdfnet=edgevox_onnx.OfflineSpeechDenoiserDpdfNetModelConfig(
    ...
    return edgevox_onnx.OfflineSpeechDenoiser(config)
