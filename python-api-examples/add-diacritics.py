...
using the edgevox-onnx Python API.
...
def main():
    ...
    config = edgevox_onnx.OfflineDiacritizationConfig(
        model=edgevox_onnx.OfflineDiacritizationModelConfig(
            catt_encoder=catt_encoder,
            catt_decoder=catt_decoder,
        ),
    )

    diacrt = edgevox_onnx.OfflineDiacritization(config)
