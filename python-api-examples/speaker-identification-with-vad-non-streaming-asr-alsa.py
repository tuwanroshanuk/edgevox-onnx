...
def load_speaker_embedding_model(args):
    config = edgevox_onnx.SpeakerEmbeddingExtractorConfig(
        model=args.model,
        num_threads=args.num_threads,
        debug=args.debug,
    ...
    extractor = edgevox_onnx.SpeakerEmbeddingExtractor(config)
    return extractor
...
def compute_speaker_embedding(
    filenames: List[str],
    extractor: edgevox_onnx.SpeakerEmbeddingExtractor,
) -> np.ndarray:
...
def main():
    ...
    alsa = edgevox_onnx.Alsa(device_name)

    recognizer = create_recognizer(args)
    extractor = load_speaker_embedding_model(args)
    speaker_file = load_speaker_file(args)

    manager = edgevox_onnx.SpeakerEmbeddingManager(extractor.dim)
    ...
    vad_config = edgevox_onnx.VadModelConfig()
    ...
    vad = edgevox_onnx.VoiceActivityDetector(vad_config, buffer_size_in_seconds=100)
