...
"""
This file shows how to use edgevox-onnx Python API for
...
def init_speaker_diarization(num_speakers: int = -1, cluster_threshold: float = 0.5):
    ...
    """
    segmentation_model = "./edgevox-onnx-pyannote-segmentation-3-0/model.onnx"
    embedding_extractor_model = (
        "./3dspeaker_speech_eres2net_base_sv_zh-cn_3dspeaker_16k.onnx"
    )

    config = edgevox_onnx.OfflineSpeakerDiarizationConfig(
        segmentation=edgevox_onnx.OfflineSpeakerSegmentationModelConfig(
            pyannote=edgevox_onnx.OfflineSpeakerSegmentationPyannoteModelConfig(
                model=segmentation_model
            ),
        ),
        embedding=edgevox_onnx.SpeakerEmbeddingExtractorConfig(
            model=embedding_extractor_model
        ),
        clustering=edgevox_onnx.FastClusteringConfig(
    ...
    return edgevox_onnx.OfflineSpeakerDiarization(config)
