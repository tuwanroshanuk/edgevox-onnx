...
python3 ./python-api-examples/non_streaming_server.py \
  --wenet-ctc ./edgevox-onnx-zh-wenet-wenetspeech/model.onnx \
  --tokens ./edgevox-onnx-zh-wenet-wenetspeech/tokens.txt
...
class NonStreamingServer:
    def __init__(
        self,
        recognizer: edgevox_onnx.OfflineRecognizer,
        max_batch_size: int,
        max_wait_ms: float,
        nn_pool_size: int,
        ...
        Args:
          recognizer:
            An instance of the edgevox_onnx.OfflineRecognizer.
          max_batch_size:
...
"""
    ...
    async def compute_and_decode(
        self,
        stream: edgevox_onnx.OfflineStream,
    ) -> None:
...
def create_recognizer(args) -> edgevox_onnx.OfflineRecognizer:
    if args.encoder:
        ...
        recognizer = edgevox_onnx.OfflineRecognizer.from_nemo_ctc(
            model=args.nemo_ctc,
            tokens=args.tokens,
            num_threads=args.num_threads,
            sample_rate=args.sample_rate,
        ...
        recognizer = edgevox_onnx.OfflineRecognizer.from_telespeech_ctc(
        ...
        recognizer = edgevox_onnx.OfflineRecognizer.from_tdnn_ctc(
