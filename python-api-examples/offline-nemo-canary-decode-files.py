...
def create_recognizer():
    ...
    return (
        edgevox_onnx.OfflineRecognizer.from_nemo_canary(
            encoder=encoder,
            decoder=decoder,
            tokens=tokens,
            debug=True,
...
def decode(recognizer, samples, sample_rate, src_lang, tgt_lang):
    ...
    recognizer.recognizer.set_config(
        config=edgevox_onnx.OfflineRecognizerConfig(
            model_config=edgevox_onnx.OfflineModelConfig(
                canary=edgevox_onnx.OfflineCanaryModelConfig(
                    src_lang=src_lang,
                    tgt_lang=tgt_lang,
                )
            )
