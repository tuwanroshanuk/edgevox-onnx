...
function createSpeakerEmbeddingExtractor() {
  ...
  return new edgevox_onnx.SpeakerEmbeddingExtractor(config);
}

function computeEmbedding(extractor, filename) {
  const stream = extractor.createStream();
  const wave = edgevox_onnx.readWave(filename);
  stream.acceptWaveform({sampleRate: wave.sampleRate, samples: wave.samples});
  return extractor.compute(stream);
}

const extractor = createSpeakerEmbeddingExtractor();
const manager = new edgevox_onnx.SpeakerEmbeddingManager(extractor.dim);
