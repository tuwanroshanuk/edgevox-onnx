...
var
  Samples: TEdgevoxOnnxSamplesArray;
  I: Integer;
begin
  EnterCriticalSection(CriticalSection);
  try
...
begin
  ...
  if Tts.GetSampleRate <> DeviceSampleRate then
    Resampler := TEdgevoxOnnxLinearResampler.Create(Tts.GetSampleRate, DeviceSampleRate);
  ...
  EnvStr := GetEnv('EDGEVOX_ONNX_MIC_DEVICE');
  if EnvStr <> '' then
    begin
      DeviceIndex := StrToIntDef(EnvStr, DeviceIndex);
      WriteLn('Use device index from environment variable EDGEVOX_ONNX_MIC_DEVICE: ', EnvStr);
    end;
  ...
  Buffer := TEdgevoxOnnxCircularBuffer.Create(30 * DeviceSampleRate);
  ...
  EdgevoxOnnxWriteWave('./kokoro-zh-en-playback-47.wav', Audio.Samples, Audio.SampleRate);
