...
if [ -n "${EDGEVOX_ONNXRUNTIME_LIB_DIR:-}" ] && [ -n "${EDGEVOX_ONNXRUNTIME_INCLUDE_DIR:-}" ]; then
  ...
  export EDGEVOX_ONNXRUNTIME_LIB_DIR="$EDGEVOX_ONNX_ONNXRUNTIME_ROOT/jni/armeabi-v7a/"
  ...
  export EDGEVOX_ONNXRUNTIME_LIB_DIR=$dir/$onnxruntime_version/jni/armeabi-v7a/
...
if [ $EDGEVOX_ONNX_ENABLE_RKNN == ON ]; then
  ...
  export EDGEVOX_ONNX_RKNN_TOOLKIT2_LIB_DIR=$PWD/librknnrt-android/v$rknn_version/armeabi-v7a/
