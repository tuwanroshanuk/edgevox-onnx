...
int32_t main() {
  ...
  EdgevoxOnnxKeywordSpotterConfig keywords_spotter_config;
  ...
  const EdgevoxOnnxKeywordSpotter *keywords_spotter =
      EdgevoxOnnxCreateKeywordSpotter(&keywords_spotter_config);
  ...
  const EdgevoxOnnxOnlineStream *stream =
      EdgevoxOnnxCreateKeywordStream(keywords_spotter);
...
#define N 3200
  ...
  while (k < wave->num_samples) {
    ...
    while (EdgevoxOnnxIsKeywordStreamReady(keywords_spotter, stream)) {
      EdgevoxOnnxDecodeKeywordStream(keywords_spotter, stream);
    }

    const EdgevoxOnnxKeywordResult *r =
        EdgevoxOnnxGetKeywordResult(keywords_spotter, stream);

    if (strlen(r->keyword)) {
      EdgevoxOnnxPrint(display, segment_id, r->keyword);
    }
  ...
  EdgevoxOnnxDestroyKeywordSpotter(keywords_spotter);
  fprintf(stderr, "\n");
