...
//
//  wget https://github.com/k2-fsa/edgevox-onnx/releases/download/source-separation-models/UVR-MDX-NET-Voc_FT.onnx
//
//  wget https://github.com/k2-fsa/edgevox-onnx/releases/download/source-separation-models/qi-feng-le-zh.wav
//
// 2. Build
//
//  cmake -DEDGEVOX_ONNX_ENABLE_C_API=ON ..
...
int32_t main() {
  ...
  const EdgevoxOnnxMultiChannelWave *wave =
      EdgevoxOnnxReadWaveMultiChannel("./qi-feng-le-zh.wav");
  if (!wave) {
    fprintf(stderr, "Failed to read ./qi-feng-le-zh.wav\n");
    EdgevoxOnnxDestroyOfflineSourceSeparation(ss);
    return -1;
  }
  ...
  const EdgevoxOnnxSourceSeparationOutput *output =
  ...
  if (!output) {
    ...
    EdgevoxOnnxFreeMultiChannelWave(wave);
    EdgevoxOnnxDestroyOfflineSourceSeparation(ss);
    return -1;
  }
  ...
  for (int32_t s = 0;
    ...
    EdgevoxOnnxWriteWaveMultiChannel((const float *const *)output->stems[s].samples, ns,
