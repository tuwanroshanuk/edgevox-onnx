...
static const float *ComputeEmbedding(
    const EdgevoxOnnxSpeakerEmbeddingExtractor *ex, const char *wav_filename) {
...
int32_t main() {
  ...
  const EdgevoxOnnxSpeakerEmbeddingExtractor *ex =
  ...
  int32_t dim = EdgevoxOnnxSpeakerEmbeddingExtractorDim(ex);
  ...
  if (!EdgevoxOnnxSpeakerEmbeddingManagerAddList(manager, "fangjun", spk1_vec)) {
    fprintf(stderr, "Failed to register fangjun\n");
    exit(-1);
  }

  if (!EdgevoxOnnxSpeakerEmbeddingManagerContains(manager, "fangjun")) {
    fprintf(stderr, "Failed to find fangjun\n");
    exit(-1);
  }

  if (!EdgevoxOnnxSpeakerEmbeddingManagerAddList(manager, "leijun", spk2_vec)) {
    fprintf(stderr, "Failed to register leijun\n");
    exit(-1);
  }

  if (!EdgevoxOnnxSpeakerEmbeddingManagerContains(manager, "leijun")) {
    fprintf(stderr, "Failed to find leijun\n");
    exit(-1);
  }

  if (EdgevoxOnnxSpeakerEmbeddingManagerNumSpeakers(manager) != 2) {
  ...
  EdgevoxOnnxSpeakerEmbeddingManagerFreeAllSpeakers(all_speakers);
  ...
  const char *name1 =
      EdgevoxOnnxSpeakerEmbeddingManagerSearch(manager, v1, threshold);
  if (name1) {
    fprintf(stderr, "%s: Found %s\n", test1, name1);
    EdgevoxOnnxSpeakerEmbeddingManagerFreeSearch(name1);
  } else {
    fprintf(stderr, "%s: Not found\n", test1);
  }

  const char *name2 =
      EdgevoxOnnxSpeakerEmbeddingManagerSearch(manager, v2, threshold);
  if (name2) {
    fprintf(stderr, "%s: Found %s\n", test2, name2);
    EdgevoxOnnxSpeakerEmbeddingManagerFreeSearch(name2);
  } else {
    fprintf(stderr, "%s: Not found\n", test2);
  }

  const char *name3 =
      EdgevoxOnnxSpeakerEmbeddingManagerSearch(manager, v3, threshold);
  if (name3) {
    fprintf(stderr, "%s: Found %s\n", test3, name3);
    EdgevoxOnnxSpeakerEmbeddingManagerFreeSearch(name3);
  } else {
    fprintf(stderr, "%s: Not found\n", test3);
  }

  int32_t ok = EdgevoxOnnxSpeakerEmbeddingManagerVerify(manager, "fangjun", v1,
                                                       threshold);
  if (ok) {
  ...
  ok = EdgevoxOnnxSpeakerEmbeddingManagerVerify(manager, "fangjun", v2,
                                               threshold);
  if (ok) {
    fprintf(stderr, "%s matches fangjun\n", test2);
  } else {
  ...
  if (!EdgevoxOnnxSpeakerEmbeddingManagerRemove(manager, "fangjun")) {
    fprintf(stderr, "Failed to remove fangjun\n");
    exit(-1);
  }

  if (EdgevoxOnnxSpeakerEmbeddingManagerNumSpeakers(manager) != 1) {
    fprintf(stderr, "There should be only 1 speaker left\n");
    exit(-1);
  }

  name1 = EdgevoxOnnxSpeakerEmbeddingManagerSearch(manager, v1, threshold);
  if (name1) {
    fprintf(stderr, "%s: Found %s\n", test1, name1);
    EdgevoxOnnxSpeakerEmbeddingManagerFreeSearch(name1);
  } else {
  ...
  if (!EdgevoxOnnxSpeakerEmbeddingManagerRemove(manager, "leijun")) {
    fprintf(stderr, "Failed to remove leijun\n");
    exit(-1);
  }

  if (EdgevoxOnnxSpeakerEmbeddingManagerNumSpeakers(manager) != 0) {
    fprintf(stderr, "There should be only 1 speaker left\n");
    exit(-1);
  }

  name2 = EdgevoxOnnxSpeakerEmbeddingManagerSearch(manager, v2, threshold);
  if (name2) {
    fprintf(stderr, "%s: Found %s\n", test2, name2);
    EdgevoxOnnxSpeakerEmbeddingManagerFreeSearch(name2);
  } else {
  ...
  all_speakers = EdgevoxOnnxSpeakerEmbeddingManagerGetAllSpeakers(manager);
  ...
  EdgevoxOnnxSpeakerEmbeddingManagerFreeAllSpeakers(all_speakers);
  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(v1);
  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(v2);
  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(v3);

  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(spk1_vec[0]);
  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(spk1_vec[1]);
  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(spk1_vec[2]);

  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(spk2_vec[0]);
  EdgevoxOnnxSpeakerEmbeddingExtractorDestroyEmbedding(spk2_vec[1]);

  EdgevoxOnnxDestroySpeakerEmbeddingManager(manager);
  EdgevoxOnnxDestroySpeakerEmbeddingExtractor(ex);
