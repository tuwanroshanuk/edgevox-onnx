...
int32_t main() {
  ...
  for (int32_t i = 0; i != n; ++i) {
    const char *text_with_punct =
        EdgevoxOnnxOnlinePunctuationAddPunct(punct, texts[i]);
    if (!text_with_punct) {
    ...
    EdgevoxOnnxOnlinePunctuationFreeText(text_with_punct);
    fprintf(stderr, "--------------------\n");
  }
