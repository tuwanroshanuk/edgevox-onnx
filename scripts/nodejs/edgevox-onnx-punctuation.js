...
function initEdgevoxOnnxOfflinePunctuationModelConfig(config, Module) {
...
function initEdgevoxOnnxOfflinePunctuationConfig(config, Module) {
  if (!('model' in config)) {
    config.model = {};
  }

  const modelConfig =
      initEdgevoxOnnxOfflinePunctuationModelConfig(config.model, Module);
...
function initEdgevoxOnnxOnlinePunctuationModelConfig(config, Module) {
...
function initEdgevoxOnnxOnlinePunctuationConfig(config, Module) {
  if (!('model' in config)) {
    config.model = {};
  }

  const modelConfig =
      initEdgevoxOnnxOnlinePunctuationModelConfig(config.model, Module);
  const len = modelConfig.len;
  const ptr = Module._malloc(len);
...
class OfflinePunctuation {
  constructor(configObj, Module) {
    const config = initEdgevoxOnnxOfflinePunctuationConfig(configObj, Module);
    const handle = Module._EdgevoxOnnxCreateOfflinePunctuation(config.ptr);
  ...
  free() {
    this.Module._EdgevoxOnnxDestroyOfflinePunctuation(this.handle);
    this.handle = 0;
  }
...
class OnlinePunctuation {
  constructor(configObj, Module) {
    const config = initEdgevoxOnnxOnlinePunctuationConfig(configObj, Module);
    const handle = Module._EdgevoxOnnxCreateOnlinePunctuation(config.ptr);
  ...
  free() {
    this.Module._EdgevoxOnnxDestroyOnlinePunctuation(this.handle);
    this.handle = 0;
  }
  ...
  addPunct(text) {
    ...
    const out = this.Module._EdgevoxOnnxOnlinePunctuationAddPunct(
        this.handle, textPtr);
    this.Module._free(textPtr);

    return copyTextAndFree(
        out, this.Module._EdgevoxOnnxOnlinePunctuationFreeText, this.Module);
  }
}
