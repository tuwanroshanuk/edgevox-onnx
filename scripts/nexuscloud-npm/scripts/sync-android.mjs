/**
 * Copy Android jniLibs + Tts.kt from @nexuscloud/edgevox-onnx into a Capacitor app.
 *
 * Usage (from consumer app root):
 *   node node_modules/@nexuscloud/edgevox-onnx/scripts/sync-android.mjs
...
const assetsVersion = path.join(
  ...
  'main',
  'assets',
  'edgevox-onnx-version.json',
);

function hasExpectedLibs() {
  const abis = ['arm64-v8a', 'armeabi-v7a', 'x86', 'x86_64'];
  const required = ['libedgevox-onnx-jni.so', 'libonnxruntime.so'];
...
function copyAbiLibs() {
  if (!existsSync(srcJni)) {
    throw new Error('Package missing android/jniLibs — rebuild @nexuscloud/edgevox-onnx first.');
  }
...
function copyKotlinApi() {
  ...
  const banner = `// AUTO-SYNCED from @nexuscloud/edgevox-onnx — DO NOT EDIT
// version: ${version}
...
function writeMetadata() {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        version,
        npmPackage: '@nexuscloud/edgevox-onnx',
        syncedAt: new Date().toISOString(),
        note: 'Android jniLibs + Tts.kt from @nexuscloud/edgevox-onnx (ZipVoice espeakVoice fork)',
      },
      null,
      2,
    ),
...
console.log(`[sync-android] Syncing Android → @nexuscloud/edgevox-onnx@${version}`);
copyAbiLibs();
copyKotlinApi();
writeMetadata();
