...
def process(out_dir: Path, whl: Path):
    ...
    if py_version:
        rpath_list = [
            f"$ORIGIN/../lib/python{py_version}/site-packages/edgevox_onnx/lib",
            f"$ORIGIN/../lib/python{py_version}/dist-packages/edgevox_onnx/lib",
            #
            f"$ORIGIN/../lib/python{py_version}/site-packages/edgevox_onnx/lib64",
            f"$ORIGIN/../lib/python{py_version}/dist-packages/edgevox_onnx/lib64",
            #
            f"$ORIGIN/../lib/python{py_version}/site-packages/edgevox_onnx.libs",
        ]
    else:
        rpath_list = []
        for p in ["3.8", "3.9", "3.10", "3.11", "3.12", "3.13", "3.14"]:
            rpath_list.extend(
                [
                    f"$ORIGIN/../lib/python{p}/site-packages/edgevox_onnx/lib",
                    f"$ORIGIN/../lib/python{p}/dist-packages/edgevox_onnx/lib",
                ]
            )

    rpaths = ":".join(rpath_list)

    for filename in glob.glob(f"{tmp_dir}/edgevox_onnx*data/data/bin/*", recursive=True):
        print(filename)
        existing_rpath = (
