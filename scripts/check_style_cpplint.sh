...
edgevox_onnx_dir=$(cd $cur_dir/.. && pwd)

build_dir=$edgevox_onnx_dir/build
mkdir -p $build_dir
...
source $edgevox_onnx_dir/scripts/utils.sh
...
function do_check() {
  case "$1" in
    ...
    2)
      ...
      files=$(find $edgevox_onnx_dir/cxx-api-examples-ignored $edgevox_onnx_dir/c-api-examples-ignored $edgevox_onnx_dir/edgevox-onnx/csrc $edgevox_onnx_dir/edgevox-onnx/python $edgevox_onnx_dir/scripts/node-addon-api/src $edgevox_onnx_dir/edgevox-onnx/jni $edgevox_onnx_dir/edgevox-onnx/c-api -name "*.h" -o -name "*.cc")
      files2=$(find $edgevox_onnx_dir/harmony-os/EdgevoxOnnxHar/edgevox_onnx/src/main/cpp/ -name "*.cc")
      ;;
    *)
