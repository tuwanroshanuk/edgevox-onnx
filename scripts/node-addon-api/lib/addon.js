...
// Package name triggered spam for edgevox-onnx-win32-x64
// so we have renamed it to edgevox-onnx-win-x64
const platform = os.platform() === 'win32' ? 'win' : os.platform();
const arch = os.arch();
const platform_arch = `${platform}-${arch}`;
const possible_paths = [
  '../build/Release/edgevox-onnx.node',
  '../build/Debug/edgevox-onnx.node',
  `./node_modules/edgevox-onnx-${platform_arch}/edgevox-onnx.node`,
  `../edgevox-onnx-${platform_arch}/edgevox-onnx.node`,
  './edgevox-onnx.node',
];
...
if (!addon) {
  let addon_path =
      `${process.env.PWD}/node_modules/edgevox-onnx-${platform_arch}`;
  const pnpmIndex = __dirname.indexOf(`node_modules${path.sep}.pnpm`);
  if (pnpmIndex !== -1) {
    const parts = __dirname.slice(pnpmIndex).split(path.sep);
    parts.pop();
    addon_path =
        `${process.env.PWD}/${parts.join('/')}/edgevox-onnx-${platform_arch}`;
  }

  let msg = `Could not find edgevox-onnx-node. Tried\n\n  ${
      possible_paths.join('\n  ')}\n`
  if (os.platform() == 'darwin' &&
      (!process.env.DYLD_LIBRARY_PATH ||
       !process.env.DYLD_LIBRARY_PATH.includes(
           `node_modules/edgevox-onnx-${platform_arch}`))) {
    msg +=
