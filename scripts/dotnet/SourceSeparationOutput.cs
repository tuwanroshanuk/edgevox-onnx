...
namespace EdgevoxOnnx
{
    public class SourceSeparationOutput : IDisposable
    {
        ...
        public bool SaveStemToWaveFile(int stemIndex, string filename)
        {
            ...
            int status = EdgevoxOnnxWriteWaveMultiChannel(stem.Samples, stem.N, impl.SampleRate, stem.NumChannels, utf8FilenameWithNull);
            return status == 1;
        }
        ...
        private void Cleanup()
        {
            if (Handle != IntPtr.Zero)
            {
                EdgevoxOnnxDestroySourceSeparationOutput(Handle);
            }
        ...
        struct Impl
        {
            public IntPtr Stems; // const EdgevoxOnnxSourceSeparationStem*
            public int NumStems;
            public int SampleRate;
        }
        ...
        private static extern void EdgevoxOnnxDestroySourceSeparationOutput(IntPtr handle);

        [DllImport(Dll.Filename)]
        private static extern int EdgevoxOnnxWriteWaveMultiChannel(IntPtr samples, int n, int sample_rate, int num_channels, [MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.I1)] byte[] utf8Filename);
    }
}
