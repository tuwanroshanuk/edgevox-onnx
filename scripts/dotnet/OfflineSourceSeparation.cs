...
namespace EdgevoxOnnx
{
    public class OfflineSourceSeparation : IDisposable
    {
        ...
        {
            IntPtr h = EdgevoxOnnxCreateOfflineSourceSeparation(ref config);
        ...
        public SourceSeparationOutput Process(float[][] channels, int sampleRate)
        {
            ...
            try
            {
                ...
                IntPtr p = EdgevoxOnnxOfflineSourceSeparationProcess(_handle.Handle, ptrArray, numChannels, numSamples, sampleRate);
        ...
        public int OutputSampleRate
        {
            get
            {
                return EdgevoxOnnxOfflineSourceSeparationGetOutputSampleRate(_handle.Handle);
            }
        }
        ...
        public int NumberOfStems
        {
            get
            {
                return EdgevoxOnnxOfflineSourceSeparationGetNumberOfStems(_handle.Handle);
            }
        }
        ...
        {
            EdgevoxOnnxDestroyOfflineSourceSeparation(_handle.Handle);
        ...
        public static MultiChannelWaveData ReadWaveMultiChannel(string filename)
        {
            ...
            IntPtr wave = EdgevoxOnnxReadWaveMultiChannel(utf8FilenameWithNull);
            if (wave == IntPtr.Zero)
            {
                return null;
            }
        ...
        private static extern IntPtr EdgevoxOnnxCreateOfflineSourceSeparation(ref OfflineSourceSeparationConfig config);

        [DllImport(Dll.Filename)]
        private static extern void EdgevoxOnnxDestroyOfflineSourceSeparation(IntPtr handle);

        [DllImport(Dll.Filename)]
        private static extern int EdgevoxOnnxOfflineSourceSeparationGetOutputSampleRate(IntPtr handle);

        [DllImport(Dll.Filename)]
        private static extern int EdgevoxOnnxOfflineSourceSeparationGetNumberOfStems(IntPtr handle);

        [DllImport(Dll.Filename)]
        private static extern IntPtr EdgevoxOnnxOfflineSourceSeparationProcess(IntPtr handle, IntPtr samples, int num_channels, int num_samples, int sample_rate);

        [DllImport(Dll.Filename)]
        private static extern IntPtr EdgevoxOnnxReadWaveMultiChannel([MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.I1)] byte[] utf8Filename);

        [DllImport(Dll.Filename)]
        private static extern void EdgevoxOnnxFreeMultiChannelWave(IntPtr wave);
    }

    // Wrapper for EdgevoxOnnxMultiChannelWave
    public class MultiChannelWaveData : IDisposable
    {
        ...
        public void Dispose()
        {
            if (_handle.Handle != IntPtr.Zero)
            {
                EdgevoxOnnxFreeMultiChannelWave(_handle.Handle);
            }
            _handle = new HandleRef(this, IntPtr.Zero);
        }
