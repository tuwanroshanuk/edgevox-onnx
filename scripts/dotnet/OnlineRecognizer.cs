...
namespace EdgevoxOnnx
{
    ...
    public class OnlineRecognizer : IDisposable
    {
        ...
        {
            IntPtr h = EdgevoxOnnxCreateOnlineRecognizer(ref config);
            _handle = new HandleRef(this, h);
        }

        public OnlineStream CreateStream()
        {
            IntPtr p = EdgevoxOnnxCreateOnlineStream(_handle.Handle);
        ...
        {
            return EdgevoxOnnxOnlineStreamIsEndpoint(_handle.Handle, stream.Handle) != 0;
        }
        ...
        {
            EdgevoxOnnxOnlineStreamReset(_handle.Handle, stream.Handle);
        }
        ...
        {
            EdgevoxOnnxDestroyOnlineRecognizer(_handle.Handle);
        ...
        private static extern IntPtr EdgevoxOnnxCreateOnlineRecognizer(ref OnlineRecognizerConfig config);

        [DllImport(Dll.Filename)]
        private static extern void EdgevoxOnnxDestroyOnlineRecognizer(IntPtr handle);

        [DllImport(Dll.Filename)]
        private static extern IntPtr EdgevoxOnnxCreateOnlineStream(IntPtr handle);

        [DllImport(Dll.Filename, EntryPoint = "EdgevoxOnnxIsOnlineStreamReady")]
        private static extern int IsReady(IntPtr handle, IntPtr stream);

        [DllImport(Dll.Filename, EntryPoint = "EdgevoxOnnxDecodeOnlineStream")]
        private static extern void Decode(IntPtr handle, IntPtr stream);

        [DllImport(Dll.Filename, EntryPoint = "EdgevoxOnnxDecodeMultipleOnlineStreams")]
        private static extern void Decode(IntPtr handle, IntPtr[] streams, int n);

        [DllImport(Dll.Filename, EntryPoint = "EdgevoxOnnxGetOnlineStreamResult")]
        private static extern IntPtr GetResult(IntPtr handle, IntPtr stream);

        [DllImport(Dll.Filename, EntryPoint = "EdgevoxOnnxDestroyOnlineRecognizerResult")]
        private static extern void DestroyResult(IntPtr result);

        [DllImport(Dll.Filename)]
        private static extern void EdgevoxOnnxOnlineStreamReset(IntPtr handle, IntPtr stream);

        [DllImport(Dll.Filename)]
        private static extern int EdgevoxOnnxOnlineStreamIsEndpoint(IntPtr handle, IntPtr stream);
    }
}
