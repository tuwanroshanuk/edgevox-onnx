...
namespace EdgevoxOnnx
{
    public class OfflineTtsGeneratedAudio
    {
        ...
        public bool SaveToWaveFile(String filename)
        {
            ...
            int status = EdgevoxOnnxWriteWave(impl.Samples, impl.NumSamples, impl.SampleRate, utf8FilenameWithNull);
            return status == 1;
        }
        ...
        {
            EdgevoxOnnxDestroyOfflineTtsGeneratedAudio(Handle);
        ...
        private static extern void EdgevoxOnnxDestroyOfflineTtsGeneratedAudio(IntPtr handle);

        [DllImport(Dll.Filename)]
        private static extern int EdgevoxOnnxWriteWave(IntPtr samples, int n, int sample_rate, [MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.I1)] byte[] utf8Filename);
    }
}
