...
if(EDGEVOX_ONNX_USE_PRE_INSTALLED_ONNXRUNTIME_IF_AVAILABLE)
  ...
  if(DEFINED ENV{EDGEVOX_ONNXRUNTIME_INCLUDE_DIR})
    set(location_onnxruntime_header_dir $ENV{EDGEVOX_ONNXRUNTIME_INCLUDE_DIR})
  ...
  if(DEFINED ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR})
    if(APPLE)
      set(location_onnxruntime_lib $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/libonnxruntime.dylib)
    elseif(WIN32)
      if(EDGEVOX_ONNX_ENABLE_GPU)
        set(location_onnxruntime_lib $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/onnxruntime.dll)
        set(location_onnxruntime_lib2 $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/onnxruntime.lib)
      else()
        set(location_onnxruntime_lib $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/onnxruntime.lib)
        if(EDGEVOX_ONNX_ENABLE_DIRECTML)
          include(onnxruntime-win-x64-directml)
        endif()
      endif()
    else()
      set(location_onnxruntime_lib $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/libonnxruntime.so)
    endif()

    if(NOT EXISTS ${location_onnxruntime_lib})
      message(STATUS "${location_onnxruntime_lib} does not exist. Try static lib")

      set(location_onnxruntime_lib $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/libonnxruntime.a)
      if(NOT EXISTS ${location_onnxruntime_lib})
        # Try framework-style path (onnxruntime 1.26.0+)
        set(location_onnxruntime_lib $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/onnxruntime.framework/onnxruntime)
        if(NOT EXISTS ${location_onnxruntime_lib})
          message(FATAL_ERROR "neither libonnxruntime.a nor onnxruntime.framework/onnxruntime found in $ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}")
        endif()
      endif()
...
if(location_onnxruntime_header_dir AND location_onnxruntime_lib)
  if(NOT DEFINED onnxruntime_lib_files)
    ...
    if(WIN32)
      file(GLOB onnxruntime_lib_files "$ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/*.dll")
    else()
      if(DEFINED ANDROID_ABI)
        file(GLOB onnxruntime_lib_files "$ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/libonnxruntime.so")
      else()
        file(GLOB _onnxruntime_all "$ENV{EDGEVOX_ONNXRUNTIME_LIB_DIR}/libonnxruntime*")
