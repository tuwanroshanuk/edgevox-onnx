...
def need_split_package():
    ans = os.environ.get("EDGEVOX_ONNX_SPLIT_PYTHON_PACKAGE", None)
    return ans is not None


def is_for_pypi():
    ans = os.environ.get("EDGEVOX_ONNX_IS_FOR_PYPI", None)
    return ans is not None
...
def enable_alsa():
    build_alsa = os.environ.get("EDGEVOX_ONNX_ENABLE_ALSA", None)
...
def get_binaries():
    binaries = [
        "edgevox-onnx",
        "edgevox-onnx-keyword-spotter",
        "edgevox-onnx-microphone",
        "edgevox-onnx-microphone-offline",
        "edgevox-onnx-microphone-offline-audio-tagging",
        "edgevox-onnx-microphone-offline-speaker-identification",
        "edgevox-onnx-offline",
        "edgevox-onnx-offline-audio-tagging",
        "edgevox-onnx-offline-denoiser",
        "edgevox-onnx-offline-diacritization",
        "edgevox-onnx-offline-language-identification",
        "edgevox-onnx-offline-punctuation",
        "edgevox-onnx-offline-source-separation",
        "edgevox-onnx-offline-speaker-diarization",
        "edgevox-onnx-offline-tts",
        "edgevox-onnx-offline-tts-play",
        "edgevox-onnx-offline-websocket-server",
        "edgevox-onnx-online-denoiser",
        "edgevox-onnx-online-punctuation",
        "edgevox-onnx-online-websocket-client",
        "edgevox-onnx-online-websocket-server",
        "edgevox-onnx-vad",
        "edgevox-onnx-vad-microphone",
        "edgevox-onnx-vad-microphone-offline-asr",
        "edgevox-onnx-vad-microphone-simulated-streaming-asr",
        "edgevox-onnx-vad-with-offline-asr",
        "edgevox-onnx-vad-with-online-asr",
        "edgevox-onnx-version",
        "edgevox-onnx-pa-devs",
    ]

    if enable_alsa():
        binaries += [
            "edgevox-onnx-alsa",
            "edgevox-onnx-alsa-offline",
            "edgevox-onnx-alsa-offline-audio-tagging",
            "edgevox-onnx-alsa-offline-speaker-identification",
            "edgevox-onnx-offline-tts-play-alsa",
            "edgevox-onnx-vad-alsa",
            "edgevox-onnx-vad-alsa-offline-asr",
        ]
...
class BuildExtension(build_ext):
    def build_extension(self, ext: setuptools.extension.Extension):
        ...
        out_bin_dir = Path(self.build_lib).resolve().parent / "edgevox_onnx" / "bin"
        install_dir = Path(self.build_lib).resolve() / "edgevox_onnx"

        edgevox_onnx_dir = Path(__file__).parent.parent.resolve()

        cmake_args = os.environ.get("EDGEVOX_ONNX_CMAKE_ARGS", "")
        make_args = os.environ.get("EDGEVOX_ONNX_MAKE_ARGS", "")
        ...
        if not need_split_package():
            extra_cmake_args += " -DEDGEVOX_ONNX_ENABLE_C_API=ON "

        extra_cmake_args += " -DEDGEVOX_ONNX_BUILD_C_API_EXAMPLES=OFF "
        extra_cmake_args += " -DEDGEVOX_ONNX_ENABLE_CHECK=OFF "
        extra_cmake_args += " -DEDGEVOX_ONNX_ENABLE_PYTHON=ON "
        extra_cmake_args += " -DEDGEVOX_ONNX_ENABLE_PORTAUDIO=ON "
        if not need_split_package():
            extra_cmake_args += " -DEDGEVOX_ONNX_ENABLE_WEBSOCKET=ON "

        if "PYTHON_EXECUTABLE" not in cmake_args:
            print(f"Setting PYTHON_EXECUTABLE to {sys.executable}")
            cmake_args += f" -DPYTHON_EXECUTABLE={sys.executable}"

        # putting `cmake_args` from env variable ${EDGEVOX_ONNX_CMAKE_ARGS} last,
        ...
        if is_windows():
            if not need_split_package():
                build_cmd = f"""
             cmake {cmake_args} -B {self.build_temp} -S {edgevox_onnx_dir}
             cmake --build {self.build_temp} --target install --config Release -- -m:2
                """
            else:
                build_cmd = f"""
             cmake {cmake_args} -B {self.build_temp} -S {edgevox_onnx_dir}
             cmake --build {self.build_temp} --target _edgevox_onnx --config Release -- -m:2
                """

            print(f"build command is:\n{build_cmd}")

            cmake_configure_cmd = (
                f'cmake {cmake_args} -B "{self.build_temp}" -S "{edgevox_onnx_dir}"'
            )
            ...
            if ret != 0:
                raise Exception("Failed to configure edgevox-onnx")
            ...
            if make_args == "" and system_make_args == "":
                ...
                print('export EDGEVOX_ONNX_MAKE_ARGS="-j"; python setup.py install')
            ...
            if "-G Ninja" in cmake_args:
                if not need_split_package():
                    build_cmd = f"""
                        cd {self.build_temp}
                        cmake {cmake_args} {edgevox_onnx_dir}
            ...
            try:
                ...
                cmake_cmd = ["cmake"] + cmake_args_list + [str(edgevox_onnx_dir)]
                ...
                if "-G Ninja" in cmake_args:
                    if not need_split_package():
                        ...
                        build_cmd_list = ["ninja"] + make_args_list + ["_edgevox_onnx"]
                else:
                    if not need_split_package():
                        build_cmd_list = ["make"] + make_args_list + ["install/strip"]
                    else:
                        build_cmd_list = ["make"] + make_args_list + ["_edgevox_onnx"]
        ...
        if need_split_package():
            dst = os.path.join(f"{self.build_lib}", "edgevox_onnx", "lib")
            ...
            pattern = os.path.join(self.build_temp, "**", f"_edgevox_onnx.*.{ext}")
            matches = glob.glob(pattern, recursive=True)
            print("matches", list(matches))

            for f in matches:
                print(f, os.path.join(f"{self.build_lib}", "edgevox_onnx", "lib"))
                shutil.copy(f"{f}", dst)
                # Directory listing for debugging - safe with shell=False
                if is_windows():
                    # On Windows, use PowerShell's Get-ChildItem or just skip the listing
