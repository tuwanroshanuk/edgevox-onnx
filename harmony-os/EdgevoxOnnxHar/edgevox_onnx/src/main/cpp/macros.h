...
#define EDGEVOX_ONNX_ASSIGN_ATTR_STR(c_name, js_name)                       \
  do {                                                                     \
...
#define EDGEVOX_ONNX_ASSIGN_ATTR_INT32(c_name, js_name)            \
  do {                                                            \
...
#define EDGEVOX_ONNX_ASSIGN_ATTR_FLOAT(c_name, js_name)            \
  do {                                                            \
    if (o.Has(#js_name) && o.Get(#js_name).IsNumber()) {          \
...
#define EDGEVOX_ONNX_DELETE_C_STR(p) \
  do {                              \
    if (p) {                        \
      delete[] p;                   \
    }                               \
