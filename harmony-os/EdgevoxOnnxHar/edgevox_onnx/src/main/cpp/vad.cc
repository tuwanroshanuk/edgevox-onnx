...
static Napi::External<EdgevoxOnnxCircularBuffer> CreateCircularBufferWrapper(
  ...
  const EdgevoxOnnxCircularBuffer *buf =
      EdgevoxOnnxCreateCircularBuffer(info[0].As<Napi::Number>().Int32Value());

  return Napi::External<EdgevoxOnnxCircularBuffer>::New(
      env, const_cast<EdgevoxOnnxCircularBuffer *>(buf),
      [](Napi::Env env, EdgevoxOnnxCircularBuffer *p) {
...
static void CircularBufferPushWrapper(const Napi::CallbackInfo &info) {
  ...
  const EdgevoxOnnxCircularBuffer *buf =
      info[0].As<Napi::External<EdgevoxOnnxCircularBuffer>>().Data();
  ...
  // elements.
  EdgevoxOnnxCircularBufferPush(buf, data.Data(),
                               data.ElementLength() / sizeof(float));
#else
  EdgevoxOnnxCircularBufferPush(buf, data.Data(), data.ElementLength());
#endif
}
...
static Napi::Float32Array CircularBufferGetWrapper(
  ...
  const float *data = EdgevoxOnnxCircularBufferGet(buf, start_index, n);

  if (enable_external_buffer) {
    Napi::ArrayBuffer arrayBuffer = Napi::ArrayBuffer::New(
        env, const_cast<float *>(data), sizeof(float) * n,
        [](Napi::Env /*env*/, void *p) {
          EdgevoxOnnxCircularBufferFree(reinterpret_cast<const float *>(p));
        });
    ...
    EdgevoxOnnxCircularBufferFree(data);
...
static void CircularBufferPopWrapper(const Napi::CallbackInfo &info) {
  ...
  EdgevoxOnnxCircularBufferPop(buf, n);
}
...
static Napi::Number CircularBufferSizeWrapper(const Napi::CallbackInfo &info) {
  ...
  int32_t size = EdgevoxOnnxCircularBufferSize(buf);
...
static Napi::Number CircularBufferHeadWrapper(const Napi::CallbackInfo &info) {
  ...
  int32_t size = EdgevoxOnnxCircularBufferHead(buf);
...
static void CircularBufferResetWrapper(const Napi::CallbackInfo &info) {
  ...
  EdgevoxOnnxCircularBufferReset(buf);
}

static EdgevoxOnnxSileroVadModelConfig GetSileroVadConfig(
    const Napi::Object &obj) {
  EdgevoxOnnxSileroVadModelConfig c;
  memset(&c, 0, sizeof(c));
  ...
  EDGEVOX_ONNX_ASSIGN_ATTR_FLOAT(min_silence_duration, minSilenceDuration);
  EDGEVOX_ONNX_ASSIGN_ATTR_FLOAT(min_speech_duration, minSpeechDuration);
  EDGEVOX_ONNX_ASSIGN_ATTR_INT32(window_size, windowSize);
  EDGEVOX_ONNX_ASSIGN_ATTR_FLOAT(max_speech_duration, maxSpeechDuration);

  return c;
}

static EdgevoxOnnxTenVadModelConfig GetTenVadConfig(const Napi::Object &obj) {
  EdgevoxOnnxTenVadModelConfig c;
  memset(&c, 0, sizeof(c));
...
static Napi::External<EdgevoxOnnxVoiceActivityDetector>
CreateVoiceActivityDetectorWrapper(const Napi::CallbackInfo &info) {
...
#if __OHOS__
  const EdgevoxOnnxVoiceActivityDetector *vad = nullptr;
  ...
  if (use_resource_manager) {
    ...
    vad = EdgevoxOnnxCreateVoiceActivityDetectorOHOS(&c, buffer_size_in_seconds,
                                                    mgr.get());
  } else {
    vad = EdgevoxOnnxCreateVoiceActivityDetector(&c, buffer_size_in_seconds);
  }
#else
  const EdgevoxOnnxVoiceActivityDetector *vad =
      EdgevoxOnnxCreateVoiceActivityDetector(&c, buffer_size_in_seconds);
#endif
  EDGEVOX_ONNX_DELETE_C_STR(c.silero_vad.model);
  EDGEVOX_ONNX_DELETE_C_STR(c.ten_vad.model);
  EDGEVOX_ONNX_DELETE_C_STR(c.provider);

  return Napi::External<EdgevoxOnnxVoiceActivityDetector>::New(
      env, const_cast<EdgevoxOnnxVoiceActivityDetector *>(vad),
      [](Napi::Env env, EdgevoxOnnxVoiceActivityDetector *p) {
...
static void VoiceActivityDetectorAcceptWaveformWrapper(
  ...
  const EdgevoxOnnxVoiceActivityDetector *vad =
      info[0].As<Napi::External<EdgevoxOnnxVoiceActivityDetector>>().Data();
...
#else
  EdgevoxOnnxVoiceActivityDetectorAcceptWaveform(vad, samples.Data(),
...
static Napi::Boolean VoiceActivityDetectorEmptyWrapper(
  ...
  int32_t is_empty = EdgevoxOnnxVoiceActivityDetectorEmpty(vad);
...
static Napi::Boolean VoiceActivityDetectorDetectedWrapper(
  ...
  int32_t is_detected = EdgevoxOnnxVoiceActivityDetectorDetected(vad);
  ...
  if (enable_external_buffer) {
    Napi::ArrayBuffer arrayBuffer = Napi::ArrayBuffer::New(
        ...
        [](Napi::Env /*env*/, void * /*data*/,
           const EdgevoxOnnxSpeechSegment *hint) {
          EdgevoxOnnxDestroySpeechSegment(hint);
        },
        segment);
