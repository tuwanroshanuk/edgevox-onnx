...
// Returns an External handle to a EdgevoxOnnxLinearResampler.
static Napi::External<EdgevoxOnnxLinearResampler> CreateLinearResamplerWrapper(
  ...
  const EdgevoxOnnxLinearResampler *p = EdgevoxOnnxCreateLinearResampler(
      input_sample_rate, output_sample_rate, 0, 0);

  return Napi::External<EdgevoxOnnxLinearResampler>::New(
      env, const_cast<EdgevoxOnnxLinearResampler *>(p),
      [](Napi::Env env, EdgevoxOnnxLinearResampler *ptr) {
        EdgevoxOnnxDestroyLinearResampler(ptr);
      });
}
...
static Napi::Float32Array ResampleLinearWrapper(
  ...
  EdgevoxOnnxLinearResampler *p =
      info[0].As<Napi::External<EdgevoxOnnxLinearResampler>>().Data();
  ...
  const EdgevoxOnnxResampleOut *out =
      EdgevoxOnnxLinearResamplerResample(p, samples.Data(), input_dim, flush);
...
static void LinearResamplerResetWrapper(const Napi::CallbackInfo &info) {
  ...
  EdgevoxOnnxLinearResamplerReset(p);
}
...
static Napi::Number LinearResamplerGetInputSampleRateWrapper(
  ...
  return Napi::Number::New(
      env, EdgevoxOnnxLinearResamplerResampleGetInputSampleRate(p));
}
...
static Napi::Number LinearResamplerGetOutputSampleRateWrapper(
  ...
  return Napi::Number::New(
      env, EdgevoxOnnxLinearResamplerResampleGetOutputSampleRate(p));
}
