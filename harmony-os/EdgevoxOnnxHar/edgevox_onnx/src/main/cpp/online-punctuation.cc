...
static EdgevoxOnnxOnlinePunctuationModelConfig GetOnlinePunctuationModelConfig(
    Napi::Object obj) {
  EdgevoxOnnxOnlinePunctuationModelConfig c;
  memset(&c, 0, sizeof(c));
  ...
  EDGEVOX_ONNX_ASSIGN_ATTR_STR(cnn_bilstm, cnnBilstm);
...
static Napi::External<EdgevoxOnnxOnlinePunctuation>
CreateOnlinePunctuationWrapper(const Napi::CallbackInfo &info) {
  ...
  EdgevoxOnnxOnlinePunctuationConfig c;
  memset(&c, 0, sizeof(c));
  c.model = GetOnlinePunctuationModelConfig(o);

#if __OHOS__
  const EdgevoxOnnxOnlinePunctuation *punct = nullptr;
  ...
  if (use_resource_manager) {
    ...
    punct = EdgevoxOnnxCreateOnlinePunctuationOHOS(&c, mgr.get());
  } else {
    punct = EdgevoxOnnxCreateOnlinePunctuation(&c);
  }
#else
  ...
  EDGEVOX_ONNX_DELETE_C_STR(c.model.cnn_bilstm);
  EDGEVOX_ONNX_DELETE_C_STR(c.model.bpe_vocab);
  ...
  return Napi::External<EdgevoxOnnxOnlinePunctuation>::New(
      env, const_cast<EdgevoxOnnxOnlinePunctuation *>(punct),
      [](Napi::Env env, EdgevoxOnnxOnlinePunctuation *punct) {
...
static Napi::String OnlinePunctuationAddPunctWrapper(
  ...
  const EdgevoxOnnxOnlinePunctuation *punct =
      info[0].As<Napi::External<EdgevoxOnnxOnlinePunctuation>>().Data();
  Napi::String js_text = info[1].As<Napi::String>();
  std::string text = js_text.Utf8Value();

  const char *punct_text =
      EdgevoxOnnxOnlinePunctuationAddPunct(punct, text.c_str());

  Napi::String ans = Napi::String::New(env, punct_text);
  EdgevoxOnnxOnlinePunctuationFreeText(punct_text);
  return ans;
}
