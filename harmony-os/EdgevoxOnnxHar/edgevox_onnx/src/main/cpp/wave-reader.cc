...
static Napi::Object ReadWaveWrapper(const Napi::CallbackInfo &info) {
  ...
  const EdgevoxOnnxWave *wave = EdgevoxOnnxReadWave(filename.c_str());
  if (!wave) {
    std::ostringstream os;
  ...
  if (enable_external_buffer) {
    Napi::ArrayBuffer arrayBuffer = Napi::ArrayBuffer::New(
        ...
        [](Napi::Env /*env*/, void * /*data*/, const EdgevoxOnnxWave *hint) {
          EdgevoxOnnxFreeWave(hint);
        },
        wave);
...
static Napi::Object ReadWaveFromBinaryWrapper(const Napi::CallbackInfo &info) {
  ...
  const EdgevoxOnnxWave *wave = EdgevoxOnnxReadWaveFromBinaryData(
